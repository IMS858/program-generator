...
class TestIntegrationKneeClient(unittest.TestCase):
    ...
    def test_coach_pdf_has_baseline_and_flags(self):
        ...
        # Section is now called DECISIONS · contains machine selected + rationale
        self.assertTrue("DECISIONS" in kt.upper() or "MACHINE SELECTED" in kt.upper())
...
class TestAmandaRegression(unittest.TestCase):
    ...
    Amanda's input ·
      Primary: Rower
      Secondary: SkiErg
      Avoid: Upright Bike, Stationary Bike, Assault Bike
      Limitations: knee_sensitive
      Z2 Baseline Test: Upright Bike (now in avoid list)
      not_cleared_for_intervals

    The engine should ·
      - Reject rower (knee-sensitive)
      - Detect that the bikes are in avoid list
      - Route to arc trainer (only safe option remaining)
      - Flag the baseline-on-avoided contradiction
      - Block intervals
      - NOT include Deep Squat Hold anywhere
      - NOT use HIIT block · use Conditioning Reset instead
      - Render full 4-week progression on cardio session pages
    """
    ...
    def setUpClass(cls):
        ...
        form = {
            ...
            "age_range": "early 50s", "sex": "F",
            "background": "knee surgery 2019",
            "training_frequency": 5, "strength_days": 3, "cardio_days": 2,
            "primary_goal": "Build cardio base safely",
            ...
            "cardio_profile": {
                ...
                "secondary_modalities": ["skierg"],
                "avoid_modalities": ["upright_bike", "stationary_bike", "assault_bike"],
                "limitations": ["knee_sensitive", "not_cleared_for_intervals"],
                "z2_baseline": {
                    "machine": "upright_bike",  # now in avoid list · contradiction
                    "duration_minutes": 20, "avg_hr": 123, "rpe": 5,
                },
                "interval_test": {},
                "hr_recovery": {"end_hr": 150, "one_min_hr": 130},
            },
            "nutrition_strategy": "maintenance", "activity_factor": 1.4,
        }
        cls.pdfs = {}
    ...
    def test_machine_routed_away_from_rower(self):
        # Coach picked rower but engine should reroute (knee-sensitive)
        # AND bikes are in avoid list, so Arc Trainer is the safe pick
        kt = self.text["coach"]
        self.assertIn("Arc Trainer", kt)
        # Rower should appear as REJECTED, not as the chosen machine
        self.assertIn("rejected", kt.lower())

    def test_contradiction_detected(self):
        # Z2 baseline was on upright bike but bike is in avoid list
        kt = self.text["coach"]
        self.assertIn("CONTRADICTIONS RESOLVED", kt.upper())

    def test_no_deep_squat_hold_in_client_plan(self):
        ct = self.text["client"]
        self.assertNotIn("Deep Squat Hold", ct,
                          "Knee-sensitive client must NOT see Deep Squat Hold")

    def test_no_hiit_block_in_client_plan(self):
        ct = self.text["client"]
        # Should be Conditioning Reset, not HIIT Finisher
        self.assertNotIn("HIIT Finisher", ct,
                          "Intervals blocked client must NOT see HIIT Finisher")

    def test_no_sprints_anywhere_in_client(self):
        ct = self.text["client"]
        # Sprint-language vetoed when intervals blocked
        for term in ("Bike Sprints", "Rower Sprints", "SkiErg Intervals"):
            self.assertNotIn(term, ct,
                              f"{term} should not appear when intervals blocked")

    def test_4_week_progression_visible_in_client(self):
        ct = self.text["client"]
        # The 4-week progression renderer adds these labels
        self.assertIn("CARDIO PRESCRIPTION", ct.upper())
        self.assertIn("4-WEEK PROGRESSION", ct.upper())
        # Each week label appears
        for label in ("BASELINE", "BUILD", "CAPACITY", "RETEST"):
            self.assertIn(label, ct.upper())
        self.assertIn("JOINT RESPONSE RULE", ct.upper())

    def test_zone_2_only_language_in_client_when_blocked(self):
        ct = self.text["client"]
        # Should NOT contain interval language since clearance is blocked
        self.assertNotIn("controlled pickups", ct.lower())
        self.assertNotIn("interval block", ct.lower())
        self.assertNotIn("20s hard / 70s easy", ct.lower())

    def test_coach_decisions_section_present(self):
        kt = self.text["coach"]
        self.assertIn("DECISIONS", kt.upper())
        self.assertIn("MACHINE SELECTED", kt.upper())
...
class TestTreadmillModality(unittest.TestCase):
    """Treadmill is a first-class cardio modality across the engine."""

    def test_treadmill_in_modality_registry(self):
        from cardio_rules import MODALITIES, MODALITY_DISPLAY
        self.assertIn("treadmill", MODALITIES)
        self.assertEqual(MODALITY_DISPLAY["treadmill"], "Treadmill")

    def test_treadmill_in_safe_lists(self):
        """Treadmill should be listed safe for low-back, shoulder, knee, wrist."""
        from cardio_rules import _SAFE_MACHINES_BY_LIMIT
        for limit in ("knee_sensitive", "low_back_sensitive",
                       "shoulder_sensitive", "wrist_sensitive"):
            self.assertIn("treadmill", _SAFE_MACHINES_BY_LIMIT[limit],
                            f"treadmill missing from {limit} safe list")

    def test_general_treadmill_no_limits(self):
        """Treadmill primary, no limits → keeps treadmill, walking text."""
        from cardio_rules import (normalize_cardio_profile,
                                   choose_primary_cardio_machine,
                                   generate_cardio_progression)
        class _P: pass
        p = _P()
        p.primary_modality = "treadmill"
        p.secondary_modalities, p.avoid_modalities, p.limitations = [], [], []
        p.z2_baseline = p.interval_test = p.hr_recovery = {}
        n = normalize_cardio_profile(p)
        m, _ = choose_primary_cardio_machine(n)
        self.assertEqual(m, "treadmill")
        prog = generate_cardio_progression(n)
        # Walking language in week 1
        self.assertIn("walk", prog[1]["main"].lower())

    def test_knee_sensitive_treadmill_warning(self):
        """Treadmill primary + knee-sensitive → emit coach warning."""
        from cardio_rules import (normalize_cardio_profile,
                                   choose_primary_cardio_machine,
        ...
        p = _P()
        p.primary_modality = "treadmill"
        p.secondary_modalities, p.avoid_modalities = [], []
        p.limitations = ["knee_sensitive"]
        p.z2_baseline = p.interval_test = p.hr_recovery = {}
        n = normalize_cardio_profile(p)
        flags = generate_cardio_coach_flags(n)
        self.assertTrue(any("Treadmill marked as primary" in f for f in flags),
                         "Should warn about treadmill + knee-sensitive")

    def test_treadmill_avoided_routes_away(self):
        """Treadmill in avoid list + knee-sensitive → routes to bike."""
        from cardio_rules import normalize_cardio_profile, choose_primary_cardio_machine
        class _P: pass
        p = _P()
        p.primary_modality = "treadmill"
        p.secondary_modalities = []
        p.avoid_modalities = ["treadmill"]
        p.limitations = ["knee_sensitive"]
        p.z2_baseline = p.interval_test = p.hr_recovery = {}
        n = normalize_cardio_profile(p)
        m, _ = choose_primary_cardio_machine(n)
        self.assertNotEqual(m, "treadmill")
        self.assertEqual(m, "stationary_bike")

    def test_treadmill_intervals_blocked_says_zone_2_only(self):
        """Treadmill primary + not-cleared-for-intervals → Zone 2 only language.
        Prescription should NOT prescribe intervals/sprints. The coach note
        legitimately says 'no sprints' as a constraint reminder · that's fine.
        """
        from cardio_rules import normalize_cardio_profile, generate_cardio_progression
        class _P: pass
        p = _P()
        ...
        prog = generate_cardio_progression(n)
        for w in (1, 2, 3, 4):
            main = prog[w]["main"].lower()
            # The MAIN prescription line shouldn't mention intervals or sprints
            self.assertNotIn("sprint", main,
                              f"W{w} main prescription should not contain 'sprint': {main}")
            self.assertNotIn(" intervals", main,
                              f"W{w} main prescription should not prescribe intervals: {main}")
            self.assertNotIn("hiit", main,
                              f"W{w} main prescription should not say HIIT: {main}")
            # Should explicitly mention walking
            self.assertIn("walk", main, f"W{w} treadmill main should say walking")


class TestAvoidContradiction(unittest.TestCase):
    """A modality marked AVOID must not appear as a SECONDARY alternative."""

    def test_avoid_filtered_from_secondary_at_normalize(self):
        from cardio_rules import normalize_cardio_profile
        class _P: pass
        p = _P()
        p.primary_modality = "stationary_bike"
        p.secondary_modalities = ["rower", "treadmill"]
        p.avoid_modalities = ["rower"]
        p.limitations = []
        p.z2_baseline = p.interval_test = p.hr_recovery = {}
        n = normalize_cardio_profile(p)
        # Rower was in BOTH lists · normalize should drop it from secondary
        self.assertNotIn("rower", n["secondary_modalities"])
        self.assertIn("treadmill", n["secondary_modalities"])
        # Avoid list keeps rower
...
class TestAmandaRegression(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        form = {
            ...
            "background": "knee surgery 2019", "training_frequency": 5,
            "strength_days": 3, "cardio_days": 2, "primary_goal": "Get stronger",
            ...
            "strength_markers": [], "strength_marker_results": {},
            "strength_marker_tests": [
                {"exercise_name": "Hip Thrusts", "movement_category": "hinge",
                 "load_style": "total_load", "tested_3rm": 235, "form_quality": "clean"},
                {"exercise_name": "Incline Bench Press",  # 30 lb per_hand · should NOT flag
                 "movement_category": "press_horizontal", "load_style": "per_hand",
                 "tested_5rm": 30, "form_quality": "clean"},
            ...
            "cardio_profile": {
                "primary_modality": "stationary_bike",
                "secondary_modalities": ["rower", "treadmill"],  # rower is also in avoid
                "avoid_modalities": ["rower"],  # contradiction · should filter
                "limitations": ["knee_sensitive", "not_cleared_for_intervals"],
                "z2_baseline": {"machine": "stationary_bike", "duration_minutes": 10,
                                  "avg_hr": 130, "rpe": 4},
                "interval_test": {}, "hr_recovery": {"end_hr": 150, "one_min_hr": 130},
            },
            "nutrition_strategy": "maintenance", "activity_factor": 1.4,
        }
        with contextlib.redirect_stdout(io.StringIO()):
            cls.client_pdf, _ = build_program_pdf({**form, "pdf_mode": "client"})
            cls.full_pdf, _ = build_program_pdf({**form, "pdf_mode": "full"})
        from pypdf import PdfReader
        cls.client_text = "\n".join(
            (p.extract_text() or "") for p in PdfReader(io.BytesIO(cls.client_pdf)).pages)
        cls.full_text = "\n".join(
            (p.extract_text() or "") for p in PdfReader(io.BytesIO(cls.full_pdf)).pages)

    def test_no_coach_finisher_in_client(self):
        # Client mode renames "COACH FINISHER" → "ASSISTED RECOVERY"
        self.assertNotIn("COACH FINISHER", self.client_text.upper())
        self.assertIn("ASSISTED RECOVERY", self.client_text.upper())

    def test_no_false_low_weight_warning_for_per_hand(self):
        # 30 lb per_hand DB Incline Bench should NOT be flagged as light
        self.assertNotIn("30 lb is unusually light", self.full_text)

    def test_no_truncation_marker(self):
        self.assertNotIn("(truncated)", self.full_text)

    def test_no_alt_avoid_contradiction(self):
        # Rower is in avoid_modalities · should NOT appear as Alt
        # Look for the appendix line "Primary: ... Alt: ..."
        import re
        m = re.search(r"Primary:\s*[^\n]+Alt:\s*([^\n]+)", self.full_text)
        if m:
            alt = m.group(1)
            self.assertNotIn("Rower", alt,
                              "Rower in avoid list should not appear as Alt")

    def test_intervals_blocked_says_zone_2_only(self):
        self.assertIn("Zone 2 only", self.client_text)
        self.assertNotIn("Zone 2 / intervals", self.client_text)
        self.assertNotIn("Zone 2 or intervals", self.client_text)
