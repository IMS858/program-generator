"""
IMS Program Generator · Test Suite

Covers six concern areas ·
  1. Week consistency       · 4-week block has NO Week 5 / Week 6 language
  2. Strength math          · Epley · working loads · DB rounding · inconsistency flag
  3. Optional testing data  · PDF still generates · falls back to RPE/RIR
  4. Side effects           · Body comp / mobility priorities / PDF generation unchanged
  5. Page replacement       · Old "Notice, adjust, grow" page is GONE · new page appears
  6. PDF text safety        · No visible ellipses · names wrap or shorten cleanly

Run from repo root ·
    python3 -m unittest tests.test_strength_system -v

Or run the file directly ·
    cd tests && python3 test_strength_system.py
"""
import os
import sys
import io
import contextlib
import unittest
from pathlib import Path

# Make the repo importable from the tests folder
THIS_DIR = Path(__file__).resolve().parent
REPO_ROOT = THIS_DIR.parent
sys.path.insert(0, str(REPO_ROOT))
sys.path.insert(0, str(REPO_ROOT / "generator"))


# ────────────────────────────────────────────────────────
# Test fixtures · canonical assessment payloads
# ────────────────────────────────────────────────────────

def assessment_with_tests():
    """A payload representing the spec's strength testing scenario."""
    return {
        "client_name": "Test Client", "age_range": "early 40s", "sex": "M",
        "background": "test", "training_frequency": 4,
        "strength_days": 3, "cardio_days": 1,
        "primary_goal": "Build strength",
        "fra_priorities": ["Hip IR L+R"],
        "mobility_map": [
            {"joint": "hip", "direction": "IR", "side": "L", "rating": "yellow"},
        ],
        "strength_markers": [], "strength_marker_results": {},
        "strength_marker_tests": [
            {
                "exercise_name": "DB Bench Press",
                "load_style": "per_hand", "equipment_type": "dumbbell",
                "load_unit": "lb",
                "tested_12rm": 35, "tested_10rm": 40, "tested_3rm": 55,
                "form_quality": "clean",
            },
        ],
        "constraints": [], "body_comp": {},
        "nutrition_strategy": "strength", "activity_factor": 1.55,
    }


def assessment_no_tests():
    """A payload with NO strength testing data · should still work."""
    return {
        "client_name": "BW Client", "age_range": "early 40s", "sex": "F",
        "background": "test", "training_frequency": 3,
        "strength_days": 2, "cardio_days": 1,
        "primary_goal": "Move better",
        "fra_priorities": ["Hip IR L+R"],
        "mobility_map": [
            {"joint": "hip", "direction": "IR", "side": "R", "rating": "red"},
        ],
        "strength_markers": [], "strength_marker_results": {},
        "strength_marker_tests": [],
        "constraints": [], "body_comp": {},
        "nutrition_strategy": "maintenance", "activity_factor": 1.4,
    }


def assessment_with_body_comp():
    """A payload with full BOD POD numbers · for side-effect verification."""
    p = assessment_with_tests()
    p["body_comp"] = {
        "weight": "180 lbs", "body_fat": "15%",
        "lean_mass": "153 lbs", "fat_mass": "27 lbs",
        "method": "BOD POD (Lohman Model)",
        "rmr_katch_mcardle": "AUTO",
        "tdee_estimated": "AUTO",
        "nutrition_targets": "AUTO",
    }
    return p


def build_pdf_silently(form):
    """Generate a PDF, suppressing print output."""
    from app import build_program_pdf
    with contextlib.redirect_stdout(io.StringIO()):
        return build_program_pdf(form)


def extract_pdf_text(pdf_bytes):
    """Read text from a PDF · uses pypdf if available, else best-effort fallback."""
    try:
        from pypdf import PdfReader
        reader = PdfReader(io.BytesIO(pdf_bytes))
        return "\n".join((p.extract_text() or "") for p in reader.pages)
    except ImportError:
        # pypdf isn't a hard dep · fall back to a naive byte search for ASCII strings
        # which is enough for the kind of substring assertions we make below.
        return pdf_bytes.decode("latin-1", errors="ignore")


# ════════════════════════════════════════════════════════
# 1 · WEEK CONSISTENCY
# ════════════════════════════════════════════════════════

class WeekConsistencyTests(unittest.TestCase):
    """Block 1 is 4 weeks · no Week 5 or Week 6 language anywhere."""

    @classmethod
    def setUpClass(cls):
        pdf_bytes, _ = build_pdf_silently(assessment_with_tests())
        cls.pdf_bytes = pdf_bytes
        cls.pdf_text = extract_pdf_text(pdf_bytes)

    def test_pdf_contains_week_01_04_label(self):
        """PDF header should say WEEK 01-04, not WEEK 01-06."""
        self.assertIn("WEEK 01-04", self.pdf_text)
        self.assertNotIn("WEEK 01-06", self.pdf_text)

    def test_no_week_5_language(self):
        """Old Week 5 verbiage ('Heavy Push', 'WK 5') must be absent."""
        forbidden = ["WK 5", "Week 5", "Heavy Push", "WK5"]
        for token in forbidden:
            self.assertNotIn(
                token, self.pdf_text,
                msg=f"Found forbidden 6-week language: {token!r}"
            )

    def test_no_week_6_language(self):
        """Old Week 6 verbiage ('Deload + Re-Test', 'WK 6') must be absent."""
        forbidden = ["WK 6", "Week 6", "Deload + Re-Test", "WK6", "Re-Test"]
        for token in forbidden:
            self.assertNotIn(
                token, self.pdf_text,
                msg=f"Found forbidden 6-week language: {token!r}"
            )

    def test_program_object_has_4_weeks(self):
        """The Program object built by Generator should have exactly 4 weeks."""
        from generator import Generator, Assessment, parse_fra_priority, MobilityRating
        from strength_testing import StrengthTest
        a = Assessment(
            name="X", age_range="40s", sex="M", background="t",
            training_frequency=4, strength_days=3, cardio_days=1,
            primary_goal="t",
            fra_priorities=[parse_fra_priority("Hip IR L+R")],
            strength_markers=[], constraints=[],
            mobility_map=[MobilityRating(joint="hip", direction="IR",
                                         side="L", rating="yellow")],
            body_comp={}, progression_mode="autoregulated",
            strength_marker_results={}, strength_marker_tests=[],
        )
        with contextlib.redirect_stdout(io.StringIO()):
            g = Generator(libraries_path=str(REPO_ROOT / "libraries"))
            program = g.build_program(a)
        self.assertEqual(len(program.weeks), 4,
                         f"Expected 4 weeks, got {len(program.weeks)}")

    def test_week_intent_strings_only_for_weeks_1_to_4(self):
        """_week_intent should return text for 1-4 only · 5/6 give the unscheduled stub."""
        from generator import Generator
        with contextlib.redirect_stdout(io.StringIO()):
            g = Generator(libraries_path=str(REPO_ROOT / "libraries"))
        for w in (1, 2, 3, 4):
            self.assertNotEqual(g._week_intent(w), "Unscheduled week",
                                f"Week {w} should have a real intent string")
        for w in (5, 6):
            self.assertEqual(g._week_intent(w), "Unscheduled week",
                             f"Week {w} should be an unscheduled stub")


# ════════════════════════════════════════════════════════
# 2 · STRENGTH MATH
# ════════════════════════════════════════════════════════

class StrengthMathTests(unittest.TestCase):
    """The exact spec scenario · DB Bench 12RM=35, 10RM=40, 3RM=55, per-hand."""

    @classmethod
    def setUpClass(cls):
        from strength_testing import StrengthTest
        cls.test = StrengthTest.from_dict({
            "exercise_name": "DB Bench Press",
            "load_style": "per_hand", "equipment_type": "dumbbell",
            "load_unit": "lb",
            "tested_12rm": 35, "tested_10rm": 40, "tested_3rm": 55,
            "form_quality": "clean",
        })

    def test_estimate_1rm_epley(self):
        """Epley formula sanity · weight × (1 + reps/30)."""
        from strength_math import estimate_1rm
        self.assertAlmostEqual(estimate_1rm(225, 5), 262.5, places=1)
        self.assertAlmostEqual(estimate_1rm(100, 12), 140.0, places=1)
        self.assertAlmostEqual(estimate_1rm(35, 12), 49.0, places=1)

    def test_calculate_estimates_returns_per_test(self):
        """calculate_estimates_from_tests should return Epley estimates for each tested rep."""
        from strength_math import calculate_estimates_from_tests
        e = calculate_estimates_from_tests(self.test)
        self.assertIn(12, e.estimated_1rm_from_each_test)
        self.assertIn(10, e.estimated_1rm_from_each_test)
        self.assertIn(3, e.estimated_1rm_from_each_test)
        self.assertAlmostEqual(e.estimated_1rm_from_each_test[12], 49.0, places=1)
        self.assertAlmostEqual(e.estimated_1rm_from_each_test[10], 53.33, places=1)
        self.assertAlmostEqual(e.estimated_1rm_from_each_test[3], 60.5, places=1)

    def test_best_estimate_uses_lowest_rep(self):
        """Best estimate prefers the lowest-rep tested value (closest to 1RM)."""
        from strength_math import calculate_estimates_from_tests
        e = calculate_estimates_from_tests(self.test)
        # Lowest rep = 3, Epley estimate = 55 × (1 + 3/30) = 60.5
        self.assertAlmostEqual(e.best_estimate, 60.5, places=1)

    def test_conservative_training_max_uses_85_percent(self):
        """Training max = 0.85 × best estimate (clean form, no pain)."""
        from strength_math import calculate_estimates_from_tests
        e = calculate_estimates_from_tests(self.test)
        # 60.5 × 0.85 = 51.425
        self.assertAlmostEqual(e.conservative_training_max, 51.425, places=1)

    def test_working_weights_generated_for_all_4_target_reps(self):
        """get_working_weight_for_reps returns sane numbers for 12, 10, 8, 6."""
        from strength_math import get_working_weight_for_reps
        for reps in (12, 10, 8, 6):
            w = get_working_weight_for_reps(self.test, reps)
            self.assertIsNotNone(w, f"Working weight for {reps}RM was None")
            self.assertGreater(w, 0, f"Working weight for {reps}RM was non-positive")

    def test_dumbbell_rounding(self):
        """DB loads should round to 2.5 lb under 30 lb, 5 lb at/over 30 lb."""
        from strength_math import round_load
        # Under 30 lb · nearest 2.5
        self.assertEqual(round_load(27.4, "dumbbell", "per_hand"), 27.5)
        self.assertEqual(round_load(28.1, "dumbbell", "per_hand"), 27.5)
        self.assertEqual(round_load(28.8, "dumbbell", "per_hand"), 30)  # past 28.75 boundary
        # At/over 30 lb · nearest 5
        self.assertEqual(round_load(47.3, "dumbbell", "per_hand"), 45)
        self.assertEqual(round_load(52.7, "dumbbell", "per_hand"), 55)
        self.assertEqual(round_load(40.0, "dumbbell", "per_hand"), 40)

    def test_inconsistency_flag_when_3rm_much_higher(self):
        """3RM estimate (60.5) is ~24% above the median of (49, 53.3) · flag must fire."""
        from strength_math import detect_inconsistencies
        flags = detect_inconsistencies(self.test)
        self.assertTrue(
            len(flags) > 0,
            "Expected an inconsistency flag for 3RM=55 vs 12RM=35, 10RM=40"
        )
        # The flag should mention the 3RM specifically
        self.assertTrue(
            any("3RM" in f or "3" in f for f in flags),
            f"Inconsistency flag should reference 3RM · got: {flags}"
        )

    def test_no_inconsistency_when_data_is_clean(self):
        """Tests that scale linearly should NOT trigger an inconsistency flag."""
        from strength_testing import StrengthTest
        from strength_math import detect_inconsistencies
        # Real-world Epley-consistent client · 12RM=100, 10RM=110, 8RM=120, 5RM=140
        clean = StrengthTest.from_dict({
            "tested_12rm": 100, "tested_10rm": 110,
            "tested_8rm": 120, "tested_5rm": 140,
        })
        flags = detect_inconsistencies(clean)
        self.assertEqual(
            flags, [],
            f"Expected no inconsistency flags on clean data · got: {flags}"
        )


# ════════════════════════════════════════════════════════
# 3 · OPTIONAL TESTING DATA
# ════════════════════════════════════════════════════════

class OptionalTestingDataTests(unittest.TestCase):
    """No test data · PDF still generates · cells fall back to RPE / RIR."""

    @classmethod
    def setUpClass(cls):
        pdf_bytes, _ = build_pdf_silently(assessment_no_tests())
        cls.pdf_bytes = pdf_bytes
        cls.pdf_text = extract_pdf_text(pdf_bytes)

    def test_pdf_generated_without_test_data(self):
        self.assertGreater(len(self.pdf_bytes), 10_000,
                           "PDF should still generate without test data")

    def test_no_lb_weights_appear_for_no_test_client(self):
        """A client with no tests should NOT have specific lb numbers in the
        strength progression cells. The fallback is RPE/RIR language.
        (Some lb references CAN appear elsewhere · nutrition, etc., so we
        scope this to the strength-table phrases.)"""
        # Look for cell-style "@ <num> lb" load notation · should not appear
        import re
        loads_pattern = re.compile(r"@ \d+(?:\.\d+)? lb")
        # We scan only the per-page text for the strength session pages
        # (heuristic · pages between Mobility Map and Nutrition).
        matches = loads_pattern.findall(self.pdf_text)
        self.assertEqual(
            matches, [],
            f"No-test-data client should have no @ X lb cells · found {matches[:5]}"
        )

    def test_rir_or_rpe_language_present(self):
        """RIR or RPE labels should appear when there's no tested load."""
        has_rir = "RIR 2-3" in self.pdf_text or "RIR" in self.pdf_text
        has_rpe = "RPE" in self.pdf_text
        self.assertTrue(
            has_rir or has_rpe,
            "No-test-data client should show RIR or RPE labels"
        )


# ════════════════════════════════════════════════════════
# 4 · SIDE EFFECTS
# ════════════════════════════════════════════════════════

class SideEffectTests(unittest.TestCase):
    """Existing pieces of the system must continue to work."""

    def test_body_comp_calculations_still_work(self):
        """A payload with body_comp should produce a PDF with the BOD POD page populated."""
        pdf_bytes, _ = build_pdf_silently(assessment_with_body_comp())
        pdf_text = extract_pdf_text(pdf_bytes)
        # The body comp page lists the lean mass, fat mass, etc.
        self.assertIn("153", pdf_text, "Lean mass (153 lbs) should appear")
        # And nutrition AUTO should have been resolved to actual numbers
        self.assertNotIn("AUTO", pdf_text,
                         "AUTO placeholders should have been replaced by real values")

    def test_mobility_priority_generation_still_works(self):
        """Generator should still produce mobility prep blocks for FRA priorities."""
        pdf_bytes, _ = build_pdf_silently(assessment_with_tests())
        pdf_text = extract_pdf_text(pdf_bytes)
        # The mobility map page mentions the Hip IR priority
        self.assertIn("Hip IR", pdf_text,
                      "Hip IR priority should surface in the PDF")
        # And the per-session pages mention CARs / Mobility Prep
        self.assertIn("CARS", pdf_text.upper(),
                      "CARs sequence block should appear in session pages")
        self.assertIn("MOBILITY PREP", pdf_text.upper(),
                      "Mobility Prep block should appear")

    def test_pdf_generation_still_works_for_full_payload(self):
        """End-to-end · realistic payload produces a multi-page PDF."""
        pdf_bytes, name = build_pdf_silently(assessment_with_body_comp())
        self.assertEqual(name, "Test Client")
        self.assertGreater(len(pdf_bytes), 50_000,
                           "Full PDF should be substantial in size")
        try:
            from pypdf import PdfReader
            reader = PdfReader(io.BytesIO(pdf_bytes))
            self.assertGreaterEqual(
                len(reader.pages), 15,
                f"PDF should have at least 15 pages · got {len(reader.pages)}"
            )
        except ImportError:
            self.skipTest("pypdf not installed · skipping page count check")


# ════════════════════════════════════════════════════════
# 5 · PAGE REPLACEMENT
# ════════════════════════════════════════════════════════

class PageReplacementTests(unittest.TestCase):
    """Old tracking page language gone · new coaching-process page present."""

    @classmethod
    def setUpClass(cls):
        pdf_bytes, _ = build_pdf_silently(assessment_with_tests())
        cls.pdf_text = extract_pdf_text(pdf_bytes)

    def test_old_tracking_language_removed(self):
        """Old phrases that used to live on the tracking page must be gone."""
        forbidden = [
            "scale 1-10",                # old "Energy · scale 1-10"
            "quality + duration",        # old "Sleep · quality + duration"
            "Training Wins",             # old fourth bullet
            "anything that felt good",   # old subtext
            "WHAT YOU TRACK",            # old section header
            "Notice,",                   # old splash headline
            "adjust, grow.",             # old splash subhead
        ]
        for token in forbidden:
            self.assertNotIn(
                token, self.pdf_text,
                msg=f"Old tracking page language still present: {token!r}"
            )

    def test_new_coaching_process_page_present(self):
        """The new page should mention coaching-process language."""
        # Allow either spec phrasing OR the implementation phrasing
        acceptable_section_headers = [
            "HOW WE ADJUST YOUR PLAN",
            "How We Adjust Your Plan",
            "HOW WE ADJUST",
        ]
        found = any(h in self.pdf_text for h in acceptable_section_headers)
        self.assertTrue(
            found,
            f"None of the acceptable headers found in PDF · expected one of {acceptable_section_headers}"
        )

    def test_new_page_has_adjustment_signals(self):
        """The new page should reference what we look at when adjusting plans ·
        test results, recovery, schedule, observation."""
        signals = ["Test results", "Client feedback", "What life looks like",
                   "Coach observation"]
        for s in signals:
            self.assertIn(
                s, self.pdf_text,
                msg=f"Expected signal {s!r} on the new coaching-process page"
            )


# ════════════════════════════════════════════════════════
# 6 · PDF TEXT SAFETY
# ════════════════════════════════════════════════════════

class TextSafetyTests(unittest.TestCase):
    """No visible ellipses · names wrap or shorten cleanly."""

    @classmethod
    def setUpClass(cls):
        # Use a payload with deliberately long exercise names to stress-test wrapping
        payload = assessment_with_tests()
        payload["strength_marker_tests"] = [
            {
                "exercise_name": "Front Foot Elevated Single Leg RDL with Dumbbell",
                "load_style": "per_hand", "equipment_type": "dumbbell",
                "tested_8rm": 30, "form_quality": "clean",
            },
            {
                "exercise_name": "Bench Supported Single Arm Dumbbell Row",
                "load_style": "per_hand", "equipment_type": "dumbbell",
                "tested_10rm": 45, "form_quality": "clean",
            },
        ]
        pdf_bytes, _ = build_pdf_silently(payload)
        cls.pdf_text = extract_pdf_text(pdf_bytes)

    def test_no_ellipsis_in_pdf_text(self):
        """The unicode ellipsis character … should never appear in client PDFs."""
        # The horizontal ellipsis · U+2026
        ELLIPSIS = "\u2026"
        self.assertNotIn(
            ELLIPSIS, self.pdf_text,
            "Ellipsis character … found in PDF · names should wrap, not truncate"
        )

    def test_no_three_dot_truncation(self):
        """Three-dot truncation '...' should not appear in exercise contexts."""
        # Three actual dots · used to be appended by the old shorten() function
        # This is a softer check · '...' might legitimately appear in prose,
        # so we only flag it inside known table contexts.
        # We scan for patterns like "Word..." or "Word ..." in close proximity
        # to typical strength-table tokens.
        if "..." in self.pdf_text:
            # Only fail if "..." appears NEAR a sets×reps pattern (i.e., in a table cell)
            import re
            # Find any "..." that's within 80 chars of "× <num>" (a strength cell)
            danger = re.search(r"\.{3}.{0,80}× \d+|× \d+.{0,80}\.{3}", self.pdf_text)
            self.assertIsNone(
                danger,
                f"Three-dot truncation found near strength table content: {danger.group() if danger else ''}"
            )

    def test_long_exercise_names_present_in_pdf(self):
        """The PDF should contain the long exercise names that the generator
        actually picks (e.g. 'Dumbbell Bench Press (Single or Double Arm)'
        from the library) without truncating them with ellipses."""
        # The strength A picker selects from the library · names like
        # 'Dumbbell Bench Press (Single or Double Arm)' or 'Trap Bar Deadlift'
        # are guaranteed to appear when there's UB strength work.
        # We check that AT LEAST ONE multi-word library exercise name is
        # rendered intact (not truncated).
        common_long_names = [
            "Trap Bar Deadlift",
            "Goblet Squat",
            "Dumbbell Bench Press",
            "Chest Supported Row",
            "Cable Hip IR",
            "Single-Leg Hip Bridge",
        ]
        found_count = sum(1 for n in common_long_names if n in self.pdf_text)
        self.assertGreater(
            found_count, 0,
            f"None of the standard library exercise names found in PDF · "
            f"text was {len(self.pdf_text)} chars long"
        )

    def test_shorten_function_never_adds_ellipses(self):
        """The shorten() helper must abbreviate via the replacement table,
        never append an ellipsis character."""
        from plan_pdf import shorten
        long_names = [
            "Front Foot Elevated Single-Leg RDL with Dumbbell",
            "Bench Supported Single-Arm Dumbbell Row",
            "External Rotation Cable Row",
            "Single Leg Romanian Deadlift",
        ]
        for n in long_names:
            result = shorten(n)
            self.assertNotIn(
                "\u2026", result,
                f"shorten({n!r}) returned an ellipsis: {result!r}"
            )
            self.assertFalse(
                result.endswith("..."),
                f"shorten({n!r}) returned three dots: {result!r}"
            )


# ════════════════════════════════════════════════════════
# 7 · CLIENT CONCERNS · joint-friendly routing + Goals page
# ════════════════════════════════════════════════════════

class ClientConcernsTests(unittest.TestCase):
    """Bad knee / bad shoulder concerns route to friendly pools and surface
    in the Goals page narrative."""

    def _bad_knee_assessment(self):
        from generator import Assessment, parse_fra_priority, MobilityRating
        return Assessment(
            name="Knee Test", age_range="50s", sex="F", background="hiker",
            training_frequency=2, strength_days=2, cardio_days=0,
            primary_goal="Hike pain-free",
            fra_priorities=[parse_fra_priority("Hip IR L+R")],
            strength_markers=[], constraints=[],
            mobility_map=[MobilityRating(joint="hip", direction="IR",
                                          side="L", rating="yellow")],
            body_comp={}, progression_mode="autoregulated",
            strength_marker_results={}, strength_marker_tests=[],
            concerns=["bad_knee"],
            concern_notes="Right knee · meniscus repair 2019",
        )

    def test_bad_knee_avoids_goblet_squat(self):
        """A client with bad_knee concern must not get Goblet Squat
        as Strength A · the picker should route to a knee-friendly pool."""
        from generator import Generator
        a = self._bad_knee_assessment()
        with contextlib.redirect_stdout(io.StringIO()):
            g = Generator(libraries_path=str(REPO_ROOT / "libraries"))
            program = g.build_program(a)

        # Find the LB session and its Strength A
        for session in program.weeks[0].sessions:
            if session.day_type == "strength_lb":
                sa = next((b for b in session.blocks if b.name == "Strength A"), None)
                self.assertIsNotNone(sa, "Strength A block should exist")
                names = [ex.name for ex in sa.exercises]
                self.assertNotIn(
                    "Goblet Squat", names,
                    "Bad-knee client must not get Goblet Squat in Strength A"
                )
                self.assertNotIn(
                    "Barbell Front Squat", names,
                    "Bad-knee client must not get Barbell Front Squat"
                )
                self.assertNotIn(
                    "Barbell Back Squat", names,
                    "Bad-knee client must not get Barbell Back Squat"
                )
                return  # Done with this test
        self.fail("No strength_lb session found")

    def test_bad_knee_picks_hip_emphasis_alternative(self):
        """Bad-knee client should still get a productive lower-body lift ·
        from the squat_knee_friendly pool (Bridges, Hip Thrusts, KB DL etc)."""
        from generator import Generator
        a = self._bad_knee_assessment()
        with contextlib.redirect_stdout(io.StringIO()):
            g = Generator(libraries_path=str(REPO_ROOT / "libraries"))
            program = g.build_program(a)

        knee_friendly_options = {
            "Bridge (Single & Double Leg)", "Hip Thrust",
            "Kettlebell Deadlift", "Step Up",
            "Bench-Supported Single Leg RDL",
            "Dumbbell RDL (Straight Leg Deadlift)",
        }
        for session in program.weeks[0].sessions:
            if session.day_type == "strength_lb":
                sa = next((b for b in session.blocks if b.name == "Strength A"), None)
                names = {ex.name for ex in sa.exercises}
                self.assertTrue(
                    any(n in knee_friendly_options for n in names),
                    f"Bad-knee client should get a knee-friendly lift, got: {names}"
                )
                return
        self.fail("No strength_lb session found")

    def test_bad_shoulder_avoids_barbell_bench(self):
        """A bad-shoulder client must not get Barbell Bench Press as the UB picks."""
        from generator import Generator, Assessment, parse_fra_priority, MobilityRating
        a = Assessment(
            name="Shoulder Test", age_range="50s", sex="M", background="t",
            training_frequency=3, strength_days=3, cardio_days=0,
            primary_goal="Strength without aggravating shoulder",
            fra_priorities=[parse_fra_priority("Hip IR L+R")],
            strength_markers=[], constraints=[],
            mobility_map=[MobilityRating(joint="hip", direction="IR",
                                          side="L", rating="yellow")],
            body_comp={}, progression_mode="autoregulated",
            strength_marker_results={}, strength_marker_tests=[],
            concerns=["bad_shoulder"],
            concern_notes="",
        )
        with contextlib.redirect_stdout(io.StringIO()):
            g = Generator(libraries_path=str(REPO_ROOT / "libraries"))
            program = g.build_program(a)

        for session in program.weeks[0].sessions:
            if session.day_type == "strength_ub":
                sa = next((b for b in session.blocks if b.name == "Strength A"), None)
                names = [ex.name for ex in sa.exercises]
                self.assertNotIn(
                    "Barbell Bench Press", names,
                    "Bad-shoulder client must not get Barbell Bench Press"
                )
                return
        self.fail("No strength_ub session found")

    def test_no_concerns_does_not_regress(self):
        """A client without concerns should still get the default picks ·
        Goblet Squat is fine, the system isn't accidentally over-filtering."""
        from generator import Generator, Assessment, parse_fra_priority, MobilityRating
        a = Assessment(
            name="Clean", age_range="40s", sex="M", background="t",
            training_frequency=3, strength_days=3, cardio_days=0,
            primary_goal="t",
            fra_priorities=[parse_fra_priority("Hip IR L+R")],
            strength_markers=[], constraints=[],
            mobility_map=[MobilityRating(joint="hip", direction="IR",
                                          side="L", rating="yellow")],
            body_comp={}, progression_mode="autoregulated",
            strength_marker_results={}, strength_marker_tests=[],
            concerns=[], concern_notes="",
        )
        with contextlib.redirect_stdout(io.StringIO()):
            g = Generator(libraries_path=str(REPO_ROOT / "libraries"))
            program = g.build_program(a)

        # No concerns · Strength A LB Day 1 should still be Goblet Squat
        # (the historical default for unconstrained clients)
        for session in program.weeks[0].sessions:
            if session.day_type == "strength_lb" and session.day_number == 1:
                sa = next((b for b in session.blocks if b.name == "Strength A"), None)
                names = [ex.name for ex in sa.exercises]
                self.assertIn(
                    "Goblet Squat", names,
                    "No-concerns client should still get Goblet Squat (default)"
                )
                return
        self.fail("No LB Day 1 found")

    def test_goals_page_mirrors_concerns(self):
        """The Goals page should echo what the coach told us about concerns."""
        a = self._bad_knee_assessment()
        # Build the assessment dict the way plan_pdf reads it
        from dataclasses import asdict
        adict = asdict(a)
        adict['fra_priorities'] = [{'description': p.description,
                                     'joints': p.joints, 'directions': p.directions}
                                    for p in a.fra_priorities]
        adict['mobility_map'] = [{'joint': m.joint, 'direction': m.direction,
                                   'side': m.side, 'rating': m.rating}
                                  for m in a.mobility_map]

        from plan_pdf import compose_working_with, compose_approach
        working_with = compose_working_with(adict)
        approach = compose_approach(adict)

        # The "what you're working with" section must mention the knee
        self.assertIn("knee", working_with.lower(),
                      "Goals page must mirror the bad-knee concern")
        # And the concern_notes text should be there too
        self.assertIn("meniscus", working_with.lower(),
                      "Goals page must mirror concern_notes verbatim or close")
        # The "approach" section should also reference the joint
        self.assertIn("knee", approach.lower(),
                      "Approach section should explain how we handle the knee")

    def test_goals_page_handles_no_concerns(self):
        """When no concerns are flagged, the Goals page shouldn't fabricate them."""
        from plan_pdf import compose_working_with
        adict = {
            "concerns": [], "concern_notes": "",
            "fra_priorities": [], "mobility_map": [],
            "constraints": [],
        }
        result = compose_working_with(adict)
        # Should produce a non-empty fallback that doesn't claim concerns exist
        self.assertTrue(len(result) > 0)
        self.assertNotIn("knee", result.lower())
        self.assertNotIn("shoulder", result.lower())


# ════════════════════════════════════════════════════════
# 7 · POLISH ITEMS · CALENDAR / NUTRITION / KNEE / COPY
# ════════════════════════════════════════════════════════

class FrequencyCalendarTests(unittest.TestCase):
    """5x/week program shows 5 sessions in weekly structure and weekly routine."""

    @classmethod
    def setUpClass(cls):
        form = {
            "client_name": "Five Times", "age_range": "40s", "sex": "M", "background": "t",
            "training_frequency": 5, "strength_days": 4, "cardio_days": 1,
            "primary_goal": "test", "fra_priorities": ["Hip IR L+R"],
            "mobility_map": [{"joint":"hip","direction":"IR","side":"L","rating":"yellow"}],
            "strength_markers": [], "strength_marker_results": {}, "strength_marker_tests": [],
            "constraints": [], "body_comp": {},
            "nutrition_strategy": "maintenance", "activity_factor": 1.4,
        }
        pdf, _ = build_pdf_silently(form)
        cls.pdf_text = extract_pdf_text(pdf)

    def test_5x_weekly_structure_has_5_sessions(self):
        """Weekly structure (Day-by-Day page) must show all 5 sessions."""
        # Look at the weekly structure section (DAY BY DAY)
        # In a 5x program, only 2 days should be 'Rest · active recovery'
        rest_count = self.pdf_text.count("Rest · active recovery")
        # Each rest mention also appears on Weekly Routine page · so total should be 4
        # (2 rest days × 2 pages where they appear)
        self.assertLessEqual(
            rest_count, 4,
            f"5x/week program shows too many Rest days · counted {rest_count}, "
            f"expected ≤4 (2 rests on each of 2 calendar pages)"
        )

    def test_5x_saturday_not_always_rest(self):
        """Saturday should be either a session or a rest depending on spread.
        Specifically · for 5 sessions with a known spread, SAT should NOT be 'Rest'."""
        # Find the SAT line in the weekly structure
        # The weekly structure shows · SAT followed by content
        # If SAT is rest, it'll be 'SAT\nRest · active recovery'
        # If SAT has a session, it'll be 'SAT\n<some focus>'
        # Use a regex that allows for any SAT content
        import re
        # Find every 'SAT\n' followed by next 60 chars to see what it says
        sat_blocks = re.findall(r'SAT\n([^\n]{0,80})', self.pdf_text)
        # At least ONE SAT entry should have content other than "Rest"
        # (the weekly structure page or the weekly routine page must show a session)
        non_rest_saturdays = [s for s in sat_blocks if "Rest" not in s and s.strip()]
        self.assertTrue(
            len(non_rest_saturdays) > 0,
            f"5x/week program · Saturday should have a session, not always rest. "
            f"Found SAT entries: {sat_blocks}"
        )


class NutritionCalorieTests(unittest.TestCase):
    """Sample day calorie total within ±100 of target."""

    def test_scaling_brings_total_within_tolerance(self):
        """scale_meals_to_target should bring 2640-cal sample down to 1959 ±100."""
        from plan_pdf import scale_meals_to_target, build_sample_meals
        targets = {'calories': '1959'}
        meals = build_sample_meals(targets, is_runner=False, is_strength=True)
        scaled = scale_meals_to_target(meals, targets)
        total = sum(c for _, _, c in scaled)
        self.assertGreaterEqual(total, 1859, f"Scaled total {total} is below tolerance")
        self.assertLessEqual(total, 2059, f"Scaled total {total} is above tolerance")

    def test_scaling_handles_low_target(self):
        """A 1500-cal target should produce a sample day in the 1400-1600 range."""
        from plan_pdf import scale_meals_to_target, build_sample_meals
        targets = {'calories': '1500'}
        meals = build_sample_meals(targets, is_runner=False, is_strength=False)
        scaled = scale_meals_to_target(meals, targets)
        total = sum(c for _, _, c in scaled)
        self.assertGreaterEqual(total, 1400, f"Scaled total {total} below tolerance")
        self.assertLessEqual(total, 1600, f"Scaled total {total} above tolerance")

    def test_scaling_handles_high_target(self):
        """A 3200-cal target should produce a sample day in the 3100-3300 range."""
        from plan_pdf import scale_meals_to_target, build_sample_meals
        targets = {'calories': '3200'}
        meals = build_sample_meals(targets, is_runner=True, is_strength=False)
        scaled = scale_meals_to_target(meals, targets)
        total = sum(c for _, _, c in scaled)
        self.assertGreaterEqual(total, 3100, f"Scaled total {total} below tolerance")
        self.assertLessEqual(total, 3300, f"Scaled total {total} above tolerance")

    def test_scaling_no_target_returns_unchanged(self):
        """No calorie target → meals returned as-is, no scaling."""
        from plan_pdf import scale_meals_to_target, build_sample_meals
        meals = build_sample_meals({}, is_runner=False, is_strength=True)
        scaled = scale_meals_to_target(meals, {})
        self.assertEqual([c for _, _, c in meals], [c for _, _, c in scaled])


class KneeFilteringTests(unittest.TestCase):
    """Knee-flagged clients don't get knee-aggressive HIIT options."""

    @classmethod
    def setUpClass(cls):
        form = {
            "client_name": "Bad Knee", "age_range": "early 50s", "sex": "F",
            "background": "old meniscus", "training_frequency": 3,
            "strength_days": 2, "cardio_days": 1, "primary_goal": "test",
            "fra_priorities": ["Hip IR L+R"],
            "mobility_map": [{"joint":"hip","direction":"IR","side":"L","rating":"yellow"}],
            "strength_markers": [], "strength_marker_results": {}, "strength_marker_tests": [],
            "constraints": [], "body_comp": {},
            "concerns": ["bad_knee"],
            "concern_notes": "Right knee · meniscus repair · sensitive to deep flexion.",
            "nutrition_strategy": "maintenance", "activity_factor": 1.4,
        }
        pdf, _ = build_pdf_silently(form)
        cls.pdf_text = extract_pdf_text(pdf)

    def test_no_lateral_bounds_for_knee_client(self):
        self.assertNotIn("Lateral Bound", self.pdf_text,
                         "Bad knee client should not see Lateral Bounds")

    def test_no_alternating_reverse_lunges(self):
        self.assertNotIn("Alternating Reverse Lunge", self.pdf_text,
                         "Bad knee client should not see Alternating Reverse Lunges")

    def test_no_jump_rope(self):
        self.assertNotIn("Jump Rope", self.pdf_text,
                         "Bad knee client should not see Jump Rope")

    def test_no_kettlebell_swings(self):
        self.assertNotIn("Kettlebell Swing", self.pdf_text,
                         "Bad knee client should not see Kettlebell Swings (deep knee flex + hinge)")

    def test_no_box_jumps_or_burpees(self):
        self.assertNotIn("Box Jump", self.pdf_text,
                         "Bad knee client should not see Box Jumps")
        self.assertNotIn("Burpee", self.pdf_text,
                         "Bad knee client should not see Burpees")

    def test_no_goblet_squat_for_knee_client(self):
        self.assertNotIn("Goblet Squat", self.pdf_text,
                         "Bad knee client should not see Goblet Squat as Strength A")

    def test_no_trap_bar_for_lower_back_client(self):
        """Separate scenario · lower-back concern should drop Trap Bar."""
        form = {
            "client_name": "Bad Back", "age_range": "40s", "sex": "M", "background": "t",
            "training_frequency": 3, "strength_days": 3, "cardio_days": 0,
            "primary_goal": "test", "fra_priorities": ["Hip IR L+R"],
            "mobility_map": [{"joint":"hip","direction":"IR","side":"L","rating":"yellow"}],
            "strength_markers": [], "strength_marker_results": {}, "strength_marker_tests": [],
            "constraints": [], "body_comp": {}, "concerns": ["lower_back"],
            "nutrition_strategy": "maintenance", "activity_factor": 1.4,
        }
        pdf, _ = build_pdf_silently(form)
        text = extract_pdf_text(pdf)
        self.assertNotIn("Trap Bar Deadlift", text,
                         "Lower back concern should drop Trap Bar Deadlift")


class CopySanitizerTests(unittest.TestCase):
    """The copy sanitizer fixes common spelling and grammar issues."""

    def test_meniscus_spelling_fix(self):
        from plan_pdf import sanitize_copy
        self.assertEqual(sanitize_copy("Mensicus repair"), "Meniscus repair")
        self.assertEqual(sanitize_copy("post-mensicus surgery"), "post-meniscus surgery")

    def test_cars_capitalization(self):
        from plan_pdf import sanitize_copy
        self.assertEqual(sanitize_copy("Level 2,3 cars"), "Level 2-3 CARs")
        self.assertEqual(sanitize_copy("daily cars routine"), "daily CARs routine")
        self.assertEqual(sanitize_copy("pails and rails work"), "PAILs and RAILs work")

    def test_article_agreement(self):
        from plan_pdf import sanitize_copy
        self.assertEqual(sanitize_copy("a early 30s body"), "an early-30s body")
        self.assertEqual(sanitize_copy("a old client"), "an old client")
        self.assertEqual(sanitize_copy("A early adopter"), "An early adopter")

    def test_compound_modifier_hyphen(self):
        from plan_pdf import sanitize_copy
        self.assertEqual(sanitize_copy("an early 30s body"),
                         "an early-30s body")
        self.assertEqual(sanitize_copy("a late 50s frame"),
                         "a late-50s frame")  # 'a' before consonant 'l' is correct

    def test_double_space_collapse(self):
        from plan_pdf import sanitize_copy
        self.assertEqual(sanitize_copy("hello  world   here"),
                         "hello world here")

    def test_other_medical_spellings(self):
        from plan_pdf import sanitize_copy
        self.assertEqual(sanitize_copy("rotater cuff"), "rotator cuff")
        self.assertEqual(sanitize_copy("siatic nerve"), "sciatic nerve")
        self.assertEqual(sanitize_copy("pirformis"), "Piriformis")

    def test_does_not_break_clean_text(self):
        """Clean input should pass through unchanged."""
        from plan_pdf import sanitize_copy
        clean = "Build strength without making your knee worse."
        self.assertEqual(sanitize_copy(clean), clean)

    def test_handles_empty_or_none(self):
        from plan_pdf import sanitize_copy
        self.assertEqual(sanitize_copy(""), "")
        self.assertEqual(sanitize_copy(None), None)


class CoachingProcessWordingTests(unittest.TestCase):
    """The coaching process page no longer claims to track sleep/energy."""

    @classmethod
    def setUpClass(cls):
        pdf, _ = build_pdf_silently(assessment_with_tests())
        cls.pdf_text = extract_pdf_text(pdf)

    def test_no_pdf_tracking_claims(self):
        """The page must NOT claim to track Energy/Sleep/Mood."""
        # Old wording was "How you felt: Energy, sleep, mood..."
        self.assertNotIn("How you felt", self.pdf_text,
                         "Old 'How you felt' label should be replaced")

    def test_uses_client_feedback_phrasing(self):
        """The new label · Client feedback."""
        self.assertIn("Client feedback", self.pdf_text,
                      "Coaching process page should use 'Client feedback' phrasing")

    def test_mentions_soreness_recovery_schedule(self):
        """The body of the new line mentions soreness, recovery, schedule."""
        page = self.pdf_text
        self.assertTrue(
            "soreness" in page.lower() or "recovery" in page.lower()
            or "schedule" in page.lower(),
            "New coaching-process line should mention soreness/recovery/schedule"
        )


# ────────────────────────────────────────────────────────
# Runner
# ────────────────────────────────────────────────────────

class TestStrengthC(unittest.TestCase):
    """Strength C accessory block · day-type routing + coach picks."""

    @classmethod
    def setUpClass(cls):
        sys.path.insert(0, os.path.join(os.path.dirname(__file__), '..'))
        sys.path.insert(0, os.path.join(os.path.dirname(__file__), '..', 'generator'))

    def _make_assessment(self, accessory_categories=None, concerns=None):
        from generator import Assessment, parse_fra_priority, MobilityRating
        from strength_testing import parse_strength_tests
        return Assessment(
            name="T", age_range="40s", sex="F", background="t",
            training_frequency=3, strength_days=3, cardio_days=0, primary_goal="t",
            fra_priorities=[parse_fra_priority("Hip Ir L+R")],
            strength_markers=[], constraints=[],
            mobility_map=[MobilityRating(joint="hip", direction="IR", side="L", rating="yellow")],
            body_comp={}, progression_mode="autoregulated",
            strength_marker_results={}, strength_marker_tests=parse_strength_tests([]),
            concerns=concerns or [], constraints_rich=[],
            accessory_categories=accessory_categories or [])

    def _strength_c_per_session(self, assessment):
        from generator import Generator
        import io as _io, contextlib as _ctx
        with _ctx.redirect_stdout(_io.StringIO()):
            g = Generator(libraries_path="libraries")
            program = g.build_program(assessment, block_number=1)
        out = []
        for sess in program.weeks[0].sessions:
            if not sess.day_type.startswith("strength"): continue
            sc = next((b for b in sess.blocks if b.name == "Strength C"), None)
            out.append((sess.day_type, [ex.name for ex in (sc.exercises if sc else [])]))
        return out

    def test_strength_c_renders_with_coach_picks(self):
        """Coach picks arms + calves + carries → Strength C shows up."""
        a = self._make_assessment(accessory_categories=["arms", "calves_tibialis", "loaded_carries"])
        sessions = self._strength_c_per_session(a)
        # Every strength session should have a Strength C block
        self.assertGreaterEqual(len(sessions), 2)
        for day_type, names in sessions:
            self.assertGreater(len(names), 0,
                                f"{day_type} should have Strength C exercises")

    def test_strength_c_routes_by_day_type(self):
        """LB days get lower-body categories · UB days get upper-body."""
        a = self._make_assessment(accessory_categories=[
            "arms", "calves_tibialis", "neck_traps", "glutes_extra", "loaded_carries"])
        sessions = self._strength_c_per_session(a)
        for day_type, names in sessions:
            joined = " ".join(names).lower()
            if day_type == "strength_lb":
                # Should include calves/glutes/carries-style picks; not arms/neck
                self.assertNotIn("hammer curl", joined)
                self.assertNotIn("bicep curl", joined)
                self.assertNotIn("cervical resist", joined)
            elif day_type == "strength_ub":
                # Should include arms/neck/shoulders; not calves/glutes
                self.assertNotIn("calf raise", joined)
                self.assertNotIn("hip thrust", joined)

    def test_strength_c_variety_across_sessions(self):
        """Same coach picks, different sessions in same week → different picks."""
        a = self._make_assessment(accessory_categories=[
            "arms", "calves_tibialis", "glutes_extra", "loaded_carries"])
        sessions = self._strength_c_per_session(a)
        # Compare LB Day 1 vs LB Day 3 specifically · both use same category pool
        lb_sessions = [(dt, names) for dt, names in sessions if dt == "strength_lb"]
        if len(lb_sessions) >= 2:
            self.assertNotEqual(lb_sessions[0][1], lb_sessions[1][1],
                                  "LB Day 1 and LB Day 3 should differ in Strength C")

    def test_strength_c_filters_concerns(self):
        """Knee-sensitive client should not get knee-avoid accessories."""
        a = self._make_assessment(
            accessory_categories=["calves_tibialis", "glutes_extra", "conditioning_accessories"],
            concerns=["bad_knee", "post_surgery_knee"])
        sessions = self._strength_c_per_session(a)
        for day_type, names in sessions:
            for n in names:
                # No knee-avoid items in our library, but check that filter runs
                self.assertNotIn("Cossack Squat", n,
                                  f"Knee-avoid pick leaked through: {n}")


class TestStrengthBVariety(unittest.TestCase):
    """Big-fix verification · core + corrective slots vary across sessions
    AND filter by client concerns + level."""

    @classmethod
    def setUpClass(cls):
        sys.path.insert(0, os.path.join(os.path.dirname(__file__), '..'))
        sys.path.insert(0, os.path.join(os.path.dirname(__file__), '..', 'generator'))
        from generator import Generator, Assessment, parse_fra_priority, MobilityRating
        from strength_testing import parse_strength_tests

        a = Assessment(name="Amanda", age_range="50s", sex="F", background="t",
            training_frequency=3, strength_days=3, cardio_days=0, primary_goal="t",
            fra_priorities=[parse_fra_priority("Hip IR L+R")],
            strength_markers=[], constraints=["post_surgery_knee"],
            mobility_map=[MobilityRating(joint="hip", direction="IR", side="L", rating="yellow")],
            body_comp={}, progression_mode="autoregulated",
            strength_marker_results={}, strength_marker_tests=parse_strength_tests([]),
            concerns=["bad_knee"], constraints_rich=[])
        import io as _io, contextlib as _ctx
        with _ctx.redirect_stdout(_io.StringIO()):
            g = Generator(libraries_path="libraries")
            cls.program = g.build_program(a, block_number=1)

    def _strength_b_exercises_per_session(self, week_idx=0):
        """Return [[ex_name, ...], ...] for each strength session in the week."""
        out = []
        for sess in self.program.weeks[week_idx].sessions:
            if not sess.day_type.startswith("strength"):
                continue
            sb = next((b for b in sess.blocks if b.name.startswith("Strength B")), None)
            if not sb:
                continue
            out.append([ex.name for ex in sb.exercises])
        return out

    def test_core_accessory_varies_across_sessions(self):
        """Strength B's core slot should NOT pick the same exercise across
        all three strength sessions in Week 1."""
        sessions = self._strength_b_exercises_per_session()
        self.assertGreaterEqual(len(sessions), 2, "Need at least 2 strength sessions to test variety")
        cores = [s[0] for s in sessions if s]  # first slot is core accessory
        unique = set(cores)
        self.assertGreater(len(unique), 1,
                            f"Core accessory should vary across sessions, got: {cores}")

    def test_corrective_varies_across_sessions(self):
        """The corrective slot (last in Strength B) should NOT pick the same
        variant across all three sessions in Week 1."""
        sessions = self._strength_b_exercises_per_session()
        self.assertGreaterEqual(len(sessions), 2)
        # Corrective is the LAST exercise in Strength B (after core + complement)
        correctives = [s[-1] for s in sessions if len(s) >= 3]
        unique = set(correctives)
        self.assertGreater(len(unique), 1,
                            f"Corrective should vary across sessions, got: {correctives}")

    def test_corrective_respects_knee_concerns(self):
        """For Amanda (post_surgery_knee + bad_knee), no corrective in any
        session should be one of the knee-AVOID variants (none in current
        library, but the filter should at minimum prefer safe variants over
        caution ones in early-week picks)."""
        sessions = self._strength_b_exercises_per_session()
        all_correctives = [s[-1] for s in sessions if len(s) >= 3]
        # Cossack Squat is tagged knee=avoid in the library · should NEVER
        # be picked for a post-surgery knee client
        for c in all_correctives:
            self.assertNotIn("Cossack Squat", c,
                               f"Knee-avoid corrective '{c}' should not be picked for post-surgery knee client")


if __name__ == "__main__":
    # Direct run · `python3 test_strength_system.py`
    unittest.main(verbosity=2)
