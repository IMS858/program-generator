...
The lambda needs four data folders at runtime, not just the Python ·

| Folder | Why |
|---|---|
| `libraries/` | the exercise databases |
| `config/` | `objective_thresholds.json` · every cutoff the objective layer reads |
| `assets/` | logo variants used by the PDF renderer |
| `web/` | the assessment form served at `/` |

`vercel.json` lists these under `includeFiles`. **If you add a new data folder,
add it there too** · the Python builder does not bundle files it cannot see
being imported.

A missing `config/` degrades rather than crashes: every client falls back to
the legacy `_default` ladder and the objective layer goes quiet. That is a safe
failure but a silent one, so check `GET /api/version` after a deploy and
confirm a coach-mode PDF still renders the objective page for a client who has
instrumented data.
