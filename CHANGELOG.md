...
## v2.2.0 — VALD vocabulary + provenance

- **`generator/vald_mapping.py`** maps VALD DynaMo `(bodyRegion, movement)`
  pairs onto canonical test ids, splits bilateral tests into per-side readings,
  and carries RFD through. Built against VALD's documented payloads, which are
  used verbatim as test fixtures. The ingest itself belongs in Coach OS; the
  vocabulary belongs here, because a mapping table in two places drifts.
- **`POST /api/vald/transform`** — raw VALD tests in, validated
  `objective_measures` out. Unmapped tests are skipped and reported rather than
  failing the batch: `/api/generate` 400s on an unknown test id, which is right
  for a hand-typed payload and wrong for a sync.
- **Provenance.** Every measurement carries `source` (`manual` | `vald_api`)
  and `source_id`. The coach page names the sources and tags individual rows
  when an assessment mixes both.
- **Reading the real schema corrected two assumptions.** ROM comes back in
  degrees at the test level (quaternions are only in the raw trace), so ROM
  auto-imports rather than staying manual. And rate of force development is
  precomputed on every strength test — it does not need trace parsing, so it is
  available from day one rather than being a later phase.
- **Fixed: the validator rejected real data.** ROM was capped at −30°, but an
  extension deficit reads negative and VALD's own documented example is −32°.
  Widened to −50°.
- Test vocabulary extended to 30 ids (wrist, cervical, trunk, ankle
  inversion/eversion, IMTP, hip/shoulder flexion-extension).
- **`docs/vald-ingest-spec.md`** — paste-ready for the app thread, including
  the API License Agreement step that gates everything and should go into the
  DynaMo negotiation now.
