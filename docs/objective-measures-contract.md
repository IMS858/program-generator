...
| `source` | no | `manual` (default) or `vald_api`. Printed on the coach page; rows are tagged when an assessment mixes both. |
| `source_id` | no | The upstream record id (VALD `testId`), for traceability. |
| `metrics` | no | Extra numbers that came with the reading — e.g. `{"rfd_n_per_s": 341}`. Displayed, not yet acted on. |
...
| `degrees` | yes | −50 to 200. Negative is legitimate — an extension deficit reads negative. |
...
`POST /api/vald/transform` converts raw VALD DynaMo test objects into a
validated `objective_measures` block, so the ingest never has to reimplement
the test vocabulary. Unmapped tests are skipped and reported in `unmapped`
rather than failing the batch. See
[`docs/vald-ingest-spec.md`](vald-ingest-spec.md).
