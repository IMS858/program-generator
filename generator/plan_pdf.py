...
def draw_session_page(c, program, session_idx, day_num_in_week,
    ...
    page_header_bar(c, f"SECTION 07 · SESSION {session_idx + 1} DETAIL",
                    f"WEEK 01-06 · DAY {day_num_in_week}")
...
def render_strength_progression(c, program, session_idx, block_name, y):
    """Render a strength block as a 6-week progression · W1-3 top, W4-6 bottom.

    Rows · exercises
    Top table · WK 1 (Establish) · WK 2 (Tempo) · WK 3 (Load)
    Bottom table · WK 4 (Volume) · WK 5 (Heavy) · WK 6 (Deload)
    """
    weeks = program['weeks']
    if len(weeks) < 6:
        # Fallback · legacy 4-week plans still render compact
    ...
    # Need 6 weeks to render the split layout
    if len(blocks_per_week) < 6:
    ...
    # Layout · 30% name column + 70% split across 3 week columns per half
    name_col_w = CONTENT_W * 0.30
    wk_col_w = (CONTENT_W - name_col_w) / 3
    ...
    wk_labels_top = ["WK 1 · ESTABLISH", "WK 2 · TEMPO", "WK 3 · LOAD"]
    for i, lbl in enumerate(wk_labels_top):
    ...
    for ex_idx, ex in enumerate(wk1_exercises):
        name = ex.get('name', '')
        orig_name = name
        while c.stringWidth(name, SERIF, 10) > name_col_w - 14 and len(name) > 8:
            name = name[:-1]
        if name != orig_name:
            name = name + "…"
        ...
        for wk_idx in range(3):  # Weeks 1-3
            ...
            if ex_idx < len(wk_block['exercises']):
                ...
                # Color code · WK1 dim, WK2-3 sky (build)
                if wk_idx == 0:
                    c.setFillColor(CREAM_DIM)
                else:
                    c.setFillColor(SKY_LIGHT)
    ...
    c.setFont(SANS_MEDIUM, 7.5)
    c.drawString(MARGIN + 12, y, "EXERCISE")
    wk_labels_bot = ["WK 4 · VOLUME", "WK 5 · HEAVY", "WK 6 · DELOAD"]
    for i, lbl in enumerate(wk_labels_bot):
        x = MARGIN + name_col_w + i * wk_col_w + wk_col_w / 2
        c.drawCentredString(x, y, lbl)
    y -= 10
    thin_divider(c, MARGIN, y, PAGE_W - MARGIN, color=DIVIDER, width=0.4)
    ...
    for ex_idx, ex in enumerate(wk1_exercises):
        ...
        for rel_idx in range(3):  # Weeks 4-6
            wk_idx = rel_idx + 3
            wk_block = blocks_per_week[wk_idx]
            if ex_idx < len(wk_block['exercises']):
                wk_ex = wk_block['exercises'][ex_idx]
                ...
                # Color · WK4-5 sky (peak), WK6 dim (deload)
                if wk_idx == 5:
                    c.setFillColor(CREAM_DIM)
                else:
                    c.setFillColor(SKY_LIGHT)
                ...
                x = MARGIN + name_col_w + rel_idx * wk_col_w + wk_col_w / 2
                c.drawCentredString(x, y, dose_compact)
        y -= 13
    ...
    legend = "W1 establish · W2 tempo push · W3 load · W4 volume · W5 heavy · W6 deload + re-test"
...
def generate_plan_pdf(program_json: str, output_pdf: str):
    ...
    for i in range(num_sessions):
        ...
        if sess.get('day_type', '').startswith('strength'):
            ...
            est_session_pages += 1
    total = len(base_pages) + est_session_pages + len(closing_pages)
    ...
    # Draw sessions · each may consume 1-2 pages
    for i in range(num_sessions):
    ...
    c.save()
    print(f"PDF v3 generated · {output_pdf}  (~{total} pages, actual = {page_num})")
