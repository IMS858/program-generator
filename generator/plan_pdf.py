...
def page_footer(c, page_num, total):
    ...
    NOTE · with NumberedCanvas (the canvas subclass below), this is called
    DURING save() with the real total page count. Existing inline call sites
    have been replaced with no-ops · NumberedCanvas handles everything.
    """
    y = MARGIN - 2
    c.setFillColor(CREAM_DIM)
...
# Canonical ReportLab pattern · subclass Canvas, capture page states in
# showPage(), re-play them in save() with the correct total page count
# drawn into each footer. Eliminates the "PG. 25 / 24" off-by-one bug
# caused by upfront estimation.
...
class NumberedCanvas(_RLCanvas):
    def __init__(self, *args, **kwargs):
        ...
        # Pages where the footer should be drawn · the cover (page 1) gets
        # no footer, every other page does. Track an index alongside.
        self._footer_skip_pages = set()

    def showPage(self):
        # Capture this page's state so we can re-play it during save() with
        # the correct total page count baked into the footer.
        self._saved_page_states.append(dict(self.__dict__))
        self._startPage()

    def skip_footer(self):
        """Call BEFORE showPage() on a page where you don't want a footer
        (e.g. the cover). The next captured page will be marked skip."""
        self._footer_skip_pages.add(len(self._saved_page_states))

    def save(self):
        total = len(self._saved_page_states)
        for i, state in enumerate(self._saved_page_states):
            self.__dict__.update(state)
            if i not in self._footer_skip_pages:
                page_footer(self, i + 1, total)
            super().showPage()
        super().save()


# ─── Cardio language helper ──────────────────────────────────
# Returns "Zone 2 only" when interval clearance is blocked, else the
# default "Zone 2 or intervals" wording. Used by every place that talks
# about cardio in client-facing copy (weekly structure, weekly routine,
# etc.) so we never tell a not-cleared client they could do intervals.

def _cardio_intensity_label(program: dict, default: str = "Zone 2 or intervals") -> str:
    """Returns the cardio intensity label appropriate for this client's
    interval clearance. If intervals are blocked (or the client is flagged
    not_cleared_for_intervals), returns "Zone 2 only" · otherwise default."""
    try:
        cp = program.get('assessment', {}).get('cardio_profile') or {}
        limits = set(cp.get('limitations') or [])
        if "not_cleared_for_intervals" in limits:
            return "Zone 2 only"
        # Run the rules engine for an authoritative answer
        from cardio_rules import normalize_cardio_profile, determine_interval_clearance
        class _Pseudo: pass
        p = _Pseudo()
        p.primary_modality = cp.get('primary_modality')
        p.secondary_modalities = cp.get('secondary_modalities') or []
        p.avoid_modalities = cp.get('avoid_modalities') or []
        p.limitations = cp.get('limitations') or []
        p.z2_baseline = cp.get('z2_baseline') or {}
        p.interval_test = cp.get('interval_test') or {}
        p.hr_recovery = cp.get('hr_recovery') or {}
        n = normalize_cardio_profile(
            p,
            concerns=program.get('assessment', {}).get('concerns'),
            constraints_rich=program.get('assessment', {}).get('constraints_rich'),
        )
        clearance = determine_interval_clearance(n)
...
def draw_strength_plan(c, program):
    ...
    for day_idx, label in enumerate(days):
        ...
        if session_num and session_num <= len(sessions):
            ...
            if dt == "cardio":
                detail = f"{_cardio_intensity_label(program)} · FRC reset finish"
            elif dt == "integration":
...
def draw_session_page(c, program, session_idx, day_num_in_week,
    ...
    def _new_page():
        ...
        # Footer drawn by NumberedCanvas in save() · no inline call here
        c.showPage()
        current_page += 1
        pages_used += 1
        fill_page(c, NAVY)
    ...
    for block in session['blocks']:
        ...
        if name.startswith("Strength A") or name.startswith("Strength B"):
            ...
            y = render_block_compact(c, block, y, program=program)
        y -= 8

    # Final showPage for the last page of this session · footer drawn by canvas
    c.showPage()
...
# Client-mode block-label remap · keep coach/data unchanged, only the
# rendered label changes for client-facing output.
_CLIENT_BLOCK_LABEL_REMAP = {
    "Coach Finisher":                  "Assisted Recovery",
    "Decompression Cool Down (optional)": "Cool Down (optional)",
}


def _client_block_label(block_name: str) -> str:
    """Translate a coach-internal block name to its client-facing label.
    No-op for any name not in the remap table."""
    if not block_name:
        return block_name
    # Match case-insensitively but preserve original prefix structure
    for k, v in _CLIENT_BLOCK_LABEL_REMAP.items():
        if block_name.startswith(k):
            return v + block_name[len(k):]
    return block_name


def render_block_compact(c, block, y, program=None):
    """Compact block renderer · for mobility, cool-down, cardio · Week 1 doses only
    since these stay constant across weeks.

    program (optional) · used to read _pdf_mode so client-mode can rename
    coach-only block labels (e.g., "Coach Finisher" → "Assisted Recovery").
    """
    block_name = block.get('name', '')
    exercises = block.get('exercises', [])

    # Client-mode label remap · "Coach Finisher" reads weird in a client plan
    pdf_mode = (program or {}).get('_pdf_mode', 'client') if program else 'client'
    if pdf_mode == 'client':
        block_name = _client_block_label(block_name)
    ...
    for ex in exercises:
        ...
        # keep the precise numbers for actual prescription. Client gets a
        # gentler "light effort" descriptor instead.
        if (pdf_mode == 'client'
                and block_name.startswith("Capsule Work")
                and "@ " in dose):
            import re as _re
            # Remove "@ XX-YY%" tokens · replace with "light effort" the first time
            simplified = _re.sub(r"@\s*\d+\s*-\s*\d+%", "", dose)
            simplified = _re.sub(r"\s+·\s+\s+·", " · ", simplified)  # tidy doubled separators
            simplified = _re.sub(r"\s{2,}", " ", simplified).strip()
            # Tail-pad with effort descriptor so the dose still reads well
            if "light effort" not in simplified.lower():
                # Insert "light effort" before "rounds"
                simplified = _re.sub(r"·\s*(\d+\s*round)", r"· light effort · \1",
                                       simplified, count=1)
            dose = simplified
...
def _cell_lines_for_week(wk_ex, week_num, max_w, c, program=None):
    ...
    if wp is None:
        wp = _synth_wp_from_dose(wk_ex.get('dose', ''), week_num, program=program)
    ...
    # Phase 3 · per-side prescription. When one side is materially weaker the
    # weaker side carries an extra set, and the cell has to say so rather than
    # printing one number for two different sides.
    side_plan = (wk_ex.get('dose_by_side') or {}).get('by_week', {}) \
        .get(str(week_num)) if isinstance(wk_ex.get('dose_by_side'), dict) else None

    sets = wp.get('sets')
    reps = wp.get('reps')
    if side_plan:
        left, right = side_plan.get('L'), side_plan.get('R')
        if left and right:
            lines.append(f"L {left['sets']} × {left['reps']}")
            lines.append(f"R {right['sets']} × {right['reps']}")
        else:
            lines.append(side_plan.get('display', f"{sets} × {reps}"))
    elif sets and reps:
...
# LEGACY LADDER · used only when the program carries no progression profile
# (a v1 program JSON, or a config-less environment).
#
# This table used to be the third independent copy of the 4-week ladder, after
# generator._strength_dose and strength_math.WEEK_TEMPLATES. That meant an
# individualized volume decision made in the generator was silently overwritten
# here on its way to the printed cell · a 68-year-old and a 28-year-old came
# out of the PDF with the same 3×12 → 3×10 → 4×8 → 4×6 no matter what the
# generator decided. _week_template_for() now reads the profile the generator
# resolved, and falls back to this only when there isn't one.
_WEEK_4_TEMPLATE = {
...
def _week_template_for(program, week_num, exercise_type="compound"):
    """The sets/reps/RPE/tempo for one week, for THIS client.

    Reads the ProgressionProfile the generator resolved and stamped onto the
    program. Falls back to the legacy IMS ladder for programs generated before
    v2.0, so old program JSON still renders.
    """
    prog = (program or {}).get("progression") or {}
    templates = (prog.get("week_templates") or {}).get(exercise_type) or []
    for tpl in templates:
        if tpl.get("week") == week_num:
            return {
                "sets": tpl.get("sets"),
                "reps": tpl.get("reps"),
                "rpe": tpl.get("rpe", ""),
    ...
    return dict(_WEEK_4_TEMPLATE.get(week_num, {})) or None


def _parse_dose_sets_reps(dose):
    """Pull (sets, reps) out of a dose string · '3 × 12/side', '4x8', '2 x 12'."""
    if not dose:
        return None, None
    m = re.search(r"(\d+)\s*[x×]\s*(\d+)", str(dose))
    if not m:
        return None, None
    return int(m.group(1)), int(m.group(2))


def _synth_wp_from_dose(dose, week_num, program=None, exercise_type="compound"):
    ...
    The DOSE STRING WINS for sets and reps. The generator already wrote it from
    this client's progression profile, so parsing it is what keeps an
    individualized volume decision intact all the way to the page. RPE, tempo
    and intent come from the profile (or the legacy ladder) because the dose
    string does not carry them.
    """
    tpl = _week_template_for(program, week_num, exercise_type)
    if not tpl:
        return None
    tpl = dict(tpl)
    sets, reps = _parse_dose_sets_reps(dose)
    if sets and reps:
        tpl["sets"], tpl["reps"] = sets, reps
    tpl["weight"] = None
    tpl["weight_unit"] = ""
    tpl["weight_note"] = ""
    return tpl
...
def draw_weekly_map(c, program):
    ...
    for day_idx, day in enumerate(days):
        ...
        if session_num and session_num <= len(sessions):
            ...
            dt = sess.get('day_type', '')
            if dt == "cardio":
                vals = ["—", "Cardio + FRC reset", "Hydration focus",
                         _cardio_intensity_label(program, default="Zone 2 / intervals")]
            elif dt == "integration":
...
def draw_coach_appendix(c, program):
    ...
    if tests:
        ...
        # Lower-bound sanity checks · the floor depends on load_style.
        # per_hand dumbbell loads (per side) are way lower than total_load
        # barbell weights · a 30 lb DB Incline Bench is normal for a beginner,
        # but a 30 lb total_load Trap Bar deadlift is almost certainly a typo.
        SUSPICIOUS_FLOOR_TOTAL = {
            "deadlift": 45, "trap_bar": 95, "squat": 45,
            "bench": 45, "row": 30, "press": 20, "hinge": 45,
        }
        SUSPICIOUS_FLOOR_PER_HAND = {
            # Dumbbell-typical · only flag if absurdly low.
            # 5 lb is a reasonable lower bound for any DB exercise.
            "deadlift": 5, "trap_bar": 5, "squat": 5,
            "bench": 5, "row": 5, "press": 5, "hinge": 5,
        }
        # bodyweight_added (pull-ups, dips with added weight) · 0 lb is fine
        # · skip the floor check entirely.
        ...
        for t in tests:  # show all anchors · spec requires no truncation
            ...
            # If cat is the placeholder "movement" or empty, derive a real one
            # from the exercise name via the alias resolver.
            if cat == 'movement' or not cat or cat.strip() == '':
                try:
                    from strength_anchor_resolver import _alias_group_for, _ALIAS_GROUPS_RAW
                    g = _alias_group_for(name)
                    if g and g in _ALIAS_GROUPS_RAW:
                        cat = _ALIAS_GROUPS_RAW[g].get("category", "movement")
                        # Use the more-specific group name when category is
                        # the umbrella label (e.g. group "hip_extension" with
                        # umbrella category "hinge" → use "hip_extension").
                        if g != cat and g in ("hip_extension", "horizontal_press",
                                                "vertical_press", "vertical_pull"):
                            cat = g
                except Exception:
                    pass
            ...
            if top:
                ...
                # Floor depends on load_style · per_hand DB weights vs total_load
                # barbell weights have very different normal ranges.
                cat_lower = (cat or "").lower()
                load_style = (t.get('load_style') or "").lower()
                if load_style == "bodyweight_added":
                    floor = None  # bodyweight_added · 0 is fine, skip check
                elif load_style == "per_hand":
                    floor_table = SUSPICIOUS_FLOOR_PER_HAND
                    floor = None
                    for cat_key, floor_v in floor_table.items():
                        if cat_key in cat_lower or cat_key in name.lower():
                            floor = floor_v
                            break
                else:
                    # default · total_load · or unspecified treated as barbell
                    floor_table = SUSPICIOUS_FLOOR_TOTAL
                    floor = None
                    for cat_key, floor_v in floor_table.items():
                        if cat_key in cat_lower or cat_key in name.lower():
                            floor = floor_v
                ...
                if floor and weight > 0 and weight < floor:
                    ...
                    c.setFont(SERIF_ITALIC, 9)
                    style_hint = f" ({load_style})" if load_style else ""
                    c.drawString(MARGIN, y,
                                  f"   ⚠ {weight:.0f} lb is unusually light for {cat_clean}{style_hint} · likely data entry error")
                    y -= 11

            if pain_notes:
                # Word-wrap onto multiple lines instead of truncating with marker
                c.setFillColor(LIMITED)
                c.setFont(SERIF_ITALIC, 9)
                wrapped = _wrap_to_lines(c, pain_notes, CONTENT_W - 20,
                                           SERIF_ITALIC, 9, max_lines=3)
                first = True
                for line in wrapped:
            ...
            if coach_notes_t:
                c.setFillColor(SKY_BLUE)
                c.setFont(SERIF_ITALIC, 9)
                wrapped = _wrap_to_lines(c, coach_notes_t, CONTENT_W - 20,
                                           SERIF_ITALIC, 9, max_lines=2)
                first = True
                for line in wrapped:
                    prefix = "   coach: " if first else "          "
                    c.drawString(MARGIN, y, f"{prefix}{line}")
                    y -= 11
                    first = False
            y -= 4
            ...
            if not used:
                ...
                for source_name, payload in used.items():
                    ...
                    for s in unique_slots[:4]:
                        method = s["method"]
                        with_loads = s["with_loads"]
                        loads_label = "loads ✓" if with_loads else "loads ✗ (rep scheme only)"
                        wp_label = "wp ✓" if s["has_wp"] else "wp ✗"
                        line = (f"applied to: {s['exercise']}  ·  match: {method}  ·  "
                                f"{wp_label}  ·  {loads_label}")
                        c.setFillColor(CREAM_DIM if with_loads else LIMITED)
                        c.setFont(SERIF_ITALIC, 9)
                        for chunk in _wrap_to_lines(c, line, CONTENT_W - 30, SERIF_ITALIC, 9, max_lines=2):
                            c.drawString(MARGIN + 28, y, chunk)
                            y -= 11

                        # Spec · explicit warning for soft matches
                        warning = None
                        if method == "category":
                            warning = ("⚠ matched by category only · the anchor's "
                                        "exercise family differs from the programmed exercise · "
                                        "loads not applied (use rep scheme only)")
                        elif method == "fuzzy":
                            warning = ("⚠ matched by fuzzy similarity · review whether this "
                                        "anchor really applies to the programmed exercise")
                        elif not with_loads and method in ("exact", "alias"):
                            warning = ("⚠ anchor matched but no weight rendered · check the "
                                        "load_style or that the tested rep max was non-zero")
                        if warning:
                            c.setFillColor(LIMITED)
                            c.setFont(SERIF_ITALIC, 8.5)
                            for chunk in _wrap_to_lines(c, warning, CONTENT_W - 36,
                                                          SERIF_ITALIC, 8.5, max_lines=2):
                                c.drawString(MARGIN + 36, y, chunk)
                                y -= 10
                    y -= 2
            y -= 4
    ...
    if rich:
        ...
        def _draw_wrapped(prefix, text, color, indent=16, max_lines=3):
            """Render a labeled note · word-wrapped, no '(truncated)' marker."""
            nonlocal y
            c.setFillColor(color)
            c.setFont(SERIF_ITALIC, 9)
            wrapped = _wrap_to_lines(c, text, CONTENT_W - indent - 4,
                                       SERIF_ITALIC, 9, max_lines=max_lines)
            first = True
            for line in wrapped:
                if first:
                    c.drawString(MARGIN + indent, y, f"{prefix}: {line}")
                else:
                    c.drawString(MARGIN + indent + len(prefix) + 2, y, line)
                y -= 11
                first = False
        ...
        for cr in rich[:6]:
            ...
            y -= 14
            if avoid:
                _draw_wrapped("avoid", avoid, LIMITED)
            if allowed:
                _draw_wrapped("allowed", allowed, OPTIMAL)
            if cnotes:
                _draw_wrapped("coach", cnotes, SKY_BLUE)
            y -= 4
        y -= 8
    ...
    if isinstance(cp, dict):
        ...
        avoid_set = set(cp.get('avoid_modalities') or [])
        # Per spec · if a modality is in avoid_modalities, it must NOT also
        # appear as an alternative. Coaches who marked something to avoid
        # don't want it offered as a fallback.
        secondaries = ", ".join(m.replace('_', ' ').title()
                                  for m in (cp.get('secondary_modalities') or [])
                                  if m not in avoid_set) or '—'
...
def has_objective_page(program) -> bool:
    """True when there is instrumented data worth a page."""
    obj = (program or {}).get("objective") or {}
    return bool(obj.get("measure_table") or obj.get("signals"))


def draw_objective_measures(c, program):
    """Coach-only page · the measured data and every decision it drove.

    Rule for this page · nothing appears here without the measurement and the
    date that produced it. A force-driven decision the coach cannot trace back
    to a number is worse than no decision.

    Client-facing pages never render this. Measured force is coach language.
    """
    obj = (program or {}).get("objective") or {}
    fill_page(c, NAVY)
    ghost_watermark(c, "ims")
    page_header_bar(c, "OBJECTIVE MEASURES", program['client_name'].upper())
    ...
    c.setFillColor(SKY_BLUE)
    c.setFont(SERIF_ITALIC, 42)
    c.drawString(MARGIN, y, "numbers said.")
    y -= 40
    ...
    def section(label):
        ...
        y -= 6
        small_caps_label(c, label, MARGIN, y, color=SKY_BLUE, size=10)
        lw = c.stringWidth(label, SANS_MEDIUM, 10)
        thin_divider(c, MARGIN + lw + 12, y + 3, PAGE_W - MARGIN, color=DIVIDER, width=0.5)
        y -= 18
        return True

    def line(text, color=None, font=None, size=8.5, indent=0):
        nonlocal y
        if y < MARGIN + 40:
            return
        c.setFillColor(color or CREAM_DIM)
        c.setFont(font or SERIF, size)
        for chunk in _wrap_to_lines(c, text, CONTENT_W - 20 - indent,
                                    font or SERIF, size, max_lines=3):
            if y < MARGIN + 30:
                return
            c.drawString(MARGIN + 8 + indent, y, chunk)
            y -= 11

    # ── provenance line ──
    cur, prev = obj.get("current_date"), obj.get("previous_date")
    stamp = f"Assessment {cur or 'undated'}"
    if prev:
        stamp += f" · previous {prev}"
    bw = obj.get("bodyweight_lb")
    if bw:
        stamp += f" · bodyweight {int(round(bw))} lb"
    line(stamp, color=CREAM, font=SANS_MEDIUM, size=8.5)

    # Provenance · a number nobody typed should be visibly distinguishable
    # from one somebody did, because they fail in different ways. A hand-typed
    # number can be a transcription error; a synced one can be attached to the
    # wrong athlete.
    sources = obj.get("sources") or []
    if sources:
        label = {"vald_api": "synced from VALD", "manual": "hand-entered"}
        line("Source · " + " + ".join(label.get(s, s) for s in sources),
             color=SKY_LIGHT, font=SERIF_ITALIC, size=8)
    if obj.get("staleness_note"):
        line(obj["staleness_note"], color=SKY_LIGHT, font=SERIF_ITALIC)
    y -= 10

    # ── routing signals · the reason this page exists ──
    signals = obj.get("signals") or []
    if signals and section("ROM × FORCE ROUTING"):
        for sig in signals:
            side = f" {sig['side']}" if sig.get("side") in ("L", "R") else ""
            head = f"{sig['joint'].title()} {sig.get('motion','')}{side} · {sig['quadrant'].replace('_',' ')}"
            if not sig.get("active"):
                head += "  [SUPPRESSED · hard contraindication wins]"
            line(head, color=CREAM, font=SANS_MEDIUM, size=9)
            line(sig.get("coach_note", ""), indent=10)
            if sig.get("evidence"):
                line(sig["evidence"], color=SKY_LIGHT, font=SERIF_ITALIC, size=8, indent=10)
            if sig.get("confidence") == "low":
                line("Low confidence · no bodyweight on file, classified on asymmetry alone.",
                     font=SERIF_ITALIC, size=7.5, indent=10)
            y -= 4

    # ── manual therapy referrals ──
    flags = obj.get("manual_therapy_flags") or []
    if flags and section("REFER · PASSIVE RESTRICTION"):
        for f in flags:
            line(f["text"], color=CREAM, size=8.5)
            line(f.get("evidence", ""), color=SKY_LIGHT, font=SERIF_ITALIC, size=8, indent=10)

    # ── weakest link ──
    emphasis = obj.get("emphasis") or []
    if emphasis and section("WEAKEST LINK · SELECTION EMPHASIS"):
        for e in emphasis:
            line(e["text"], color=CREAM, size=8.5)
            line(e.get("evidence", ""), color=SKY_LIGHT, font=SERIF_ITALIC, size=8, indent=10)

    # ── asymmetry ──
    asym = obj.get("asymmetries") or []
    if asym and section("SIDE-TO-SIDE"):
        for a in asym[:6]:
            tag = "significant" if a.get("significant") else "flagged"
            line(f"{a['test'].replace('_',' ')} · {a['gap_pct']}% gap, weaker on "
                 f"{a['weak_side']} ({tag})", color=CREAM, size=8.5)
            line(a.get("evidence", ""), color=SKY_LIGHT, font=SERIF_ITALIC, size=8, indent=10)

    # ── deltas since last assessment ──
    deltas = [d for d in (obj.get("deltas") or []) if d.get("meaningful")]
    if deltas and section(f"CHANGE SINCE {prev or 'LAST ASSESSMENT'}"):
        for d in deltas[:10]:
            # ASCII only · the embedded serif has no arrow or triangle glyphs
            # and silently renders them as unrelated letters.
            unit = d.get("unit", "")
            pct = f" ({d['delta_pct']:+.0f}%)" if d.get("delta_pct") is not None else ""
            direction = "up" if d["direction"] == "up" else "down"
            line(f"{d['label']} · {d['previous']} to {d['current']} {unit}{pct} "
                 f"· {direction}", color=CREAM, size=8.5)

    # ── full measure table ──
    rows = obj.get("measure_table") or []
    if rows and section("MEASURE TABLE"):
        mixed = len(obj.get("sources") or []) > 1
        for r in rows:
            side = f" {r['side']}" if r.get("side") in ("L", "R") else ""
            pos = f" {r['position'].replace('_','-')}" if r.get("position") else ""
            # Only tag the row when the assessment mixes sources · tagging every
            # row when they all came the same way is noise.
            tag = ""
            if mixed:
                tag = "  [VALD]" if r.get("source") == "vald_api" else "  [typed]"
            metrics = r.get("metrics") or {}
            rfd = metrics.get("rfd_n_per_s")
            extra = f"  · RFD {int(round(rfd))} N/s" if rfd else ""
            line(f"{r['device']} · {r['test'].replace('_',' ')}{pos}{side} · "
                 f"{r['value']} {r['unit']}{extra}{tag}", size=8)

    # ── parse warnings · never silent ──
    warns = obj.get("parse_warnings") or []
    if warns and section("INGEST WARNINGS"):
        for w in warns[:6]:
            line(w, color=SKY_LIGHT, font=SERIF_ITALIC, size=8)


def draw_coach_context(c, program):
    """Coach-only page · the free-text fields that used to go nowhere.

    red_flags, the six-row coach_notes textarea and the resolved progression
    profile were all collected and rendered by nothing. Someone was typing into
    that textarea every week and it was being dropped on the floor.
    """
    assessment = (program or {}).get("assessment", {})
    prog = (program or {}).get("progression") or {}
    fill_page(c, NAVY)
    ghost_watermark(c, "ims")
    page_header_bar(c, "COACH CONTEXT", program['client_name'].upper())

    y = PAGE_H - MARGIN - 150
    c.setFillColor(CREAM)
    c.setFont(SERIF, 42)
    c.drawString(MARGIN, y, "Your notes,")
    y -= 50
    c.setFillColor(SKY_BLUE)
    c.setFont(SERIF_ITALIC, 42)
    c.drawString(MARGIN, y, "on the record.")
    y -= 44
    ...
    def para(text, color=None, font=None, size=9):
        nonlocal y
        if not text or y < MARGIN + 40:
            return
        c.setFillColor(color or CREAM_DIM)
        c.setFont(font or SERIF, size)
        for chunk in _wrap_to_lines(c, str(text), CONTENT_W - 20, font or SERIF,
                                    size, max_lines=14):
            if y < MARGIN + 30:
                return
    ...
    if prog:
        section("PROGRESSION PROFILE")
        para(f"Ladder · {prog.get('ladder_id', '_default')} "
             f"({prog.get('progression_mode', '')})", color=CREAM, font=SANS_MEDIUM, size=9)
        reasons = prog.get("reasons") or []
        if reasons:
            para("Because · " + "; ".join(reasons))
        tmpl = (prog.get("week_templates") or {}).get("compound") or []
        if tmpl:
            para("Compound ladder · " + "  ".join(
                f"W{t['week']} {t['sets']}×{t['reps']}" for t in tmpl),
                color=SKY_LIGHT, font=SERIF_ITALIC, size=8.5)
        y -= 12

    red = str(assessment.get("red_flags") or "").strip()
    if red:
        section("RED FLAGS · HARD CONTRAINDICATIONS")
        para(red, color=CREAM)
        para("Any joint named above is treated as a checked concern box by the "
             "exercise picker · it can only remove exercises, never add them.",
             font=SERIF_ITALIC, size=8)
        y -= 12

    notes = str(assessment.get("coach_notes") or "").strip()
    if notes:
        section("COACH NOTES")
        para(notes, color=CREAM)
        y -= 12

    legacy = assessment.get("strength_marker_results") or {}
    if legacy:
        section("LEGACY MARKER RESULTS · NOT USED FOR LOAD")
        for k, v in list(legacy.items())[:12]:
            para(f"{str(k).replace('_', ' ')} · {v}", size=8.5)
        para("Free-text only. These no longer write a load · the deleted "
             "_fill_load path used to parse them with a regex.",
             font=SERIF_ITALIC, size=8)
...
def generate_plan_pdf(program_json: str, output_pdf: str, pdf_mode: str = "client"):
    ...
    c = NumberedCanvas(output_pdf, pagesize=LETTER)
    title_suffix = {
        "client": "",
        "coach": " · Coach Plan",
        "full": " · Full Plan",
    ...
    if pdf_mode == "coach":
        ...
        if has_objective_page(program):
            base_pages.append(draw_objective_measures)
        base_pages.append(draw_coach_context)
        closing_pages = []
    ...
    # In full mode, the coach appendix adds pages at the end
    appendix_pages = 0
    if pdf_mode == "full":
        appendix_pages = 2 + (1 if has_objective_page(program) else 0)
    total = len(base_pages) + est_session_pages + len(closing_pages) + appendix_pages

    # Draw base pages · cover (page 1) skips footer, others get one in save()
    page_num = 0
    for fn in base_pages:
        page_num += 1
        fn(c, program)
        if page_num == 1:
            c.skip_footer()  # cover page
        c.showPage()
    ...
    if pdf_mode == "full":
        ...
        if has_objective_page(program):
            page_num += 1
            draw_objective_measures(c, program)
            c.showPage()
        page_num += 1
        draw_coach_context(c, program)
        c.showPage()

    c.save()
    # Actual total comes from the canvas itself · saved_page_states
    actual_total = len(c._saved_page_states) if hasattr(c, '_saved_page_states') else page_num
    print(f"PDF generated · mode={pdf_mode} · {output_pdf}  ({actual_total} pages)")
