...
def draw_coach_appendix(c, program):
    ...
    if tests:
        ...
        for t in tests[:6]:  # cap to keep page tidy
            name = t.get('exercise_name') or '(unnamed)'
            cat = t.get('movement_category') or '?'
            form_q = t.get('form_quality') or '?'
            pain_notes = t.get('pain_or_compensation_notes') or ''
            coach_notes_t = t.get('coach_notes') or ''
            ...
            if pain_notes:
                ...
                c.setFont(SERIF_ITALIC, 9)
                c.drawString(MARGIN, y, f"   ⚠ {pain_notes[:90]}")
                y -= 11
            if coach_notes_t:
                c.setFillColor(SKY_BLUE)
                c.setFont(SERIF_ITALIC, 9)
    ...
    if rich:
        ...
        for cr in rich[:6]:
            ...
            avoid = cr.get('avoid_notes') or ''
            allowed = cr.get('allowed_notes') or ''
            cnotes = cr.get('coach_notes') or ''

            c.setFillColor(CREAM)
            c.setFont(SANS_MEDIUM, 10)
            c.drawString(MARGIN, y,
                         f"{display}  ·  side: {side}  ·  status: {status}"
                         + (f"  ·  pain: {pain}/10" if pain is not None else ""))
            y -= 14
            if avoid:
                c.setFillColor(LIMITED)
                c.setFont(SERIF_ITALIC, 9)
                c.drawString(MARGIN + 16, y, f"avoid: {avoid[:90]}")
                y -= 11
            if allowed:
                c.setFillColor(OPTIMAL)
                c.setFont(SERIF_ITALIC, 9)
                c.drawString(MARGIN + 16, y, f"allowed: {allowed[:90]}")
                y -= 11
            if cnotes:
                c.setFillColor(SKY_BLUE)
                c.setFont(SERIF_ITALIC, 9)
                c.drawString(MARGIN + 16, y, f"coach: {cnotes[:90]}")
                y -= 11
            y -= 4
        y -= 8
    ...
    if isinstance(cp, dict):
        ...
        try:
            ...
            machine_id, machine_rat = choose_primary_cardio_machine(normalized)
            clearance = determine_interval_clearance(normalized)
            flags = generate_cardio_coach_flags(normalized)

            c.setFillColor(CREAM)
            c.setFont(SANS_MEDIUM, 9)
            c.drawString(MARGIN, y, "MACHINE CHOICE")
            y -= 11
            c.setFillColor(CREAM_DIM)
            c.setFont(SERIF_ITALIC, 9)
            machine_label = MODALITY_DISPLAY.get(machine_id, machine_id)
            c.drawString(MARGIN + 14, y, f"{machine_label} · {machine_rat[:90]}"[:110])
            y -= 14
            ...
            if flags:
                ...
                for flag in flags[:6]:
                    ...
                    c.setFillColor(color)
                    c.setFont(SERIF_ITALIC, 9)
                    # Wrap long flags · simple width-aware split
                    text = flag.strip()
                    while text:
                        chunk = text[:105]
                        if len(text) > 105:
