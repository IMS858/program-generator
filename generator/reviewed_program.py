...
def validate_reviewed_program(program):
    ...
    for wi, week in enumerate(weeks):
        ...
        # Day types must remain aligned for the weekly map; rotating exercises
        # are supported by explicit week-specific PDF pages.
        if expected_days is None:
            expected_days = day_types
        elif day_types != expected_days:
            fail("session day types differ between weeks; weekly map cannot represent them")
    return program
