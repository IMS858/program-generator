...
Coach OS captures optional instrumented assessment data such as:
  ...
  VOLTRA        · explicitly verified isometric compound-pattern measurements only
...
CANONICAL_JOINTS = {"knee", "shoulder", "hip", "lumbar", "cervical", "wrist", "elbow", "ankle"}
DYNAMO_TESTS = {
    "grip": ("wrist", "grip"), "hand_grip": ("wrist", "grip"), "wrist_flexion": ("wrist", "flexion"), "wrist_extension": ("wrist", "extension"),
    "shoulder_ir": ("shoulder", "ir"), "shoulder_er": ("shoulder", "er"), "shoulder_abduction": ("shoulder", "abduction"), "shoulder_adduction": ("shoulder", "adduction"), "shoulder_flexion": ("shoulder", "flexion"), "shoulder_extension": ("shoulder", "extension"),
    "hip_abduction": ("hip", "abduction"), "hip_adduction": ("hip", "adduction"), "hip_ir": ("hip", "ir"), "hip_er": ("hip", "er"), "hip_flexion": ("hip", "flexion"), "hip_extension": ("hip", "extension"), "imtp": ("hip", "isometric_mid_thigh_pull"),
    "knee_extension": ("knee", "extension"), "knee_flexion": ("knee", "flexion"), "ankle_dorsiflexion": ("ankle", "dorsiflexion"), "ankle_plantarflexion": ("ankle", "plantarflexion"), "ankle_inversion": ("ankle", "inversion"), "ankle_eversion": ("ankle", "eversion"), "elbow_flexion": ("elbow", "flexion"), "elbow_extension": ("elbow", "extension"), "cervical_flexion": ("cervical", "flexion"), "cervical_extension": ("cervical", "extension"), "cervical_lateral_flexion": ("cervical", "lateral_flexion"), "trunk_flexion": ("lumbar", "flexion"), "trunk_extension": ("lumbar", "extension")}
...
def canonical_joint(raw):
    ...
    s=str(raw).strip().lower().replace(" ","_").replace("-","_");s=_JOINT_ALIASES.get(s,s);return s if s in CANONICAL_JOINTS else None
def canonical_side(raw):return _SIDE_ALIASES.get(str(raw or "").strip().lower(),"bilateral")
def canonical_motion(raw):
    s=str(raw or "").strip().lower().replace(" ","_").replace("-","_");return {"internal_rotation":"ir","int_rotation":"ir","internal_rot":"ir","external_rotation":"er","ext_rotation":"er","external_rot":"er","abd":"abduction","add":"adduction","flex":"flexion","ext":"extension","df":"dorsiflexion"}.get(s,s)
def _to_float(v):
    import math,re
...
@dataclass
class ForceMeasure:
    test:str;joint:Optional[str];motion:str;side:str;value_lb:float;device:str="dynamo";position:Optional[str]=None;raw_value:Optional[float]=None;raw_unit:Optional[str]=None;measured_on:Optional[str]=None;source:str="manual";source_id:Optional[str]=None;metrics:dict=field(default_factory=dict)
    @property
...
class ObjectiveMeasures:
    ...
    def deltas(self,cfg=None):
        ...
        for f in self.current.forces:
            ...
            delta=f.value_lb-prev.value_lb;pct=delta/prev.value_lb*100;out.append({"kind":"force","label":f.label(),"previous":round(prev.value_lb,1),"current":round(f.value_lb,1),"unit":"lb","delta":round(delta,1),"delta_pct":round(pct,1),"meaningful":abs(pct)>=pct_gate,"direction":"up" if delta>0 else "down" if delta<0 else "flat","previous_date":self.previous_date,"current_date":self.current_date})
        for r in self.current.roms:
            prev=self.previous.rom(r.joint,r.motion,r.side)
            if prev is None:continue
            delta=r.degrees-prev.degrees;out.append({"kind":"rom","label":r.label(),"previous":round(prev.degrees,1),"current":round(r.degrees,1),"unit":"deg","delta":round(delta,1),"delta_pct":round(delta/prev.degrees*100,1) if prev.degrees else None,"meaningful":abs(delta)>=deg_gate,"direction":"up" if delta>0 else "down" if delta<0 else "flat","previous_date":self.previous_date,"current_date":self.current_date})
        return out
...
def _parse_measure_set(raw,cfg,warnings,which,fallback_date=None):
    ...
    for i,entry in enumerate(raw.get("dynamo") or raw.get("force") or []):
        ...
        joint,motion=DYNAMO_TESTS[test];ms.forces.append(ForceMeasure(test=test,joint=joint,motion=motion,side=canonical_side(entry.get("side")),value_lb=lb,device="activforce_2" if source.startswith("activforce") else "dynamo",raw_value=val,raw_unit=str(entry.get("unit") or "lb"),measured_on=measured_on,source=source,source_id=str(entry["source_id"]) if entry.get("source_id") else None,metrics=entry.get("metrics") if isinstance(entry.get("metrics"),dict) else {}))
    for i,entry in enumerate(raw.get("voltra") or []):
        ...
        test_type=str(entry.get("test_type") or "").strip().lower()
        if test_type != "isometric":warnings.append(f"{which}.voltra[{i}] ignored · explicit test_type=isometric required");continue
        ...
        val=_to_float(entry.get("value") if entry.get("value") is not None else entry.get("peak_force"));lb=_to_lb(val,entry.get("unit"),cfg)
        if lb is None or not (0 < lb <= 700):warnings.append(f"{which}.voltra[{i}] ignored · invalid or implausible isometric force");continue
        ms.forces.append(ForceMeasure(test=pattern,joint=canonical_joint(entry.get("joint")),motion=canonical_motion(entry.get("motion") or "compound"),side=canonical_side(entry.get("side")),value_lb=lb,device="voltra",position=position,raw_value=val,raw_unit=str(entry.get("unit") or "lb"),measured_on=measured_on,source="voltra_isometric",source_id=str(entry["source_id"]) if entry.get("source_id") else None))
...
def parse_objective_measures(raw,cfg=None):
    ...
    cfg=cfg or load_thresholds();warnings=[];current_raw=raw.get("current")
