...
# LEGACY LADDER · the fallback when no ProgressionProfile is supplied.
# Not the only ladder any more. See config/objective_thresholds.json →
# progression.ladders and generator/progression_profile.py.
WEEK_TEMPLATES = [
...
def generate_4_week_progression(strength_test, exercise_name: Optional[str] = None,
                                ladder: Optional[list] = None) -> dict:
    """Build a 4-week progression for one exercise.

    ``ladder`` · the client's sets/reps/RPE ladder, resolved by
    progression_profile. Pass None to get WEEK_TEMPLATES, which is the legacy
    IMS block and now exists only as the no-individualization fallback · the
    ladder used to be hard-coded here AND in generator._strength_dose AND in
    plan_pdf._WEEK_4_TEMPLATE, and an individualized volume decision could not
    survive the trip to the printed cell. All three resolve through
    progression_profile now.
    """
    estimates = calculate_estimates_from_tests(strength_test)
    has_data = estimates.conservative_training_max is not None

    templates = ladder or WEEK_TEMPLATES

    weeks = []
    for tpl in templates:
        week_num = tpl.get("week")
        target_reps = tpl.get("reps")
