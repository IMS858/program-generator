...
class Generator:
    ...
    def _attach_week_prescriptions(self, ex: Exercise, assessment: Assessment) -> Exercise:
        """If a strength anchor matches this exercise, attach 4-week prescriptions.

        Uses strength_anchor_resolver for exact / alias / category / fuzzy matching
        (much smarter than the old 2-token-overlap heuristic). Records the match in
        the assessment-level usage tracker so the coach appendix can list USED vs
        UNUSED anchors.
        """
        tests = getattr(assessment, "strength_marker_tests", None) or []
        if not tests:
    ...
    def _strength_dose(self, wk_num: int, exercise_type: str, pattern: str = None) -> str:
        ...
        Recovery-adjusted: if _current_recovery < 0.85, W1 volume is reduced
        to manage fatigue in sleep-deprived / high-stress clients.
        """
        ...
        if exercise_type == "compound":
            if wk_num == 1:
                base = f"3 × 12{side}"
                return self._volume_adjusted_dose(base, recovery) if recovery < 0.85 else base
            if wk_num == 2:
                return f"3 × 10{side}"
            if wk_num == 3:
                return f"4 × 8{side}"
    ...
    def _build_strength_b(self, day_type: str, priorities: list[FRAPriority],
        ...
        core_ex = self._fill_load(core_ex, assessment)
        ...
        if priorities:
            ...
            if corrective:
                corrective.dose = corrective.dose or self._strength_dose(wk_num, "corrective")
                ...
                corrective = self._fill_load(corrective, assessment)
