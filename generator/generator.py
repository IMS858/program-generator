...
class Generator:
    ...
    def _pick_core_accessory(self, assessment, day_type: str, day_num: int,
        ...
        """Pick a core accessory · filtered by concerns, rotated by session+week.

        Strategy ·
          1. Load the core_accessory_library exercises
          2. Filter by client level (beginner gets beginner only,
             intermediate gets beginner+intermediate, advanced gets all)
          3. Filter by client concerns (drop avoid, defer caution)
          4. Filter by exclude_names so we never duplicate within the session
          5. Rotate by combined (day_num, wk_num) so LB Day 1 ≠ LB Day 2
             ≠ UB Day 1, AND each rotates across the 4-week block
        ...
        # Drop names already used in this session
        pool = [v for v in pool if v["name"] not in exclude_names]
        if not pool:
            return None

        # Rotation index · stride by day_num so each session of the same week
        # gets a different pick, plus wk_num offset so the 4-week block rotates
        rotation_idx = ((day_num - 1) + (wk_num - 1) * 2) % len(pool)
    ...
    def _build_session(self, assessment: Assessment, day_num: int,
        ...
        # in the same session (e.g., Hip Thrust appearing in both because the
        # bridge picker also consults anchors). Pass the used names through.
