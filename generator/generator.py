"""
IMS PROGRAM GENERATOR v2.0
==========================
Takes a filled assessment → outputs a 4-week structured program.

Usage ·
    from generator import Assessment, Generator
    assessment = Assessment.from_json("matt_assessment.json")
    generator = Generator()
    program = generator.build_program(assessment)
    program.to_json("matt_program.json")

Design principles ·
- Never ships a contraindicated exercise
- Never adds load to a RED-scored joint
- Coach can override any generator decision
- Defaults match the IMS methodology; overrides use coach judgment
- ONE mechanism writes load. Never two.

v2.0 adds an optional objective-measurement layer (VALD DynaMo / VOLTRA /
passive ROM) and individualized volume. Both degrade to nothing ·

    No objective_measures and no age/training-age/conditioning on file
    → byte-identical output to v1.0. Proven by
      tests/test_objective_measures.TestGracefulDegradation.

Every cutoff used by that layer lives in config/objective_thresholds.json.
"""

import json
import random
import re
import zlib
from dataclasses import dataclass, field, asdict
from pathlib import Path
from typing import Optional

from ims_contract import (
    CONTRACT_VERSION, GENERATOR_VERSION, PROTOCOL_VERSION,
    load_thresholds, version_stamp,
)
from joint_taxonomy import (
    CONCERN_TO_JOINT, PATTERN_JOINT_MAP, SAFE_PATTERNS, concern_to_joint,
)

# ==========================================================
# DATA MODELS
# ==========================================================

@dataclass
class MobilityRating:
    """Traffic light per joint direction."""
    joint: str           # e.g. "hip"
    direction: str       # e.g. "IR" or "flexion"
    side: str            # "L", "R", or "bilateral"
    rating: str          # "red", "yellow", "green"

    @property
    def key(self) -> str:
        return f"{self.joint}_{self.direction}_{self.side}".lower()


@dataclass
class FRAPriority:
    """One joint + direction requiring focused attention."""
    description: str     # e.g. "Hip IR L+R"
    joints: list[str]    # parsed from description · ["hip"]
    directions: list[str]  # ["IR"]
    sides: list[str]     # ["L", "R"]
    region: str          # "lower_body" or "upper_body" or "spine"


@dataclass
class Assessment:
    """The complete assessment input for one client."""
    name: str
    age_range: str                      # "late 40s"
    sex: str
    background: str                     # "ex-military"
    training_frequency: int             # Total days/week (strength + cardio). Kept for back-compat.
    primary_goal: str
    fra_priorities: list[FRAPriority]
    strength_markers: list[str]         # IDs from marker library
    constraints: list[str]              # e.g. ["SI_joint_sensitivity", "no_axial_loading"]
    mobility_map: list[MobilityRating]
    body_comp: dict = field(default_factory=dict)
    # Was "autoregulated" · which was not true. The 4-week ladder is a fixed
    # calendar with no performance input, and printing "autoregulated" on a
    # client-facing document described something the generator does not do.
    # Set by the generator from the resolved ProgressionProfile; any legacy
    # value coming in over the wire is normalized on ingest.
    progression_mode: str = "fixed_4week_block"
    # DEPRECATED · free-text marker results. Read by nothing. Until v2.0 this
    # fed a second, parallel load path (_fill_load) that raced the real one;
    # that path is deleted. The field is still accepted so old Coach OS
    # payloads do not 400, and it is surfaced verbatim in the coach appendix,
    # but it never writes a load.
    strength_marker_results: dict = field(default_factory=dict)
    strength_days: int = 0              # Explicit strength days/week (preferred over training_frequency)
    cardio_days: int = 0                # Explicit cardio days/week
    # NEW · richer per-marker test data (StrengthTest objects from strength_testing.py)
    # Optional · empty list means we fall back to legacy strength_marker_results.
    strength_marker_tests: list = field(default_factory=list)
    # NEW · client concerns · checkbox-driven joint flags + free-text nuance
    # concerns examples · ["bad_knee", "bad_shoulder", "lower_back", "hip", "neck", "wrist", "elbow", "ankle"]
    # concern_notes · "Right meniscus repair 2019, still touchy on deep flexion"
    concerns: list = field(default_factory=list)
    concern_notes: str = ""
    # NEW · structured constraint data · each entry is a dict with side/status/pain/notes
    # Schema: {key, display_name, side, status, pain_level, avoid_notes, allowed_notes, coach_notes}
    # status values: active_flare_up | history | cleared | post_surgery | avoid_loading
    constraints_rich: list = field(default_factory=list)
    # NEW · PDF rendering mode · "client" / "coach" / "full"
    pdf_mode: str = "client"
    # NEW · cardio capacity & machine tolerance profile (CardioProfile object)
    # If left as default empty profile, generator falls back to generic cardio block.
    cardio_profile: object = None
    # NEW · coach-selected accessory categories for the Strength C block.
    # Examples: ["arms", "calves_tibialis", "loaded_carries"]
    # When empty, the picker uses sensible day-type defaults (LB→calves/glutes/carries,
    # UB→arms/neck/shoulders).
    accessory_categories: list = field(default_factory=list)
    # ── Coach OS integration fields ──────────────────────
    # These enable recovery-aware, posture-corrective, pain-graded programming.
    sleep_quality: str = ""             # "good", "fair", "poor"
    sleep_hours: str = ""               # e.g. "6"
    stress_level: str = ""              # "low", "moderate", "high", "very_high"
    desk_hours: str = ""                # e.g. "9"
    resting_hr: str = ""               # e.g. "82"
    posture: dict = field(default_factory=dict)  # {head_position, shoulder_position, thoracic_curve, lumbar_curve, pelvic_tilt, knee_position, foot_arch}
    pain_map: dict = field(default_factory=dict)  # {area: {severity: "0-10", description: "", status: ""}}
    rom_degrees: dict = field(default_factory=dict)  # {joint: degrees_str}
    red_flags: str = ""                 # coach's hard contraindications
    coach_notes: str = ""               # free-text coach observations
    # ── v2.0 · objective measurement + individualization ──
    # Optional instrumented assessment · VALD DynaMo peak isometric force,
    # passive ROM in degrees, VOLTRA mid/end-range compound force. Carries the
    # current assessment, the previous one when it exists, and both dates.
    # Absent for most clients for months; some never have it. Absent must cost
    # nothing · see tests/test_objective_measures.TestGracefulDegradation.
    objective_measures: object = None   # ObjectiveMeasures | None
    assessment_date: str = ""           # ISO date of this intake
    training_age_years: object = None   # years of consistent training
    conditioning_level: str = ""        # deconditioned | moderate | well_conditioned
    # Examples (legacy strength_marker_results) ·
    #   {"inverted_rows": "18 reps (30s)", "lat_pulldown": "140x3",
    #    "landmine_sa_press": "6 x 40 lbs"}

    @classmethod
    def from_json(cls, path: str) -> "Assessment":
        data = json.loads(Path(path).read_text())
        data["fra_priorities"] = [FRAPriority(**p) for p in data["fra_priorities"]]
        data["mobility_map"] = [MobilityRating(**r) for r in data["mobility_map"]]
        # Hydrate strength_marker_tests if present
        if data.get("strength_marker_tests"):
            from strength_testing import StrengthTest
            data["strength_marker_tests"] = [StrengthTest.from_dict(t) for t in data["strength_marker_tests"]]
        return cls(**data)

    def to_json(self, path: str):
        data = asdict(self)
        Path(path).write_text(json.dumps(data, indent=2))


@dataclass
class Exercise:
    """One exercise as it appears in a session."""
    name: str
    library: str                        # which library it was drawn from
    library_id: str | None = None       # entry ID in source library
    dose: str = ""                      # "3x8/side" or "2x5 @ 70%"
    tempo: str | None = None            # e.g. "3-sec eccentric"
    progression_note: str | None = None # week-to-week modifier
    rationale: str | None = None        # why this exercise for this client
    # NEW · Per-week prescriptions from strength_math (when test data exists).
    # List of dicts · [{week, sets, reps, weight, weight_unit, weight_note,
    #                   rpe, intent_label, tempo_note, fallback_text}, ...]
    # Empty list = use default dose strings (legacy behavior).
    week_prescriptions: list = field(default_factory=list)
    # NEW · anchor matching metadata · used by coach appendix to render
    # USED / UNUSED anchors and the per-exercise debug info (week_prescriptions
    # yes/no, rendered_with_loads yes/no). Set by _attach_week_prescriptions.
    anchor_match_method: str | None = None    # "exact"|"alias"|"category"|"fuzzy"|None
    anchor_source_name: str | None = None     # the exercise_name of the matched anchor
    anchor_rendered_with_loads: bool = False  # whether ANY week ended up with a weight
    # NEW v2.0 · every force-driven decision is annotated with the measurement
    # that caused it. load_source is "tested_anchor" when the existing resolver
    # matched, "measured_isometric_force" when a DynaMo/VOLTRA value served as
    # the anchor instead, and None when the exercise carries no load.
    load_source: str | None = None
    load_source_detail: str | None = None     # e.g. "DYNAMO knee extension L 62 lb, 2026-06-14"
    # NEW v2.0 · per-side prescription · {"L": {...}, "R": {...}, "_weak_side", "_reason"}
    dose_by_side: dict = field(default_factory=dict)
    # NEW v2.0 · why the objective layer touched this exercise (selection bias,
    # routing, side dose). Rendered in the coach plan, never the client plan.
    objective_notes: list = field(default_factory=list)


@dataclass
class Block:
    """One block of a session (e.g. Mobility Prep, Strength A)."""
    name: str
    exercises: list[Exercise]
    duration_note: str = ""


@dataclass
class Session:
    """One training day."""
    day_number: int
    day_type: str                       # "strength_lb", "strength_ub", "cardio", "integration"
    focus: str                          # "Lower Body Strength + Left Shoulder ER"
    blocks: list[Block]


@dataclass
class Week:
    """One week of the program."""
    week_number: int
    intent: str                         # "Establish Tolerance" / "Push One Lever" / etc.
    sessions: list[Session]
    progression_notes: list[str] = field(default_factory=list)


@dataclass
class Program:
    """Complete 4-week program for one client."""
    client_name: str
    block_number: int                   # which 4-week block (1st, 2nd, etc.)
    weeks: list[Week]
    assessment: Assessment
    generator_version: str = GENERATOR_VERSION
    contract_version: str = CONTRACT_VERSION
    protocol_version: str = PROTOCOL_VERSION
    # Everything the objective layer decided, and why. Empty dict when no
    # instrumented data was supplied.
    objective: dict = field(default_factory=dict)
    # The resolved volume/progression individualization (Phase 2).
    progression: dict = field(default_factory=dict)

    def to_json(self, path: str):
        # Convert assessment, with custom handling for cardio_profile (not a dataclass)
        a_dict = asdict(self.assessment)
        cp = getattr(self.assessment, 'cardio_profile', None)
        if cp is not None and hasattr(cp, 'to_dict'):
            a_dict['cardio_profile'] = cp.to_dict()
        elif cp is None:
            a_dict['cardio_profile'] = None
        om = getattr(self.assessment, 'objective_measures', None)
        a_dict['objective_measures'] = om.to_dict() if hasattr(om, 'to_dict') else None
        data = {
            "client_name": self.client_name,
            "block_number": self.block_number,
            "generator_version": self.generator_version,
            "contract_version": self.contract_version,
            "protocol_version": self.protocol_version,
            "weeks": [asdict(w) for w in self.weeks],
            "assessment": a_dict,
        }
        # Only emit the v2 sections when they carry something · keeps the
        # no-objective-data output identical to the v1 shape.
        if self.objective:
            data["objective"] = self.objective
        if self.progression:
            data["progression"] = self.progression
        Path(path).write_text(json.dumps(data, indent=2, default=str))


# ==========================================================
# THE GENERATOR
# ==========================================================

class Generator:
    """Builds a complete 4-week program from an assessment."""

    def __init__(self, libraries_path: str = None):
        if libraries_path is None:
            # Default: ../libraries relative to this file (for GitHub repo structure)
            libraries_path = str(Path(__file__).resolve().parent.parent / "libraries")
        self.lib_path = Path(libraries_path)
        self.db = self._load_unified_db()
        self.markers = self._load_markers()
        self.hip_mobility = self._load_hip_mobility()
        self.corrective_lib = self._load_corrective_library()
        self.core_lib = self._load_core_accessory_library()
        self.accessory_lib = self._load_accessory_library()
        # Objective layer state · always present, empty until build_program
        # resolves it, so every consumer can read it unconditionally.
        try:
            self._cfg = load_thresholds()
        except Exception:
            self._cfg = {}
        self._reset_objective_state()

    def _load_accessory_library(self) -> dict:
        """Load Strength C accessory library (categorized · arms, calves, etc.).
        Returns {} if missing · Strength C falls back to None and gets skipped."""
        path = self.lib_path / "accessory_library.json"
        if not path.exists():
            return {}
        try:
            return json.loads(path.read_text())
        except Exception:
            return {}

    def _load_corrective_library(self) -> dict:
        """Load FRA corrective library (multiple variants per joint+direction).
        Returns {} if missing · picker falls back to legacy hard-coded mapping."""
        path = self.lib_path / "corrective_library.json"
        if not path.exists():
            return {}
        try:
            return json.loads(path.read_text())
        except Exception:
            return {}

    def _load_core_accessory_library(self) -> dict:
        """Load core accessory library (rotates by session+week).
        Returns {} if missing · picker falls back to legacy hard-coded list."""
        path = self.lib_path / "core_accessory_library.json"
        if not path.exists():
            return {}
        try:
            return json.loads(path.read_text())
        except Exception:
            return {}

    def _filter_variants_by_concerns(self, variants: list, assessment) -> list:
        """Filter library variants against client concerns.

        Each variant is a dict with knee_sensitive/low_back_sensitive/etc. tags
        of "safe" | "caution" | "avoid". A concern that maps to a sensitivity
        flag drops any variant tagged "avoid" for that flag, and de-prioritizes
        (but does not drop) any "caution" variants by sorting them last.

        Returns a new list of variants, with avoid-tagged ones removed and
        caution-tagged ones at the end.
        """
        if not variants:
            return []
        concerns = set(getattr(assessment, "concerns", None) or [])
        constraints = set(getattr(assessment, "constraints", None) or [])
        all_flags = concerns | constraints

        # Concern key → tag field name
        concern_to_tag = {
            "bad_knee":           "knee_sensitive",
            "post_surgery_knee":  "knee_sensitive",
            "meniscus_flare":     "knee_sensitive",
            "knee_sensitive":     "knee_sensitive",
            "bad_back":           "low_back_sensitive",
            "low_back_sensitive": "low_back_sensitive",
            "bad_shoulder":       "shoulder_sensitive",
            "shoulder_sensitive": "shoulder_sensitive",
            "bad_wrist":          "wrist_sensitive",
            "wrist_sensitive":    "wrist_sensitive",
        }
        active_tags = {concern_to_tag[c] for c in all_flags if c in concern_to_tag}

        if not active_tags:
            return list(variants)

        safe_first, cautioned = [], []
        for v in variants:
            verdict = "safe"
            for tag in active_tags:
                t = v.get(tag, "safe")
                if t == "avoid":
                    verdict = "avoid"
                    break
                if t == "caution":
                    verdict = "caution"
            if verdict == "avoid":
                continue
            (cautioned if verdict == "caution" else safe_first).append(v)
        return safe_first + cautioned

    def _pick_core_accessory(self, assessment, day_type: str, day_num: int,
                              wk_num: int, exclude_names: set = None) -> Optional['Exercise']:
        """Pick a core accessory · rotates ACROSS categories, not flat list.

        Strategy ·
          1. Load library, filter by client level + concerns + exclude_names
          2. Group remaining picks by category
          3. Rotate the category by session: LB Day 1 → antiextension,
             UB Day 2 → antirotation, LB Day 3 → antilateralflexion, etc.
             This guarantees variety across the week (no 3 deadbugs in a row).
          4. Within the chosen category, rotate the variant by week_num.
          5. If the chosen category has nothing left after filtering, fall
             through to the next category in the rotation order.

        Returns None if the library isn't loaded · caller falls back to legacy.
        """
        exclude_names = exclude_names or set()
        lib = (self.core_lib or {}).get("exercises", [])
        if not lib:
            return None

        # Filter by client level · advanced gets full pool
        tier = self._infer_client_tier(assessment)
        level_order = {"beginner": ["beginner"],
                       "intermediate": ["beginner", "intermediate"],
                       "advanced": ["beginner", "intermediate", "advanced"]}
        levels_allowed = set(level_order.get(tier, ["beginner", "intermediate"]))
        pool = [v for v in lib if v.get("client_level") in levels_allowed]

        # Filter by concerns + already-used names
        pool = self._filter_variants_by_concerns(pool, assessment)
        pool = [v for v in pool if v["name"] not in exclude_names]
        if not pool:
            return None

        # Group by category · then walk in canonical rotation order so each
        # session lands on a different category. The rotation is keyed on
        # (day_num + wk_num) so a 4-week block also varies week-to-week.
        from collections import defaultdict
        by_cat = defaultdict(list)
        for v in pool:
            by_cat[v.get("category", "antiextension")].append(v)

        # Canonical rotation order across categories. Start with the highest-
        # value patterns (antiext then antirot) and cycle down. Spinal/carry
        # come last because they're more accessory than core.
        canonical_order = [
            "antiextension", "antirotation", "antilateralflexion",
            "spinal_articulation", "loaded_carry",
        ]
        # Keep only categories that have variants after filtering
        available_cats = [c for c in canonical_order if by_cat.get(c)]
        if not available_cats:
            return None

        # Rotate the category by (day + week) · this guarantees that in a
        # week with 3 sessions, all 3 land on different categories.
        cat_idx = ((day_num - 1) + (wk_num - 1)) % len(available_cats)
        chosen_cat = available_cats[cat_idx]
        cat_pool = by_cat[chosen_cat]

        # Within the category, rotate the variant by week so the same category
        # picked on different weeks gets a different variant.
        var_idx = (wk_num - 1) % len(cat_pool)
        pick = cat_pool[var_idx]

        return Exercise(
            name=pick["name"],
            library="core_accessory",
            dose=pick.get("default_dose", "3 × 10"),
            rationale=pick.get("rationale", "Core accessory volume"),
        )

    def _pick_corrective_from_library(self, priority: 'FRAPriority',
                                        assessment, day_type: str = None,
                                        day_num: int = 1,
                                        wk_num: int = 1,
                                        exclude_names: set = None) -> Optional['Exercise']:
        """Pick a corrective from the new tagged library.

        Routes by (joint, direction) → variant pool → filter by concerns +
        client level → rotate by session+week. Returns None if the library
        isn't loaded OR no variants match · caller falls back to the legacy
        hard-coded mapping.
        """
        exclude_names = exclude_names or set()
        catalog = (self.corrective_lib or {}).get("correctives", {})
        if not catalog:
            return None

        joint = priority.joints[0].lower() if priority.joints else ""
        direction = priority.directions[0].lower() if priority.directions else ""

        # Joint synonyms · "spine"/"thoracic" both → "thoracic_*", "tspine"→"thoracic"
        joint_synonyms = {
            "spine":     "thoracic",  # most spine priorities are T-spine related
            "tspine":    "thoracic",
            "t-spine":   "thoracic",
            "lspine":    "lumbar",
            "l-spine":   "lumbar",
            "neck":      "cervical",
            "toe":       "great_toe",
            "scap":      "scapular",
            "scapula":   "scapular",
        }
        joint = joint_synonyms.get(joint, joint)

        # If direction came back as "general" but description carries a real
        # direction word (rotation, extension, flexion), use that instead.
        if direction in ("", "general"):
            desc_lower = (priority.description or "").lower()
            for keyword in ("extension", "flexion", "rotation", "abduction",
                              "adduction", "ir", "er", "dorsiflexion",
                              "plantarflexion", "eversion", "inversion"):
                if keyword in desc_lower:
                    direction = keyword
                    break

        # Try (joint, direction) first, then joint-only fallback
        key = f"{joint}_{direction}" if direction and direction != "general" else joint
        variants = catalog.get(key) or catalog.get(joint)
        if not variants:
            return None

        # Filter by client level
        tier = self._infer_client_tier(assessment)
        level_order = {"beginner": ["beginner"],
                       "intermediate": ["beginner", "intermediate"],
                       "advanced": ["beginner", "intermediate", "advanced"]}
        levels_allowed = set(level_order.get(tier, ["beginner", "intermediate"]))
        pool = [v for v in variants if v.get("client_level") in levels_allowed]

        # Filter by concerns
        pool = self._filter_variants_by_concerns(pool, assessment)

        # Drop names already used in session
        pool = [v for v in pool if v["name"] not in exclude_names]
        if not pool:
            return None

        # Rotation by session+week · stride 2 on day_num gives more spread
        rotation_idx = ((day_num - 1) * 2 + (wk_num - 1)) % len(pool)
        pick = pool[rotation_idx]

        # Side-specific rendering
        side_note = ""
        if "L+R" not in priority.description and len(priority.sides) == 1:
            side_note = f" ({priority.sides[0]} side)"

        return Exercise(
            name=pick["name"] + side_note,
            library="corrective",
            dose=pick.get("default_dose"),
            rationale=pick.get("rationale", f"Corrective for {priority.description}"),
        )

    def _pick_strength_c_accessories(self, assessment, day_type: str,
                                       day_num: int, wk_num: int,
                                       exclude_names: set = None) -> list:
        """Pick 3 Strength C accessories for this day.

        Strategy ·
          1. Read coach-selected accessory_categories from the assessment.
          2. Filter to categories that fit this day type:
             - LB sessions get lower-body categories
             - UB sessions get upper-body categories
             - "both" categories (carries, conditioning, grip) on either day
          3. From each remaining category, pick 1 variant filtered by client
             level + concerns + exclude_names. Rotate by week.
          4. If fewer than 3 categories survive after coach picks, top up with
             defaults so we always return 3 picks (or fewer if even defaults
             run dry · caller hides Strength C if 0 picks returned).

        Returns a list of Exercise objects (0-3 entries).
        """
        catalog = (self.accessory_lib or {}).get("accessories", {})
        if not catalog:
            return []

        coach_picks = list(getattr(assessment, "accessory_categories", None) or [])

        # Day-type routing
        routing = (self.accessory_lib or {}).get("category_routing", {})
        if day_type == "strength_lb":
            day_cats = set(routing.get("lower_body_session", []))
        elif day_type == "strength_ub":
            day_cats = set(routing.get("upper_body_session", []))
        else:
            day_cats = set()
        bothcats = set(routing.get("both_sessions", []))

        # Coach picks intersected with day categories · coach order preserved
        eligible = [c for c in coach_picks if c in (day_cats | bothcats)]

        # Top up with default day-type categories if coach didn't pick enough
        if len(eligible) < 3:
            default_cats = routing.get(
                "lower_body_session" if day_type == "strength_lb"
                else "upper_body_session", []
            )
            for c in default_cats:
                if c not in eligible:
                    eligible.append(c)
                if len(eligible) >= 3:
                    break

        if not eligible:
            return []

        # Filter pools by client level + concerns
        tier = self._infer_client_tier(assessment)
        level_order = {"beginner": ["beginner"],
                       "intermediate": ["beginner", "intermediate"],
                       "advanced": ["beginner", "intermediate", "advanced"]}
        levels_allowed = set(level_order.get(tier, ["beginner", "intermediate"]))

        used_names = set(exclude_names or [])
        out = []
        for cat_key in eligible[:6]:  # cap at 6 to fill 3 slots
            if len(out) >= 3:
                break
            pool = catalog.get(cat_key, [])
            pool = [v for v in pool if v.get("client_level") in levels_allowed]
            pool = self._filter_variants_by_concerns(pool, assessment)
            pool = [v for v in pool if v["name"] not in used_names]
            if not pool:
                continue
            # Rotate variant by day+week · LB Day 1 ≠ LB Day 3 within a week,
            # AND each rotates across the 4-week block.
            idx = ((wk_num - 1) + (day_num - 1)) % len(pool)
            pick = pool[idx]
            ex = Exercise(
                name=pick["name"],
                library="accessory",
                dose=pick.get("default_dose", "3 × 10"),
                rationale=pick.get("rationale", "Accessory volume"),
            )
            out.append(ex)
            used_names.add(pick["name"])

        return out

    def _load_hip_mobility(self) -> dict:
        """Load the structured hip mobility targets library (optional).

        Used by _build_cool_down to render PAIL/RAIL prescriptions with proper
        setup, intent, cues, and effort percentages by client level.

        Returns an empty dict (not error) if the file is missing · the system
        falls back to the previous hard-coded dose strings.
        """
        path = self.lib_path / "hip_mobility_targets.json"
        if not path.exists():
            return {}
        try:
            return json.loads(path.read_text())
        except Exception:
            return {}

    def _load_unified_db(self) -> dict:
        # Support both the new repo name and the legacy name
        for fname in ("exercise_database.json", "ims_unified_database_v2.json"):
            path = self.lib_path / fname
            if path.exists():
                return json.loads(path.read_text())
        raise FileNotFoundError(
            f"Could not find exercise database in {self.lib_path}. "
            f"Expected 'exercise_database.json' or 'ims_unified_database_v2.json'."
        )

    def _load_markers(self) -> dict:
        # Support both locations · generator folder + libraries folder
        candidates = [
            Path(__file__).resolve().parent / "strength_markers.json",
            self.lib_path / "strength_markers.json",
            self.lib_path / "ims_strength_markers_v1.json",
        ]
        for path in candidates:
            if path.exists():
                return json.loads(path.read_text())
        raise FileNotFoundError(
            f"Could not find strength_markers.json. Checked: {[str(p) for p in candidates]}"
        )

    # ------------------------------------------------------
    # MAIN ENTRY POINT
    # ------------------------------------------------------

    def build_program(self, assessment: Assessment, block_number: int = 1) -> Program:
        """Build a 4-week program for one client (IMS Block 1 structure)."""
        # ── Coach OS integration: enrich assessment with derived data ──
        # 1. Recovery factor from sleep/stress/HR
        self._current_recovery = self._recovery_factor(assessment)

        # 0. Objective measurement layer · builds advisory routes, weak-link
        #    emphasis and side bias. Everything it produces is optional and
        #    additive; with no data every structure below stays empty and the
        #    rest of the build proceeds exactly as it did before v2.0.
        self._prepare_objective_layer(assessment)

        # 0b. Red flags · the coach's free-text hard contraindications. This
        #     field was collected and read by nothing. It now feeds the same
        #     constraint list the picker already respects.
        for c in self._red_flag_constraints(assessment):
            if c not in assessment.constraints:
                assessment.constraints.append(c)

        # 2. Merge posture-driven + desk-driven FRA priorities (additive)
        posture_pris = self._posture_driven_priorities(assessment)
        desk_pris = self._desk_driven_priorities(assessment)
        existing_descs = {p.description for p in assessment.fra_priorities}
        for desc in posture_pris + desk_pris:
            if desc not in existing_descs:
                try:
                    assessment.fra_priorities.append(parse_fra_priority(desc))
                    existing_descs.add(desc)
                except Exception:
                    pass

        # 3. Merge pain-graded constraints (additive)
        pain_constraints = self._pain_graded_constraints(assessment)
        for c in pain_constraints:
            if c not in assessment.constraints:
                assessment.constraints.append(c)

        # Step 1 · assign priorities to training days
        day_assignments = self._assign_priorities(assessment)

        # Step 2 · build each week (4-week block)
        weeks = []
        for wk_num in range(1, 5):
            week = self._build_week(assessment, day_assignments, wk_num)
            weeks.append(week)

        return Program(
            client_name=assessment.name,
            block_number=block_number,
            weeks=weeks,
            assessment=assessment,
            objective=self._objective_summary(assessment),
            progression=self._progression_summary(),
        )

    # ------------------------------------------------------
    # OBJECTIVE MEASUREMENT LAYER (v2.0)
    # ------------------------------------------------------

    def _reset_objective_state(self):
        self._objective = None
        self._force_signals = []
        self._weak_joints = []
        self._advisory_unload = set()
        self._control_first = set()
        self._loaded_end_range = set()
        self._side_biases = {}
        self._progression = None
        self._objective_events = []

    def _prepare_objective_layer(self, assessment: Assessment):
        """Resolve everything the objective data implies, once, up front.

        Order matters ·
          1. parse (tolerant · never raises into generation)
          2. build ROM × force signals
          3. apply precedence · hard contraindications suppress any signal
             that would route work toward a blocked joint
          4. rank weakest links, resolve side bias
          5. resolve the progression profile (Phase 2 · needs no force data)
        """
        self._reset_objective_state()
        try:
            self._cfg = load_thresholds()
        except Exception:
            self._cfg = {}

        # Phase 2 runs for every client, with or without instrumentation.
        try:
            from progression_profile import derive_profile
            self._progression = derive_profile(
                assessment, recovery=getattr(self, "_current_recovery", 1.0),
                cfg=self._cfg)
            assessment.progression_mode = self._progression.progression_mode
        except Exception:
            self._progression = None

        objective = getattr(assessment, "objective_measures", None)
        if objective is not None and not hasattr(objective, "has_data"):
            # A raw dict came in (direct library use, not through app.py).
            try:
                from objective_measures import parse_objective_measures
                objective = parse_objective_measures(objective, self._cfg)
                assessment.objective_measures = objective
            except Exception:
                objective = None
                assessment.objective_measures = None
        if objective is None or not objective.has_data():
            return

        self._objective = objective

        try:
            from rom_force_routing import (
                apply_precedence, build_signals, joints_for_loaded_end_range,
                joints_needing_control_first, joints_to_unload,
            )
            signals = build_signals(objective, self._cfg)
            signals = apply_precedence(signals, self._hard_contraindicated_joints(assessment))
            self._force_signals = signals
            self._advisory_unload = joints_to_unload(signals)
            self._control_first = joints_needing_control_first(signals)
            self._loaded_end_range = joints_for_loaded_end_range(signals)
        except Exception:
            self._force_signals = []

        try:
            from weakest_link import weak_joints
            self._weak_joints = weak_joints(objective, self._cfg)
        except Exception:
            self._weak_joints = []

        try:
            from side_dose import resolve as resolve_side
            self._side_biases = resolve_side(assessment, objective, self._cfg)
        except Exception:
            self._side_biases = {}

    def _hard_contraindicated_joints(self, assessment: Assessment) -> set:
        """Joints that carry a HARD block · a measured signal may never
        route work toward these, no matter what the numbers say.

        Three sources, all of them existing safety inputs ·
          * concern checkboxes (bad_knee, bad_shoulder, …)
          * constraints_rich rows whose status is an active problem
          * red_flags free text
        """
        blocked = set()
        for c in (assessment.concerns or []):
            j = concern_to_joint(c)
            if j:
                blocked.add(j)

        active_statuses = {"active_flare_up", "post_surgery", "avoid_loading"}
        for cr in (assessment.constraints_rich or []):
            if not isinstance(cr, dict):
                continue
            if str(cr.get("status") or "").lower() in active_statuses:
                j = concern_to_joint(cr.get("key")) or concern_to_joint(cr.get("display_name"))
                if j:
                    blocked.add(j)

        text = str(getattr(assessment, "red_flags", "") or "").lower()
        if text:
            for word, joint in CONCERN_TO_JOINT.items():
                if word.replace("_", " ") in text or word in text:
                    blocked.add(joint)
        return blocked

    def _red_flag_constraints(self, assessment: Assessment) -> list:
        """Turn the coach's free-text red flags into picker constraints.

        This field used to be collected and read by nothing. Free text cannot
        be parsed reliably, so the rule is deliberately blunt and one-way: a
        joint named in red_flags gets the same treatment as a checked concern
        box. It can only ever remove exercises, never add them.
        """
        text = str(getattr(assessment, "red_flags", "") or "").strip().lower()
        if not text:
            return []
        out = []
        for word, joint in CONCERN_TO_JOINT.items():
            if word.replace("_", " ") in text or word in text:
                flag = f"bad_{joint}" if joint not in ("lumbar", "cervical") else (
                    "lower_back" if joint == "lumbar" else "neck")
                if flag not in out:
                    out.append(flag)
        if out:
            # Concerns is what the picker actually reads for joint-level vetoes.
            for flag in out:
                if flag not in assessment.concerns:
                    assessment.concerns.append(flag)
        return []

    def _objective_summary(self, assessment: Assessment) -> dict:
        """Everything the objective layer decided · for the coach plan."""
        if self._objective is None:
            return {}
        try:
            from force_load import staleness_note
            from rom_force_routing import manual_therapy_flags
            from weakest_link import asymmetries, emphasis_notes, rank_joints
        except Exception:
            return {}

        cfg = getattr(self, "_cfg", {}) or {}
        summary = {
            **version_stamp(cfg),
            "current_date": self._objective.current_date,
            "previous_date": self._objective.previous_date,
            "bodyweight_lb": self._objective.current.bodyweight_lb,
            "measure_table": self._objective.measure_table(),
            "sources": sorted({m.get("source", "manual")
                               for m in self._objective.measure_table()}),
            "deltas": self._objective.deltas(cfg),
            "signals": [s.to_dict() for s in self._force_signals],
            "suppressed_signals": [s.to_dict() for s in self._force_signals
                                   if not s.active],
            "manual_therapy_flags": manual_therapy_flags(self._force_signals),
            "weakest_links": [s.to_dict() for s in rank_joints(self._objective, cfg)],
            "emphasis": emphasis_notes(self._objective, cfg),
            "asymmetries": asymmetries(self._objective, cfg),
            "side_biases": {j: b.to_dict() for j, b in (self._side_biases or {}).items()},
            "parse_warnings": list(self._objective.warnings),
            "events": list(self._objective_events),
        }
        note = staleness_note(self._objective, cfg)
        if note:
            summary["staleness_note"] = note
        return summary

    def _progression_summary(self) -> dict:
        if self._progression is None:
            return {}
        d = self._progression.to_dict()
        d["week_templates"] = {
            t: self._progression.week_templates(t)
            for t in ("compound", "accessory", "corrective")
        }
        return d

    def _note_objective_event(self, text: str):
        if text and text not in self._objective_events:
            self._objective_events.append(text)

    # ------------------------------------------------------
    # STEP 1 · PRIORITY ROTATION
    # ------------------------------------------------------

    # ── COACH OS INTEGRATION · Recovery & Posture Intelligence ──

    def _recovery_factor(self, assessment: Assessment) -> float:
        """Compute a 0.7–1.0 volume multiplier from sleep, stress, resting HR.
        
        1.0 = fully recovered, program at normal volume.
        <0.85 = reduced volume (fewer reps in W1).
        <0.75 = conservative start (fewer sets AND reps).
        """
        factor = 1.0
        sq = (assessment.sleep_quality or "").lower()
        if sq == "poor":
            factor -= 0.15
        elif sq == "fair":
            factor -= 0.05

        hrs = assessment.sleep_hours or ""
        try:
            if float(hrs) < 6:
                factor -= 0.1
            elif float(hrs) < 7:
                factor -= 0.05
        except (ValueError, TypeError):
            pass

        sl = (assessment.stress_level or "").lower()
        if sl == "very_high" or sl == "very high":
            factor -= 0.15
        elif sl == "high":
            factor -= 0.1
        elif sl == "moderate":
            factor -= 0.03

        rhr = assessment.resting_hr or ""
        try:
            if int(rhr) > 85:
                factor -= 0.1
            elif int(rhr) > 75:
                factor -= 0.05
        except (ValueError, TypeError):
            pass

        return max(factor, 0.7)

    def _posture_driven_priorities(self, assessment: Assessment) -> list[str]:
        """Auto-derive FRA-style priorities from posture findings.
        
        These are ADDED to the coach's explicit FRA priorities, not replacing them.
        Posture findings → corrective focus:
          forward head → Cervical Extension + Cervical Rotation
          rounded shoulders → Scapular Depression + Shoulder ER
          excessive kyphosis → Thoracic Extension
          flat lumbar / posterior tilt → Hip Extension (hinge patterning)
          anterior tilt → Hip Flexion + Glute activation
          flat feet → Ankle Dorsiflexion
        """
        posture = assessment.posture or {}
        additions = []

        head = (posture.get("head_position") or "").lower()
        if "forward" in head:
            additions.append("Cervical Extension + Rotation")

        shoulder = (posture.get("shoulder_position") or "").lower()
        if "rounded" in shoulder:
            additions.append("Scapular Depression + Shoulder ER")

        thoracic = (posture.get("thoracic_curve") or "").lower()
        if "kyphosis" in thoracic:
            additions.append("Thoracic Extension")

        lumbar = (posture.get("lumbar_curve") or "").lower()
        pelvic = (posture.get("pelvic_tilt") or "").lower()
        if "posterior" in pelvic or "flat" in lumbar:
            additions.append("Hip Extension bilateral")
        elif "anterior" in pelvic:
            additions.append("Hip Flexion bilateral")

        foot = (posture.get("foot_arch") or "").lower()
        if "flat" in foot or "pronated" in foot:
            additions.append("Ankle Dorsiflexion L+R")

        return additions

    def _desk_driven_priorities(self, assessment: Assessment) -> list[str]:
        """If client sits > 6 hrs/day, auto-add hip flexor + thoracic priorities."""
        try:
            hrs = float(assessment.desk_hours or "0")
        except (ValueError, TypeError):
            return []
        if hrs >= 6:
            return ["Hip IR L+R", "Thoracic Extension"]
        return []

    def _pain_graded_constraints(self, assessment: Assessment) -> list[str]:
        """Convert pain_map severity into exercise picker constraints.
        
        Pain 7-10: AVOID that area entirely (hard constraint)
        Pain 4-6: CAUTION (add to concerns, picker filters but doesn't block)
        Pain 1-3: NOTE (informational, no restriction)
        """
        extra_constraints = []
        pm = assessment.pain_map or {}
        for area, data in pm.items():
            if not isinstance(data, dict):
                continue
            try:
                severity = int(data.get("severity") or "0")
            except (ValueError, TypeError):
                continue
            if severity >= 7:
                # Hard constraint
                area_clean = area.replace("_", " ").lower()
                if "back" in area_clean or "lumbar" in area_clean:
                    extra_constraints.append("SI_joint_sensitivity")
                    extra_constraints.append("no_axial_loading")
                elif "shoulder" in area_clean:
                    extra_constraints.append("chronic_shoulder_impingement")
                elif "knee" in area_clean:
                    extra_constraints.append("post_surgery_knee")
        return list(set(extra_constraints))

    def _volume_adjusted_dose(self, base_dose: str, recovery: float) -> str:
        """Reduce dose if recovery factor is low.
        
        recovery >= 0.85: no change
        0.75-0.84: reduce reps by 2
        < 0.75: reduce sets by 1 AND reps by 2
        """
        if recovery >= 0.85:
            return base_dose
        import re
        # Parse "3x12" style
        m = re.match(r"(\d+)\s*[x×]\s*(\d+)(.*)", base_dose)
        if not m:
            return base_dose
        sets, reps, suffix = int(m.group(1)), int(m.group(2)), m.group(3)
        if recovery < 0.75:
            sets = max(sets - 1, 2)
            reps = max(reps - 2, 6)
        else:
            reps = max(reps - 2, 6)
        return f"{sets}x{reps}{suffix}"

    def _assign_priorities(self, assessment: Assessment) -> dict:
        """Distribute FRA priorities across training days.

        Uses explicit strength_days + cardio_days if set; otherwise falls back
        to training_frequency with legacy defaults.

        Rules ·
          Strength days alternate LB → UB → LB → UB (up to 4)
          If only 1 strength day · picks the region matching top FRA priority
          Cardio days go AFTER all strength days in the weekly order
        """
        lb_priorities = [p for p in assessment.fra_priorities if p.region == "lower_body"]
        ub_priorities = [p for p in assessment.fra_priorities if p.region == "upper_body"]
        spine_priorities = [p for p in assessment.fra_priorities if p.region == "spine"]

        # Read explicit counts · fall back to legacy training_frequency if unset
        sd = getattr(assessment, 'strength_days', 0) or 0
        cd = getattr(assessment, 'cardio_days', 0) or 0
        if sd == 0 and cd == 0:
            # Legacy path · use training_frequency
            freq = assessment.training_frequency
            if freq <= 2:
                sd, cd = 2, 0
            elif freq == 3:
                sd, cd = 2, 1
            else:
                sd, cd = 3, 1

        # Clamp sanity
        sd = max(1, min(sd, 4))
        cd = max(0, min(cd, 2))

        assignments = {}
        day_idx = 1

        # Strength days · alternate LB / UB starting with LB
        # Focus priorities:
        #   Day 1 (LB): LB priorities + a spine secondary if present
        #   Day 2 (UB): UB priorities + a spine secondary if present
        #   Day 3 (LB): LB priorities (rotated) + remaining UB secondary
        #   Day 4 (UB): UB priorities (rotated) + remaining LB secondary
        for i in range(sd):
            is_lb_day = (i % 2 == 0)
            day_type = "strength_lb" if is_lb_day else "strength_ub"

            if is_lb_day:
                # LB day · LB priorities first, then maybe 1 UB as integration
                focus = list(lb_priorities)
                if ub_priorities and len(focus) < 3:
                    focus.append(ub_priorities[-1])
                # Attach a spine priority if we have one and there's room
                if spine_priorities and len(focus) < 3:
                    focus.append(spine_priorities[0])
            else:
                # UB day
                focus = list(ub_priorities)
                if lb_priorities and len(focus) < 3:
                    focus.append(lb_priorities[-1])
                if spine_priorities and len(focus) < 3:
                    spine_pick = spine_priorities[-1] if len(spine_priorities) > 1 else spine_priorities[0]
                    focus.append(spine_pick)

            # If client has priorities on one side only and this is their "weak" day,
            # still give them at least one priority (the top overall)
            if not focus and assessment.fra_priorities:
                focus = [assessment.fra_priorities[0]]

            assignments[f"day_{day_idx}"] = {
                "day_type": day_type,
                "focus_priorities": focus[:3],
            }
            day_idx += 1

        # Cardio days · after all strength days
        for i in range(cd):
            assignments[f"day_{day_idx}"] = {
                "day_type": "cardio",
                "focus_priorities": [],
            }
            day_idx += 1

        return assignments

    def _worst_joint(self, assessment: Assessment) -> MobilityRating:
        """Return the mobility rating that's most restricted."""
        reds = [r for r in assessment.mobility_map if r.rating == "red"]
        if reds:
            return reds[0]
        yellows = [r for r in assessment.mobility_map if r.rating == "yellow"]
        return yellows[0] if yellows else assessment.mobility_map[0]

    # ------------------------------------------------------
    # STEP 2 · BUILD A WEEK
    # ------------------------------------------------------

    def _build_week(self, assessment: Assessment, day_assignments: dict, wk_num: int) -> Week:
        intent = self._week_intent(wk_num)
        sessions = []
        for day_key, config in day_assignments.items():
            day_num = int(day_key.split("_")[1])
            session = self._build_session(assessment, day_num, config, wk_num)
            sessions.append(session)

        return Week(
            week_number=wk_num,
            intent=intent,
            sessions=sessions,
            progression_notes=self._week_progression_notes(wk_num)
        )

    def _week_intent(self, wk_num: int) -> str:
        return {
            1: "Base Volume · 3 × 12 · build tolerance and groove the pattern",
            2: "Tempo Control · 3 × 10 with 3-sec eccentrics · own the descent",
            3: "Strength Build · 4 × 8 · push the working weight",
            4: "Performance Week · 4 × 6 · top-end strength · or retest 10RM",
        }.get(wk_num, "Unscheduled week")

    def _week_progression_notes(self, wk_num: int) -> list[str]:
        return {
            1: ["Base Volume week. Three sets of twelve at moderate load (RPE 7).",
                "Goal · move clean reps. Stop with 2-3 reps in reserve.",
                "If form breaks down before rep 12, drop the weight."],
            2: ["Tempo Control. Same sets, drop reps to ten, control the eccentric.",
                "Three seconds down on every rep · feel the muscle work.",
                "Weight stays similar to Week 1, RPE climbs to 7-8 from the tempo."],
            3: ["Strength Build. Four sets of eight at the prescribed working weight.",
                "Standard tempo returns. RPE 8 · the last rep should feel hard.",
                "This is the first real strength push of the block."],
            4: ["Performance Week. Four sets of six at top-end load · RPE 8-9.",
                "OR · retest 10RM if coach approves · directly compare to baseline.",
                "Either way, this is the load benchmark for next block."],
        }.get(wk_num, [])

    # ------------------------------------------------------
    # STEP 3 · BUILD A SESSION
    # ------------------------------------------------------

    def _build_session(self, assessment: Assessment, day_num: int,
                       config: dict, wk_num: int) -> Session:
        day_type = config["day_type"]
        priorities = config["focus_priorities"]

        if day_type == "cardio":
            return self._build_cardio_session(day_num, wk_num, assessment=assessment)

        if day_type == "integration":
            return self._build_integration_session(assessment, day_num, priorities, wk_num)

        # Strength days (LB or UB) · Jason's 8-block IMS v2 flow
        blocks = [
            self._build_cars_sequence(priorities, assessment),
            self._build_dynamic_warmup(assessment, priorities),
        ]
        # Joint Care · only included when client has flagged concerns
        joint_care = self._build_joint_care(assessment)
        if joint_care:
            blocks.append(joint_care)
        strength_a_block = self._build_strength_a(day_type, assessment, wk_num, day_num=day_num)
        # Cross-block dedup · Strength B should not repeat a name from Strength A
        # Pass the used names through.
        used_in_session = {ex.name for ex in strength_a_block.exercises}
        strength_b_block = self._build_strength_b(day_type, priorities, assessment, wk_num,
                                                    exclude_names=used_in_session,
                                                    day_num=day_num)
        # Strength C builds on the cumulative exclude set · A names + B names.
        # If the accessory picker returns 0 (no coach picks AND defaults exhaust),
        # the block is hidden. Otherwise it renders 1-3 exercises.
        used_in_session |= {ex.name for ex in strength_b_block.exercises}
        c_picks = self._pick_strength_c_accessories(
            assessment, day_type, day_num, wk_num,
            exclude_names=used_in_session,
        )
        blocks.extend([
            self._build_mobility_prep(priorities, assessment.constraints,
                                       wk_num=wk_num, assessment=assessment),
            strength_a_block,
            strength_b_block,
        ])
        if c_picks:
            # Apply tempo/RIR per week to each pick so the prescription has
            # week_prescriptions in line with everything else
            c_exercises = []
            for ex in c_picks:
                ex.progression_note = self._progression_note(wk_num, "accessory")
                ex = self._apply_week_progression(ex, wk_num, "accessory")
                ex = self._attach_week_prescriptions(ex, assessment, exercise_type="accessory")
                c_exercises.append(ex)
            blocks.append(Block(
                name="Strength C",
                exercises=c_exercises,
                duration_note="Accessory volume · day-specific picks",
            ))
        blocks.extend([
            self._build_hiit_finisher(assessment, context="strength"),
            self._build_decompression_cooldown(priorities, assessment=assessment),
            self._build_coach_finisher(priorities),
        ])

        # Capsule Work (PAIL/RAIL) · end of session · all modes
        # Level 1 default for client safety · coach can advance per spec
        capsule_block = self._build_capsule_work_block(priorities, assessment)
        if capsule_block is not None:
            blocks.append(capsule_block)

        focus_labels = [p.description for p in priorities]
        focus_text = ("Lower Body Strength" if day_type == "strength_lb"
                      else "Upper Body Strength")
        if focus_labels:
            focus_text += " + " + " / ".join(focus_labels[:2]) + " Focus"

        return Session(
            day_number=day_num,
            day_type=day_type,
            focus=focus_text,
            blocks=blocks
        )

    # ------------------------------------------------------
    # BLOCK BUILDERS · CARs SEQUENCE (pre-session)
    # ------------------------------------------------------

    def _build_cars_sequence(self, priorities: list[FRAPriority], assessment: Assessment) -> Block:
        """Jason's CARs opener · joint-by-joint controlled articular rotations.

        Sequence order (fixed) · neck → scap → shoulder → T-spine → cat-cow →
        thoracic twist → supine hip capsule CARs.

        Intensity ramps based on:
          - Client tier (new client starts 20-40%; advanced works up to 80-90%)
          - Day's hardest work (heavy LB day = higher hip CARs intensity)

        Reps scale too · 2-3 for new clients, 3-5 for advanced.
        """
        # Determine intensity tier
        tier = self._infer_client_tier(assessment)
        intensity = self._cars_intensity_for_day(tier, priorities)

        # Reps scale with tier
        reps = "2-3 reps/direction" if tier == "new" else "3-5 reps/direction"

        sequence = [
            Exercise(
                name="Neck CARs",
                library="cars",
                dose=reps,
                rationale=f"Start light · {intensity} effort",
            ),
            Exercise(
                name="Scapular CARs",
                library="cars",
                dose=reps,
                rationale=f"{intensity} effort",
            ),
            Exercise(
                name="Shoulder CARs (both arms)",
                library="cars",
                dose=reps,
                rationale=f"{intensity} effort",
            ),
            Exercise(
                name="T-Spine CARs (quadruped rotation)",
                library="cars",
                dose=reps,
                rationale=f"{intensity} effort",
            ),
            Exercise(
                name="Cat / Cow",
                library="cars",
                dose="5-8 reps",
                rationale="Spinal segmentation",
            ),
            Exercise(
                name="Thoracic Twist (open-book)",
                library="cars",
                dose="5 reps/side",
                rationale="T-spine rotation",
            ),
            Exercise(
                name="Supine Hip Capsule CARs",
                library="cars",
                dose=reps,
                rationale=f"{intensity} effort · ramps to match today's work",
            ),
        ]

        return Block(
            name="CARs Sequence",
            exercises=sequence,
            duration_note=f"Always first · {intensity} effort today",
        )

    def _infer_client_tier(self, assessment: Assessment) -> str:
        """Infer client tier (new / intermediate / advanced) from the assessment.

        Heuristic · if MORE THAN 3 mobility joints are rated 'red', the client
        has significant restrictions and is treated as new. If no reds AND some
        greens, they're advanced. Otherwise intermediate.
        """
        reds = sum(1 for r in assessment.mobility_map if r.rating == "red")
        greens = sum(1 for r in assessment.mobility_map if r.rating == "green")
        total = len(assessment.mobility_map) or 1

        if reds / total > 0.4:
            return "new"
        if greens / total > 0.4 and reds == 0:
            return "advanced"
        return "intermediate"

    def _cars_intensity_for_day(self, tier: str, priorities: list[FRAPriority]) -> str:
        """Pick the effort % for CARs today.

        Rule (hybrid) ·
          tier sets the CEILING · new = 20-40%, intermediate = 40-60%, advanced = 70-90%
          day's hardest work sets WITHIN that range
        """
        # For now we treat every strength day as "medium-hard" since we always pair
        # LB + UB compounds. Cardio day would use lower intensity but cardio has
        # its own warmup.
        if tier == "new":
            return "20-40%"
        if tier == "advanced":
            return "70-90%"
        return "40-60%"

    # ------------------------------------------------------
    # BLOCK BUILDERS · DYNAMIC WARMUP
    # ------------------------------------------------------

    def _build_dynamic_warmup(self, assessment: Assessment, priorities: list[FRAPriority]) -> Block:
        """Jason's dynamic warmup · meets the client where they are.

        Dose is a RANGE · 5-15 min based on how the client moves that day.
        Core elements · hip flexor rockers → groin rockers → World's Greatest Stretch →
        in-place or walking dynamic. Athletes can add heavier dynamic.
        """
        tier = self._infer_client_tier(assessment)

        flow = [
            Exercise(
                name="Hip Flexor Rockers",
                library="external_training",
                dose="8-10 reps/side",
                rationale="Open front of hip",
            ),
            Exercise(
                name="Groin Rockers",
                library="external_training",
                dose="8-10 reps",
                rationale="Adductor + inner hip prep",
            ),
            Exercise(
                name="World's Greatest Stretch",
                library="external_training",
                dose="5 reps/side",
                rationale="Full-body movement prep",
            ),
        ]

        # Athlete / advanced client · add a dynamic gait element
        if tier == "advanced":
            flow.append(Exercise(
                name="Dynamic Walking Flow (high knees, skips, cariocas)",
                library="external_training",
                dose="10 yards × 2-3 rounds",
                rationale="Athlete-tier warmup",
            ))
        else:
            flow.append(Exercise(
                name="In-Place Dynamic Flow (high knees, butt kicks, lateral shuffle)",
                library="external_training",
                dose="20 sec × 3",
                rationale="Movement prep",
            ))

        return Block(
            name="Dynamic Warmup",
            exercises=flow,
            duration_note="5-15 min · match the client's day",
        )

    # ------------------------------------------------------
    # BLOCK BUILDERS · COACH FINISHER (table work)
    # ------------------------------------------------------

    def _build_coach_finisher(self, priorities: list[FRAPriority]) -> Block:
        """Jason's signature end-of-session table work · 5 min passive mobility.

        Standard flow · supine foot traction → hip ER/IR rotation (add PAIL/RAIL
        if tight side) → patella glides → knee rotations → shoulder ER/IR →
        flip prone → Hypervolt lumbar→cervical 2 min each side → optional calves.

        This is coach-delivered and always present on strength days.
        """
        # Pick the tight-side hip call-out (if priorities indicate)
        hip_side_note = ""
        for p in priorities:
            d = p.description.lower()
            if "hip" in d and ("ir" in d or "er" in d):
                hip_side_note = f" · emphasize {p.description}"
                break

        flow = [
            Exercise(
                name="Supine Foot Traction Pull",
                library="manual",
                dose="30-60 sec",
                rationale="Open the posterior chain",
            ),
            Exercise(
                name="Hip Rotation Mobilization (ER / IR)",
                library="manual",
                dose="1-2 min/side",
                rationale=f"Rotate into tight side{hip_side_note} · add PAIL/RAIL if severe",
            ),
            Exercise(
                name="Patella Glides",
                library="manual",
                dose="30 sec/side",
                rationale="Kneecap mobility",
            ),
            Exercise(
                name="Supine Knee Rotations",
                library="manual",
                dose="30 sec/side",
                rationale="Tibio-femoral rotation",
            ),
            Exercise(
                name="Supine Shoulder ER/IR Passive Range",
                library="manual",
                dose="30-45 sec/side",
                rationale="Capsule work",
            ),
            Exercise(
                name="Prone Hypervolt · Lumbar → Cervical",
                library="manual",
                dose="2 min/side",
                rationale="Soft-tissue flush along the spine",
            ),
            Exercise(
                name="Calves (optional)",
                library="manual",
                dose="30-60 sec/side if tight",
                rationale="Only if calves feel tight today",
            ),
        ]

        return Block(
            name="Coach Finisher",
            exercises=flow,
            duration_note="~5 min table work · coach-delivered",
        )

    # ------------------------------------------------------
    # BLOCK BUILDERS · HIIT FINISHER (optional · tier-matched)
    # ------------------------------------------------------

    def _build_hiit_finisher(self, assessment, context: str = "strength", week_num: int = None) -> Block:
        """Optional HIIT finisher · 3-5 min · tier-matched to client.

        Three tiers ·
          T1 Joint-Friendly  · sled, carries, bike sprints, battle rope (no impact)
          T2 Mid             · KB swings, jump rope, step-ups, med ball slams
          T3 Plyo/Athletic   · box jumps, broad jumps, burpees, skater bounds

        Tier selection · inferred from client_tier (new=T1, intermediate=T2, advanced=T3)
        or from constraints (post-surgery / chronic-anything → T1 always).
        """
        # Pick tier
        if assessment is None:
            # Cardio day context · default to T2, coach can substitute
            tier = "mid"
            rationale_prefix = "Cardio day finisher"
        else:
            client_tier = self._infer_client_tier(assessment)
            tier = self._hiit_tier_for_client(assessment, client_tier)
            rationale_prefix = f"{client_tier.title()} client"

        # Tier libraries
        TIER_1_JOINT_FRIENDLY = [
            ("Sled Push", "10 yd × 4-5 rounds · 45 sec rest", "No impact, all-hip drive"),
            ("Sled Drag (backward)", "10 yd × 4 rounds · 45 sec rest", "Quad and knee friendly"),
            ("Farmer Carry Intervals", "30 sec heavy carry / 30 sec rest × 4", "Grip + core + posture"),
            ("Bike Sprints (Assault / Air Bike)", "20 sec sprint / 40 sec easy × 6", "All-out, zero impact"),
            ("Rower Sprints", "250m × 4 rounds · 60 sec rest", "Full-body pull pattern"),
            ("Battle Rope Slams", "30 sec on / 30 sec off × 5", "Standing or half-kneeling · no jumping"),
        ]
        TIER_2_MID = [
            ("Jump Rope Intervals", "30 sec on / 30 sec off × 5", "Low-grade plyo · stiffness drill"),
            ("Kettlebell Swings", "20 sec on / 40 sec off × 6", "Hinge ballistic · hip power"),
            ("Step-Ups with Knee Drive", "30 sec/side × 3 rounds", "Unilateral · hip extension"),
            ("Med Ball Slams", "10 reps × 4 rounds", "Full-body rotational power"),
            ("Lateral Bounds (controlled)", "5/side × 4 rounds · focus on stick", "Single-leg power, controlled"),
            ("Alternating Reverse Lunges", "45 sec × 3 rounds", "Split-stance conditioning"),
        ]
        TIER_3_PLYO = [
            ("Box Jumps", "5 reps × 4 rounds · full rest", "Max concentric, step down"),
            ("Broad Jumps + Stick", "5 reps × 4 rounds", "Horizontal power"),
            ("Depth Jumps", "5 reps × 3 rounds · 90 sec rest", "Reactive strength · advanced only"),
            ("Lateral Hops (continuous)", "30 sec on / 30 sec off × 5", "Ankle stiffness + frontal plane"),
            ("Burpees", "30 sec on / 30 sec off × 5", "Full-body conditioning classic"),
            ("Skater Bounds", "30 sec × 4 rounds", "Lateral power · single-leg stick"),
            ("Plyo Push-Ups", "5 reps × 4 rounds", "Upper-body power · advanced"),
        ]

        # Build block · offer 2-3 options, coach/client picks
        if tier == "joint_friendly":
            pool = TIER_1_JOINT_FRIENDLY
            tier_label = "Joint-Friendly"
        elif tier == "plyo":
            pool = TIER_3_PLYO
            tier_label = "Plyo / Athletic"
        else:
            pool = TIER_2_MID
            tier_label = "Mid-Intensity"

        # Filter pool against client concerns · veto knee-aggressive options
        # for clients with knee concerns (and similar for other joints).
        if assessment is not None:
            pool = self._filter_hiit_pool_by_concerns(pool, assessment)

        # Check interval clearance · if blocked, abandon the HIIT pool entirely
        # and return a Conditioning Reset block instead. No sprints, no jumps,
        # no intervals · just safe Z2-style movement and core work.
        clearance = None
        if assessment is not None:
            try:
                from cardio_rules import normalize_cardio_profile, determine_interval_clearance
                normalized = normalize_cardio_profile(
                    getattr(assessment, "cardio_profile", None),
                    concerns=getattr(assessment, "concerns", None),
                    constraints_rich=getattr(assessment, "constraints_rich", None),
                )
                clearance = determine_interval_clearance(normalized)
            except Exception:
                clearance = None

        if clearance == "blocked":
            # Build a Conditioning Reset block · no HIIT language anywhere
            reset_pool = [
                ("Zone 2 Bike (or approved machine)",
                 "5-8 min easy aerobic · RPE 4 · nasal breathing",
                 "Aerobic flush · no intensity"),
                ("Farmer Carry (light, controlled)",
                 "2 rounds × 30 sec heavy carry / 30 sec rest",
                 "Posture + grip · stop if symptoms increase"),
                ("Backward Sled Drag (if tolerated)",
                 "10 yd × 3 rounds · easy pace",
                 "Quad-friendly · no joint stress"),
                ("Dead Bug Variations",
                 "30 sec × 2 rounds",
                 "Anti-extension core · no impact"),
                ("Diaphragmatic Breathing Reset",
                 "2 min · supine, knees bent, slow nasal breaths",
                 "Down-regulate the nervous system"),
            ]
            # Filter once more to honor any non-cardio veto
            if assessment is not None:
                from cardio_rules import filter_finishers_by_cardio_limitations
                try:
                    normalized = normalize_cardio_profile(
                        getattr(assessment, "cardio_profile", None),
                        concerns=getattr(assessment, "concerns", None),
                        constraints_rich=getattr(assessment, "constraints_rich", None),
                    )
                    reset_pool = filter_finishers_by_cardio_limitations(reset_pool, normalized)
                except Exception:
                    pass
            if not reset_pool:
                reset_pool = [
                    ("Diaphragmatic Breathing Reset",
                     "2 min · supine, knees bent, slow nasal breaths",
                     "Down-regulate the nervous system"),
                ]
            exercises = [
                Exercise(name=name, library="external_training", dose=dose,
                          rationale=f"Conditioning reset · {note}")
                for name, dose, note in reset_pool[:3]
            ]
            return Block(
                name="Conditioning Reset (optional · low-intensity)",
                exercises=exercises,
                duration_note="3-5 min · NO sprints / NO intervals / Z2 only this block",
            )

        if not pool:
            # Last-resort fallback · everything in T1 is broadly safe except sled
            pool = [
                ("Farmer Carry Intervals", "30 sec heavy carry / 30 sec rest × 4", "Grip + core + posture"),
                ("Bike Sprints (Assault / Air Bike)", "20 sec sprint / 40 sec easy × 6", "All-out, zero impact"),
                ("Battle Rope Slams", "30 sec on / 30 sec off × 5", "Standing or half-kneeling · no jumping"),
                ("Rower Sprints", "250m × 4 rounds · 60 sec rest", "Full-body pull pattern"),
                ("Dead Bug Variations", "30 sec × 3 rounds", "Anti-extension core · no joint stress"),
            ]

        # Deterministic but varied · pick 3 options based on week number.
        # NOTE · this used hash(context), which Python salts per process, so
        # the same assessment produced different finishers on every run. That
        # made the generator irreproducible and made a byte-identical
        # regression test impossible to write. crc32 is stable across
        # processes and machines.
        seed = (week_num or 1) * 17 + (zlib.crc32(str(context).encode("utf-8")) % 100)
        rng = random.Random(seed)
        picks = rng.sample(pool, min(3, len(pool)))

        exercises = [
            Exercise(
                name=name,
                library="external_training",
                dose=dose,
                rationale=f"{rationale_prefix} · {note}",
            )
            for name, dose, note in picks
        ]

        return Block(
            name=f"HIIT Finisher (optional · {tier_label})",
            exercises=exercises,
            duration_note="3-5 min · skip if table-work priority that day",
        )

    def _filter_hiit_pool_by_concerns(self, pool, assessment):
        """Remove HIIT options that load a flagged joint heavily.

        For each pool entry, check the exercise NAME against a veto list
        keyed by concern. Pool entries are tuples · (name, dose, note).
        """
        concerns = getattr(assessment, "concerns", None) or []
        if not concerns:
            return list(pool)

        # Veto lists · case-insensitive substring match on the exercise name
        VETOES = {
            "knee": [  # bad_knee, knee, knee_pain, meniscus, post_surgery_knee
                "lateral bound", "alternating reverse lunge", "reverse lunge",
                "jump rope", "kettlebell swing", "deep squat", "box jump",
                "broad jump", "depth jump", "lateral hop", "burpee", "skater",
                "step-up", "step up",  # knee drive impacts patellofemoral
                "plyo push-up",
            ],
            "shoulder": [
                "battle rope", "med ball slam", "plyo push-up",
            ],
            "lumbar": [  # lower_back, back, lumbar
                "kettlebell swing", "med ball slam", "broad jump", "burpee",
                "deep squat", "plyo push-up",
            ],
            "back": [
                "kettlebell swing", "med ball slam", "broad jump", "burpee",
                "deep squat", "plyo push-up",
            ],
            "lower_back": [
                "kettlebell swing", "med ball slam", "broad jump", "burpee",
                "deep squat", "plyo push-up",
            ],
            "hip": [
                "kettlebell swing", "lateral bound", "skater", "burpee",
                "broad jump", "depth jump",
            ],
            "ankle": [
                "lateral bound", "lateral hop", "skater", "jump rope",
                "box jump", "depth jump", "broad jump", "plyo push-up",
            ],
            "wrist": [
                "battle rope", "med ball slam", "burpee", "plyo push-up",
                "farmer carry",
            ],
            "elbow": [
                "battle rope", "med ball slam", "burpee", "plyo push-up",
            ],
            "neck": [
                "med ball slam", "burpee", "plyo push-up",
            ],
            "cervical": [
                "med ball slam", "burpee", "plyo push-up",
            ],
        }

        # Normalize concern strings · drop bad_, etc.
        concern_keys = []
        for c in concerns:
            key = str(c).lower().replace(" ", "_").replace("-", "_")
            key = key.replace("bad_", "").replace("_pain", "")
            concern_keys.append(key)
            # Also add the canonical joint name for redundancy
            joint = self._concern_to_joint(key)
            if joint:
                concern_keys.append(joint)

        all_vetoes = set()
        for ck in concern_keys:
            all_vetoes.update(VETOES.get(ck, []))

        if not all_vetoes:
            # Check cardio profile limitations as last layer before returning unfiltered
            return self._filter_hiit_pool_by_cardio_profile(list(pool), assessment)

        filtered = []
        for entry in pool:
            name = entry[0].lower()
            if any(v in name for v in all_vetoes):
                continue
            filtered.append(entry)
        # Now apply cardio profile filters on top
        return self._filter_hiit_pool_by_cardio_profile(filtered, assessment)

    def _filter_hiit_pool_by_cardio_profile(self, pool, assessment):
        """Apply cardio_profile.limitations to HIIT pool · removes options
        that conflict with limitation flags. Delegates to cardio_rules.

        If filtering wipes the pool clean, fall back to the rules engine's
        safe replacement pool for the dominant limitation.
        """
        try:
            from cardio_rules import (
                normalize_cardio_profile,
                filter_finishers_by_cardio_limitations,
                replacement_finisher_pool,
            )
        except Exception:
            return list(pool)

        profile = getattr(assessment, "cardio_profile", None)
        concerns = getattr(assessment, "concerns", None) if assessment else None
        constraints_rich = getattr(assessment, "constraints_rich", None) if assessment else None
        normalized = normalize_cardio_profile(profile, concerns=concerns,
                                               constraints_rich=constraints_rich)

        filtered = filter_finishers_by_cardio_limitations(list(pool), normalized)
        if not filtered:
            replacement = replacement_finisher_pool(normalized)
            if replacement:
                return replacement
        return filtered

    def _hiit_tier_for_client(self, assessment, client_tier: str) -> str:
        """Pick HIIT tier from client profile.

        Constraint-driven downgrade rules ·
          Any post-surgery / chronic-anything / pregnancy → T1 joint-friendly always
          ANY checkbox concern (bad knee, bad shoulder, etc) → T1 joint-friendly
          Otherwise client_tier maps · new→T1, intermediate→T2, advanced→T3
        """
        constraints = [c.lower() for c in (assessment.constraints or [])]
        downgrade_flags = [
            "post_surgery", "chronic", "pregnancy", "si_joint", "knee", "hip", "shoulder",
            "lumbar", "disc", "back", "ankle", "wrist", "elbow", "neck", "cervical",
            "meniscus", "acl", "mcl", "labrum",
        ]
        for c in constraints:
            if any(f in c for f in downgrade_flags):
                return "joint_friendly"

        # Concerns checkbox ALSO triggers downgrade · same rationale
        concerns = getattr(assessment, "concerns", None) or []
        if concerns:
            return "joint_friendly"

        # Cardio profile limitations also trigger downgrade · joint-sensitive
        # flags or poor recovery / deconditioned mean we stay in joint-friendly tier.
        profile = getattr(assessment, "cardio_profile", None)
        if profile is not None:
            joint_limit_flags = {
                "knee_sensitive", "hip_sensitive", "low_back_sensitive",
                "shoulder_sensitive", "wrist_sensitive",
                "conditioning_beginner", "deconditioned",
                "high_stress_poor_recovery", "not_cleared_for_intervals",
            }
            if any(l in joint_limit_flags for l in profile.limitations):
                return "joint_friendly"

        if client_tier == "new":
            return "joint_friendly"
        if client_tier == "advanced":
            return "plyo"
        return "mid"

    # ------------------------------------------------------
    # BLOCK BUILDERS · DECOMPRESSION COOL DOWN (optional · self-led)
    # ------------------------------------------------------

    def _build_decompression_cooldown(self, priorities: list[FRAPriority],
                                       assessment: 'Assessment' = None) -> Block:
        """Optional self-led decompression flow · passive holds client can do alone.

        When the client has cardio limitations (knee_sensitive, etc), unsafe
        named picks (Deep Squat Hold) get substituted via cardio_rules.
        """
        # Build normalized profile once for substitution lookups
        normalized = None
        if assessment is not None:
            try:
                from cardio_rules import normalize_cardio_profile, safe_substitute_for_priority_pick
                normalized = normalize_cardio_profile(
                    getattr(assessment, "cardio_profile", None),
                    concerns=getattr(assessment, "concerns", None),
                    constraints_rich=getattr(assessment, "constraints_rich", None),
                )
            except Exception:
                normalized = None

        def _safe(name):
            if normalized is None:
                return name
            try:
                from cardio_rules import safe_substitute_for_priority_pick
                return safe_substitute_for_priority_pick(name, normalized)
            except Exception:
                return name

        flow = [
            Exercise(
                name=_safe("Supine 90/90 Decompression"),
                library="base_positions",
                dose="1-2 min",
                rationale="Neutral spine · hip flexor lengthen",
            ),
            Exercise(
                name=_safe("Child's Pose with Arm Reach"),
                library="base_positions",
                dose="1 min · breathe into lats",
                rationale="T-spine + lat decompression",
            ),
            Exercise(
                name=_safe("Supine Hip Capsule Lean (side-lying)"),
                library="base_positions",
                dose="45 sec/side",
                rationale="Passive hip IR / ER hold",
            ),
        ]

        # Priority-specific addition
        if priorities:
            primary = priorities[0]
            d = primary.description.lower()
            if "shoulder" in d:
                flow.append(Exercise(
                    name=_safe("Doorway Pec Stretch"),
                    library="base_positions",
                    dose="45 sec/side",
                    rationale=f"Opens front shoulder · supports {primary.description}",
                ))
            elif "thoracic" in d or "t-spine" in d:
                flow.append(Exercise(
                    name=_safe("Foam Roller Thoracic Extension"),
                    library="base_positions",
                    dose="5 slow breath cycles",
                    rationale="Passive T-spine decompression",
                ))
            elif "ankle" in d:
                # If knee-sensitive, substitute Deep Squat Hold for Heel-Elevated Deep Squat
                flow.append(Exercise(
                    name=_safe("Deep Squat Hold"),
                    library="base_positions",
                    dose="45 sec · hold railing if needed",
                    rationale="Ankle + hip decompression hold",
                ))

        return Block(
            name="Decompression Cool Down (optional)",
            exercises=flow,
            duration_note="2-4 min · self-led · skip if Coach Finisher replaces it",
        )

    # ------------------------------------------------------
    # BLOCK BUILDERS · PASSIVE STRETCH
    # ------------------------------------------------------

    def _build_passive_stretch(self, priorities: list[FRAPriority]) -> Block:
        if not priorities:
            return Block(name="Passive Stretch (2 min)",
                        exercises=[Exercise(name="General mobility breath work",
                                           library="manual",
                                           dose="2 min")])
        primary = priorities[0]
        # Pick a base position that targets this priority
        stretch_name = self._passive_stretch_for_priority(primary)
        return Block(
            name="Passive Stretch (2 min)",
            exercises=[Exercise(
                name=stretch_name,
                library="base_positions",
                dose="1 min/side" if "L+R" in primary.description or "bilateral" in primary.description.lower() else "2 min",
                rationale=f"Opens up {primary.description}"
            )]
        )

    def _passive_stretch_for_priority(self, priority: FRAPriority) -> str:
        """Map a priority to a matching base-position passive stretch."""
        joint = priority.joints[0].lower() if priority.joints else ""
        direction = priority.directions[0].lower() if priority.directions else ""

        mapping = {
            ("hip", "ir"): "90/90 Hip Hold",
            ("hip", "er"): "Pigeon Pose",
            ("hip", "flexion"): "Deep Squat Hold",
            ("hamstring", ""): "Hamstring Wall Stretch (Posterior Tilt Bias)",
            ("shoulder", "er"): "Child's Pose with Arm Reach",
            ("shoulder", "ir"): "Sleeper Stretch",
            ("ankle", "plantarflexion"): "Kneeling Ankle Stretch",
            ("ankle", "eversion"): "Tibialis Wall Stretch",
            ("thoracic", ""): "Prayer Stretch on Foam Roller",
            ("cervical", ""): "Supine Cervical Traction Hold",
            ("lumbar", ""): "Supine Knee-to-Chest",
        }
        key = (joint, direction)
        if key in mapping:
            return mapping[key]
        # Fallback
        for (j, _), val in mapping.items():
            if j == joint:
                return val
        return f"Targeted hold · {priority.description}"

    # ------------------------------------------------------
    # BLOCK BUILDER · JOINT CARE (red-flag joints, gentle drills)
    # ------------------------------------------------------

    def _build_joint_care(self, assessment: Assessment) -> Block | None:
        """Build a Joint Care block for any flagged client concerns.

        Returns None if there are no concerns to address (block won't appear).

        For each flagged joint, prescribes 2 gentle defaults ·
          - Passive CARs · low-load joint exploration
          - Light Flexibility Hold · pain-free range only
        Coach can swap or expand once they review the plan.

        Dose stays gentle and CONSTANT across all 4 weeks. No progression
        until the joint is cleared (yellow/green).
        """
        concerns = getattr(assessment, "concerns", None) or []
        if not concerns:
            return None

        # Map concern keywords to canonical joint names + drill labels
        # The drill labels are constructed to be unambiguous on the PDF
        # so the coach SEES exactly which joint was flagged
        joint_drills = []  # list of (joint_label, drills)

        for concern in concerns:
            concern_norm = str(concern).lower().replace(" ", "_").replace("-", "_")
            joint = self._concern_to_joint(concern_norm)
            if joint is None:
                continue

            joint_label = self._joint_display_name(joint)
            drills = [
                Exercise(
                    name=f"{joint_label} Passive CARs",
                    library="external_training",
                    dose="2-3 reps/direction · gentle · stop at any pinch",
                    rationale=f"Joint exploration without load · respects {joint_label.lower()} flag",
                ),
                Exercise(
                    name=f"{joint_label} Light Flexibility Hold",
                    library="external_training",
                    dose="30 sec · short of any restriction · NO pain",
                    rationale=f"Tissue prep · pain-free range only",
                ),
            ]
            joint_drills.append((joint_label, drills))

        if not joint_drills:
            return None

        # Flatten · Joint Care contains all drills across all flagged joints
        all_exercises = []
        for joint_label, drills in joint_drills:
            all_exercises.extend(drills)

        # Build the duration note that tells the coach what's happening
        flagged_joints = ", ".join(jl for jl, _ in joint_drills)
        return Block(
            name="Joint Care",
            exercises=all_exercises,
            duration_note=f"Working AROUND · {flagged_joints} · gentle · same dose all 4 weeks until cleared",
        )

    def _joint_display_name(self, canonical_joint: str) -> str:
        """Convert canonical joint key to a readable display label."""
        return {
            "knee": "Knee",
            "shoulder": "Shoulder",
            "lumbar": "Lower Back",
            "hip": "Hip",
            "cervical": "Neck",
            "wrist": "Wrist",
            "elbow": "Elbow",
            "ankle": "Ankle",
        }.get(canonical_joint, canonical_joint.title())

    # ------------------------------------------------------
    # BLOCK BUILDERS · MOBILITY PREP (RAILs-based)
    # ------------------------------------------------------

    def _build_mobility_prep(self, priorities: list[FRAPriority],
                             constraints: list[str],
                             wk_num: int = 1,
                             assessment: Assessment = None) -> Block:
        """RAILs-based mobility prep · constraint-aware progression.

        Dose progression rules ·
          - GREEN-rated joint (Optimal Control) · push hardest · time + rounds ramp aggressively
          - YELLOW-rated joint (Moderate Control) · moderate ramp
          - RED-rated joint (Limited Control) · gentle, conservative ramp
          - Unknown / no rating · default (yellow) progression

        Pattern per priority ·
          priority 1 · Lift-Off + Hover OR ERR
          priority 2 · Lift-Off only
          priority 3 · Lift-Off only
        """
        exercises = []
        for i, priority in enumerate(priorities[:3]):
            rating = self._rating_for_priority(priority, assessment)

            lift_off = self._pick_lift_off_for_priority(priority, constraints,
                                                        wk_num=wk_num, rating=rating)
            exercises.append(lift_off)

            if i == 0:
                slow_control = self._pick_slow_control_for_priority(priority, constraints,
                                                                     wk_num=wk_num, rating=rating)
                exercises.append(slow_control)

        return Block(
            name="Mobility Prep (RAILs-Based)",
            exercises=exercises,
            duration_note="Lift-Offs primary · Hover/ERR for slow-twitch control"
        )

    def _rating_for_priority(self, priority: FRAPriority,
                              assessment: Assessment) -> str:
        """Look up the worst rating in the mobility_map for joints/directions
        in this priority. Returns one of 'green' / 'yellow' / 'red'.

        If the assessment has no map or no match for this priority, default to
        'yellow' (moderate · conservative middle ground).
        """
        if assessment is None or not getattr(assessment, "mobility_map", None):
            return "yellow"

        priority_joints = set((j or "").lower() for j in (priority.joints or []))
        priority_dirs = set((d or "").lower() for d in (priority.directions or []))

        worst_score = 999  # green=2 yellow=1 red=0 · we want the smallest
        rating_map = {"green": 2, "yellow": 1, "red": 0,
                      "optimal": 2, "moderate": 1, "limited": 0}
        score_to_rating = {2: "green", 1: "yellow", 0: "red"}

        for entry in assessment.mobility_map:
            joint = (getattr(entry, "joint", "") or "").lower()
            direction = (getattr(entry, "direction", "") or "").lower()
            rating = (getattr(entry, "rating", "") or "").lower()

            joint_match = (not priority_joints or joint in priority_joints)
            dir_match = (not priority_dirs or direction in priority_dirs)
            if joint_match and dir_match and rating in rating_map:
                score = rating_map[rating]
                if score < worst_score:
                    worst_score = score

        if worst_score == 999:
            return "yellow"  # no match · safe default
        return score_to_rating[worst_score]

    def _mobility_dose(self, drill_type: str, wk_num: int, rating: str,
                        is_unilateral: bool = True) -> str:
        """Compute the per-week dose for a mobility drill · constraint-aware.

        drill_type · 'lift_off' | 'err' | 'hover'

        Progression model · TIME + ROUNDS + CONSTRAINT-MODULATED
          GREEN (Optimal)   · most aggressive · longer holds, more rounds
          YELLOW (Moderate) · moderate ramp · the default for most clients
          RED (Limited)     · steady · low rounds, modest hold ramp · safety first

        Returns dose string like "3×8/side · 15s hold" or "2×6/side · 10s hold".
        """
        side = "/side" if is_unilateral else ""

        # Per-week multipliers indexed by [rating][wk_num]
        # Each entry · (sets, reps, hold_seconds)
        TABLES = {
            "lift_off": {
                "green":  {1: (2, 6, 5),  2: (2, 8, 8),  3: (3, 6, 10), 4: (3, 8, 12)},
                "yellow": {1: (2, 6, 5),  2: (2, 6, 8),  3: (2, 8, 8),  4: (3, 6, 10)},
                "red":    {1: (2, 5, 3),  2: (2, 6, 5),  3: (2, 6, 5),  4: (2, 6, 5)},
            },
            "err": {
                # ERR is a slow eccentric ROM drill · lower reps, longer "slow" cue
                "green":  {1: (2, 4, 0), 2: (2, 5, 0), 3: (3, 4, 0), 4: (3, 5, 0)},
                "yellow": {1: (2, 3, 0), 2: (2, 4, 0), 3: (2, 5, 0), 4: (3, 4, 0)},
                "red":    {1: (2, 3, 0), 2: (2, 3, 0), 3: (2, 4, 0), 4: (2, 4, 0)},
            },
            "hover": {
                # Hover is a hold · time-dominant
                "green":  {1: (1, 5, 10), 2: (2, 4, 12), 3: (2, 5, 15), 4: (3, 4, 20)},
                "yellow": {1: (1, 5, 8),  2: (1, 5, 10), 3: (2, 4, 12), 4: (2, 5, 15)},
                "red":    {1: (1, 5, 5),  2: (1, 5, 8),  3: (2, 4, 8),  4: (2, 4, 10)},
            },
        }

        table = TABLES.get(drill_type, TABLES["lift_off"])
        rating_data = table.get(rating, table["yellow"])
        sets, reps, hold = rating_data.get(wk_num, rating_data[1])

        if drill_type == "err":
            return f"{sets}×{reps}{side} · slow"
        if drill_type == "hover":
            if hold > 0:
                return f"{sets}×{reps}{side} · {hold}s hold"
            return f"{sets}×{reps}{side}"
        # lift_off
        if hold > 0:
            return f"{sets}×{reps}{side} · {hold}s pause at end range"
        return f"{sets}×{reps}{side}"

    def _pick_lift_off_for_priority(self, priority: FRAPriority,
                                     constraints: list[str],
                                     wk_num: int = 1,
                                     rating: str = "yellow") -> Exercise:
        """Build a Lift-Off exercise · dose progresses by week + rating."""
        joint = priority.joints[0].title() if priority.joints else "General"
        direction = priority.directions[0] if priority.directions else ""
        direction_label = self._direction_label(direction)

        name = f"{joint} {direction_label} Lift-Offs".replace("  ", " ").strip()
        dose = self._mobility_dose("lift_off", wk_num, rating, is_unilateral=True)

        # Search DB for a matching PRLO; fall back to constructed name if none found
        matches = self._search_library(library="end_range", priority=priority,
                                        constraints=constraints, limit=5)
        for m in matches:
            mod = str(m.get("modality", "")).lower()
            if "prlo" in mod or "lift" in mod:
                return Exercise(
                    name=m.get("name", name),
                    library="end_range",
                    library_id=m.get("id"),
                    dose=dose,
                    rationale=f"Targets {priority.description} · {rating} rating · W{wk_num} ramp"
                )

        return Exercise(
            name=name,
            library="end_range",
            dose=dose,
            rationale=f"Lift-Off activation · {rating} rating · W{wk_num} dose"
        )

    def _pick_slow_control_for_priority(self, priority: FRAPriority,
                                         constraints: list[str],
                                         wk_num: int = 1,
                                         rating: str = "yellow") -> Exercise:
        """Pick a Hover OR ERR · dose progresses by week + rating."""
        joint = priority.joints[0].title() if priority.joints else "General"
        direction = priority.directions[0] if priority.directions else ""
        direction_label = self._direction_label(direction)

        is_rotation = direction.lower() in ["ir", "er"]

        if is_rotation:
            name = f"{joint} {direction_label} ERRs".replace("  ", " ").strip()
            dose = self._mobility_dose("err", wk_num, rating, is_unilateral=True)
            return Exercise(
                name=name,
                library="end_range",
                dose=dose,
                rationale=f"Rotational slow-twitch · {rating} rating · W{wk_num}"
            )
        else:
            name = f"{joint} Hovers".strip()
            dose = self._mobility_dose("hover", wk_num, rating, is_unilateral=True)
            return Exercise(
                name=name,
                library="end_range",
                dose=dose,
                rationale=f"Slow-twitch hold · {rating} rating · W{wk_num}"
            )

    def _direction_label(self, direction: str) -> str:
        """Human-friendly label for direction codes."""
        mapping = {
            "IR": "IR", "ER": "ER",
            "flexion": "Flexion", "extension": "Extension",
            "plantarflexion": "Plantarflexion", "dorsiflexion": "DF",
            "eversion": "Eversion", "inversion": "Inversion",
            "abduction": "Abduction", "adduction": "Adduction",
            "general": "", "restricted": ""
        }
        return mapping.get(direction, direction.title())

    # ------------------------------------------------------
    # BLOCK BUILDERS · STRENGTH A (compound lifts)
    # ------------------------------------------------------

    def _build_strength_a(self, day_type: str, assessment: Assessment, wk_num: int,
                            day_num: int = 1) -> Block:
        if day_type == "strength_lb":
            patterns = self._pick_lb_patterns(assessment.constraints, day_num=day_num,
                                                wk_num=wk_num,
                                                concerns=getattr(assessment, "concerns", None))
        else:
            patterns = ["push_horizontal", "pull_horizontal"]  # default UB

        exercises = []
        used_names = set()
        for pattern in patterns:
            ex = self._pick_strength_exercise(pattern, assessment.constraints,
                                               assessment=assessment,
                                               day_type=day_type,
                                               day_num=day_num,
                                               exclude_names=used_names)
            ex.dose = self._strength_dose(wk_num, exercise_type="compound",
                                          pattern=pattern, assessment=assessment)
            ex.progression_note = self._progression_note(wk_num, "compound")
            ex = self._apply_week_progression(ex, wk_num, "compound")
            ex = self._attach_week_prescriptions(ex, assessment, exercise_type="compound",
                                                pattern=pattern)
            exercises.append(ex)
            used_names.add(ex.name)

        return Block(
            name=f"Strength A",
            exercises=exercises,
            duration_note="Compound lifts · prioritize form over load"
        )

    def _find_matching_strength_test(self, exercise_name: str, assessment: Assessment):
        """Find a StrengthTest whose exercise_name matches this exercise.

        Matching is fuzzy · case-insensitive substring or token overlap.
        Returns None if no match (caller falls back to default dose strings).
        """
        tests = getattr(assessment, "strength_marker_tests", None) or []
        if not tests:
            return None

        target = (exercise_name or "").strip().lower()
        if not target:
            return None
        target_tokens = set(target.replace("(", " ").replace(")", " ").split())

        best = None
        best_score = 0
        for t in tests:
            test_name = (getattr(t, "exercise_name", None) or "").strip().lower()
            if not test_name:
                continue
            # Exact substring match · score 100
            if target == test_name or target in test_name or test_name in target:
                return t
            # Token overlap score
            test_tokens = set(test_name.replace("(", " ").replace(")", " ").split())
            overlap = len(target_tokens & test_tokens)
            if overlap > best_score and overlap >= 2:
                # Need at least 2 token overlap to count · avoids "Squat" matching
                # everything that has "squat" in it
                best_score = overlap
                best = t
        return best

    def _attach_week_prescriptions(self, ex: Exercise, assessment: Assessment,
                                   exercise_type: str = "compound",
                                   pattern: str = None) -> Exercise:
        """Attach 4-week prescriptions to an exercise. THE single load path.

        Order of precedence for where a load comes from ·

          1. A tested strength anchor (strength_anchor_resolver → strength_math).
             Unchanged, still first · a real tested lift beats an inferred one.
          2. A measured isometric value (force_load), used ONLY when step 1
             does not resolve. Prescribed as a percentage of that client's own
             measured force rather than a population rep-max formula.
          3. Nothing · the renderer falls back to RIR/RPE, as before.

        Sets and reps come from the client's ProgressionProfile in all three
        cases · force data sets the LOAD, the profile sets the VOLUME, and the
        two stay independent.

        Raises ImplausibleLoadError (deliberately · see force_load) if a
        computed load is not sensible for the exercise's equipment. A failed
        generation is better than a wrong number on a client's plan.
        """
        profile = getattr(self, "_progression", None)
        ladder = (profile.week_templates(exercise_type) if profile is not None
                  else None)

        entry = self._entry_or_shim(ex.name)
        self._attach_side_dose(ex, assessment, exercise_type, pattern, entry,
                               ladder)
        self._annotate_objective_selection(ex, entry)

        tests = getattr(assessment, "strength_marker_tests", None) or []
        test, method = None, None
        if tests:
            try:
                from strength_anchor_resolver import resolve_anchor_for_exercise
                test, method = resolve_anchor_for_exercise(ex.name, tests)
            except Exception:
                test, method = None, None

            # Record usage on the assessment-level tracker (lazy-init)
            if not hasattr(assessment, "_anchor_tracker") or assessment._anchor_tracker is None:
                try:
                    from strength_anchor_resolver import AnchorUsageTracker
                    assessment._anchor_tracker = AnchorUsageTracker(tests)
                except Exception:
                    assessment._anchor_tracker = None
            if test is not None and assessment._anchor_tracker is not None:
                assessment._anchor_tracker.record_match(test, ex.name, method)

        if test is None:
            # ── 2 · measured isometric force as the fallback anchor ──
            return self._attach_force_prescriptions(ex, assessment, entry,
                                                    ladder, exercise_type)

        try:
            from strength_math import generate_4_week_progression
            prog = generate_4_week_progression(test, ex.name, ladder=ladder)
        except Exception:
            return ex

        # Convert WeekPrescription dataclasses → plain dicts for JSON-serialization.
        # If the match was category-only (the anchor and the programmed exercise
        # share movement_category but not exercise family), the literal weight
        # almost certainly does not transfer · null it out so the renderer falls
        # back to RIR/RPE rather than slapping a wrong-family load on screen.
        # Reps and sets DO transfer · they're the rep scheme for the block.
        category_only = (method == "category")
        weeks_as_dicts = []
        for w in prog.get("weeks", []):
            wd = {
                "week": w.week,
                "sets": w.sets,
                "reps": w.reps,
                "weight": None if category_only else w.weight,
                "weight_unit": None if category_only else w.weight_unit,
                "weight_note": None if category_only else w.weight_note,
                "rpe": w.rpe,
                "intent_label": w.intent_label,
                "tempo_note": w.tempo_note,
                "fallback_text": w.fallback_text,
                "display_dose": w.display_dose(),
            }
            weeks_as_dicts.append(wd)

        ex.week_prescriptions = weeks_as_dicts
        # Tag exercise with how it matched · useful for appendix detail
        ex.anchor_match_method = method
        ex.anchor_source_name = getattr(test, "exercise_name", None)
        # Whether the renderer will actually show calculated loads · True only
        # when at least one week's weight came through.
        ex.anchor_rendered_with_loads = any(
            w.get("weight") is not None for w in weeks_as_dicts
        )
        if ex.anchor_rendered_with_loads:
            ex.load_source = "tested_anchor"
            ex.load_source_detail = (f"tested {ex.anchor_source_name}"
                                     if ex.anchor_source_name else "tested anchor")
        return ex

    def _attach_force_prescriptions(self, ex: Exercise, assessment: Assessment,
                                    entry, ladder, exercise_type: str) -> Exercise:
        """Load from measured isometric force · fallback anchor only."""
        objective = getattr(self, "_objective", None)
        if objective is None or ladder is None or entry is None:
            return ex
        if entry.get("_shim"):
            # A name-derived joint is good enough to bias selection or add a
            # set. It is not good enough to hang a weight off · a load needs a
            # real library entry with real equipment tagging.
            return ex
        from force_load import (ImplausibleLoadError, build_force_prescriptions,
                                find_force_anchor)
        try:
            anchor = find_force_anchor(entry, objective, self._cfg, ex.name)
        except ImplausibleLoadError:
            raise
        except Exception:
            return ex
        if anchor is None:
            return ex

        # A joint the routing layer says to unload does not get a force-derived
        # working load. Advisory routes cannot add load to a joint the safety
        # layer or the quadrant is steering away from.
        if anchor.joint and anchor.joint in getattr(self, "_advisory_unload", set()):
            ex.objective_notes.append(
                f"No measured load applied · {anchor.joint} routed away from loading.")
            return ex
        if anchor.joint and anchor.joint in getattr(self, "_control_first", set()):
            ex.objective_notes.append(
                f"No measured load applied · {anchor.joint} needs end-range "
                f"control before load.")
            return ex

        weeks = build_force_prescriptions(ex.name, entry, anchor, ladder, self._cfg)
        if not weeks:
            return ex

        ex.week_prescriptions = weeks
        ex.anchor_match_method = "measured_force"
        ex.anchor_source_name = anchor.label()
        ex.anchor_rendered_with_loads = True
        ex.load_source = "measured_isometric_force"
        ex.load_source_detail = anchor.evidence()
        pct = weeks[0].get("load_source_pct")
        ex.objective_notes.append(
            f"Load set from measured force · {pct}% of {anchor.evidence()}.")
        self._note_objective_event(
            f"{ex.name} · load anchored to {anchor.evidence()}")
        return ex

    def _attach_side_dose(self, ex: Exercise, assessment: Assessment,
                          exercise_type: str, pattern: str, entry, ladder):
        """Phase 3 · per-side sets/reps when one side is materially weaker."""
        if not getattr(self, "_side_biases", None) or entry is None:
            return
        try:
            from side_dose import applies_to, bias_for_exercise, format_dose, split_dose
        except Exception:
            return
        effective_pattern = pattern or entry.get("pattern") or ""
        if not applies_to(exercise_type, effective_pattern, self._cfg,
                          name=ex.name, dose=ex.dose):
            # Not a unilateral accessory/corrective · per-side dosing would not
            # be a coherent prescription.
            return
        bias = bias_for_exercise(entry, self._side_biases)
        if bias is None:
            return

        profile = getattr(self, "_progression", None)
        templates = (ladder or (profile.week_templates(exercise_type)
                                if profile is not None else []))

        # Several accessory and corrective exercises carry an EXPLICIT dose set
        # by their block builder ("3 × 8/side · 2-sec hold"), not the ladder.
        # Splitting those against the generic ladder would print a per-side
        # dose that contradicts the dose on the same row. When an explicit dose
        # is present, it is the base.
        explicit_sets, explicit_reps = self._parse_dose(ex.dose)
        if ex.dose and explicit_sets is None:
            # A dose we cannot parse ("3 × 30 sec/side", "3 × 30-40 yd") · a
            # per-side set count would not render coherently against it.
            return

        by_week = {}
        for tpl in templates:
            base_sets = explicit_sets if explicit_sets is not None else tpl["sets"]
            base_reps = explicit_reps if explicit_reps is not None else tpl["reps"]
            split = split_dose(base_sets, base_reps, bias, self._cfg)
            by_week[str(tpl["week"])] = {
                "L": split["L"], "R": split["R"],
                "display": format_dose(split),
            }
        if not by_week:
            return
        ex.dose_by_side = {
            "weak_side": bias.weak_side,
            "reason": f"{bias.detail} ({bias.source.replace('_', ' ')})",
            "source": bias.source,
            "by_week": by_week,
        }
        ex.objective_notes.append(
            f"{bias.weak_side} side gets an extra set · {bias.detail}.")
        self._note_objective_event(
            f"{ex.name} · +1 set on {bias.weak_side} ({bias.detail})")

    def _annotate_objective_selection(self, ex: Exercise, entry):
        """Explain, on the exercise, why the objective layer picked it."""
        if entry is None:
            return
        try:
            from joint_taxonomy import joints_loaded_by
        except Exception:
            return
        joints = joints_loaded_by(entry)
        if not joints and entry.get("_shim"):
            return
        if not joints:
            return
        for j in (getattr(self, "_weak_joints", None) or []):
            if j in joints:
                ex.objective_notes.append(
                    f"Selected with {j} emphasis · weakest tested link.")
                break
        for j in (getattr(self, "_loaded_end_range", None) or set()):
            if j in joints:
                ex.objective_notes.append(
                    f"{j.title()} · limited range with weak end-range force · "
                    f"progressive loaded end-range work indicated.")
                break
        for j in (getattr(self, "_control_first", None) or set()):
            if j in joints:
                ex.objective_notes.append(
                    f"{j.title()} · full range, weak end-range control · earn "
                    f"control before load.")
                break

    def _pick_lb_patterns(self, constraints: list[str], day_num: int = 1,
                            wk_num: int = 1, concerns: list = None) -> list[str]:
        """Pick lower body patterns, respecting constraints AND concerns.

        IMS convention for spine-constrained clients on LB day 1 ·
          Strength A · unilateral knee-dominant + hip-extension teaching
          Strength A is where the client learns hip extension cleanly before loading it.

        Spine triggers · formal SI/disc/lumbar flags · OR free-text "lower_back" concern.
        Knee triggers · formal flag · OR "bad_knee" concern → unilateral knee + hip extension.
        """
        has_spine = self._has_spine_constraint(constraints) or self._has_concern(concerns, "lower_back")
        has_knee = self._has_concern(concerns, "bad_knee") or self._has_concern(concerns, "knee")

        if has_spine:
            # Strength A · knee-dominant unilateral + hip extension teaching
            return ["squat_unilateral", "hip_extension_teaching"]
        if has_knee:
            # Bad knee · keep loaded squats off the menu · use unilateral hip-dominant work
            return ["squat_unilateral", "hinge_unilateral"]
        # Default · knee-dominant + hip-dominant
        return ["squat", "hinge"]

    def _has_spine_constraint(self, constraints: list[str]) -> bool:
        flags = {"si_joint_sensitivity", "no_axial_loading", "lumbar_issue", "disc_issue"}
        return any(c.lower().replace(" ", "_") in flags for c in (constraints or []))

    def _has_concern(self, concerns: list, *keywords) -> bool:
        """True if any concern entry contains any of the keywords (case-insensitive)."""
        if not concerns:
            return False
        normalized = [str(c).lower().replace(" ", "_").replace("-", "_") for c in concerns]
        for kw in keywords:
            kw_lower = kw.lower().replace(" ", "_").replace("-", "_")
            for n in normalized:
                if kw_lower in n:
                    return True
        return False

    def _pick_strength_exercise(self, pattern: str, constraints: list[str],
                                  assessment=None, day_type: str = None,
                                  day_num: int = 1,
                                  exclude_names: set = None) -> Exercise:
        """Select a strength exercise matching the pattern, filtered by constraints.

        Picker priority ·
          1. If client has a strength_marker_test for an exercise in this pattern's
             pool, USE that exercise. Coach tested it · they probably want it.
          2. Otherwise pick from the pool, varying by day_type so LB Day 1 and
             LB Day 4 don't always get the same pick.
          3. Constraint-safe variants always rise to the top.

        exclude_names: optional set of names already chosen in the same block.
        Picker will avoid duplicates · falls through to the next valid candidate.
        """
        exclude_names = exclude_names or set()
        # ─── POOLS · each pattern has multiple candidates ───
        # Order within each list = preference order for typical client.
        # Constraint filtering below then reorders.
        pools = {
            # SQUAT family · axial vs non-axial
            "squat":                  ["Goblet Squat", "Landmine Goblet Squat", "Barbell Front Squat", "Barbell Back Squat"],
            "squat_axial_safe":       ["Goblet Squat", "Landmine Goblet Squat", "Wide Stance Kettlebell Lateral Squat (Cossack)"],
            # Knee-friendly squat-pattern alternatives · emphasize hip, minimize knee shear
            # Used when client has bad_knee concern · loads the posterior chain instead
            "squat_knee_friendly":    ["Bridge (Single & Double Leg)", "Hip Thrust",
                                       "Kettlebell Deadlift", "Step Up",
                                       "Bench-Supported Single Leg RDL",
                                       "Dumbbell RDL (Straight Leg Deadlift)"],
            "squat_unilateral":       ["Rear Foot Elevated Split Squat", "Front Foot Elevated Split Squat",
                                       "Split Squat", "TRX Assisted Split Squat",
                                       "Step Up", "Step Down",
                                       "Slide Board Reverse Lunge", "Landmine Reverse Lunge",
                                       "Around the World Lunge",
                                       "Single Leg Squat (Pistol or Assisted)",
                                       "Wide Stance Kettlebell Lateral Squat (Cossack)"],
            "squat_lateral":          ["Lateral Lunge",
                                       "Slide Board Lateral Lunge",
                                       "Wide Stance Kettlebell Lateral Squat (Cossack)"],

            # HINGE family
            "hinge":                  ["Trap Bar Deadlift", "Dumbbell RDL (Straight Leg Deadlift)",
                                       "Barbell RDL (Straight Leg Deadlift)", "Kettlebell Deadlift",
                                       "Landmine Deadlift", "Landmine RDL", "Barbell Deadlift"],
            "hinge_teach":            ["TRX Double-Leg Hinge", "Dowel Hip Hinge", "Kettlebell Deadlift",
                                       "Bridge (Single & Double Leg)"],
            "hinge_axial_safe":       ["Kettlebell Deadlift", "TRX Double-Leg Hinge",
                                       "Landmine Deadlift", "Landmine RDL",
                                       "Dumbbell RDL (Straight Leg Deadlift)"],
            "hinge_unilateral":       ["Bench-Supported Single Leg RDL",
                                       "TRX Bodyweight Single Leg RDL",
                                       "TRX Split-Stance Hinge",
                                       "Dumbbell Single Leg RDL",
                                       "Kettlebell Single Leg RDL",
                                       "Landmine Single Leg RDL",
                                       "Single Leg RDL (weighted or unweighted)",
                                       "Single Leg Deadlift (reaching or kettlebell)"],

            # HAMSTRING CURL (new category)
            "hamstring_curl":         ["Physioball Hamstring Curl", "Slide Board Hamstring Curls"],

            # TEACHING / ACTIVATION (hip extension)
            "hip_extension_teaching": ["Double-Leg Hip Bridge", "Frog Pump", "Banded Hip Bridge",
                                       "Bridge (Single & Double Leg)", "Kettlebell Deadlift", "Dowel Hip Hinge"],

            # HIP BRIDGE PROGRESSION (used for Strength B accessory)
            "hip_bridge":             ["Double-Leg Hip Bridge", "Frog Pump", "Banded Hip Bridge",
                                       "Single-Leg Hip Bridge", "Foot-Elevated Hip Bridge",
                                       "Foot-Elevated Single-Leg Hip Bridge",
                                       "Dumbbell Hip Bridge", "Barbell Hip Bridge"],
            "hip_bridge_axial_safe":  ["Double-Leg Hip Bridge", "Frog Pump", "Banded Hip Bridge",
                                       "Single-Leg Hip Bridge", "Foot-Elevated Hip Bridge",
                                       "Dumbbell Hip Bridge"],

            # PRESSING · horizontal
            "push_horizontal":        ["Dumbbell Bench Press (Single or Double Arm)",
                                       "Dumbbell Floor Press (Single or Double Arm)",
                                       "Pushup (Full, Eccentric, Isometric)",
                                       "Barbell Bench Press", "Cable Press (Single or Double Arm)"],
            # Shoulder-friendly push · floor press, landmine, neutral grip · less impingement risk
            "push_horizontal_shoulder_friendly": [
                                       "Dumbbell Floor Press (Single or Double Arm)",
                                       "Half-Kneeling Landmine Press", "Cable Press (Single or Double Arm)",
                                       "Pushup (Full, Eccentric, Isometric)"],

            # PRESSING · vertical (shoulder)
            "push_vertical":          ["Half Kneeling Overhead Press", "Seated Overhead Press (Single or Double Arm)",
                                       "Half-Kneeling Landmine Press", "Standing Landmine Press",
                                       "Front Foot Elevated Overhead Press", "Standing Overhead Press"],
            "push_vertical_axial_safe": ["Half-Kneeling Landmine Press", "Half Kneeling Overhead Press",
                                         "Seated Overhead Press (Single or Double Arm)"],
            # Shoulder-friendly vertical push · landmine angle, less direct overhead
            "push_vertical_shoulder_friendly": [
                                       "Half-Kneeling Landmine Press", "Standing Landmine Press"],

            # PULLING · horizontal
            "pull_horizontal":        ["Chest Supported Row", "Single Arm Dumbbell Row",
                                       "Inverted Row", "TRX Row", "Landmine Chest-Supported Row",
                                       "Landmine (Meadows) Row", "Landmine T-Bar Row"],

            # PULLING · vertical
            "pull_vertical":          ["Lat Pulldown (Close and Wide Grip)", "Single Arm Cable Pulldown",
                                       "Cable X Pulldown", "Chin-up", "Pull-up (Pronated)"],

            # ROTATION (thoracic-dominant)
            "rotation":               ["Half Kneeling Chops", "Half Kneeling Lifts", "Standing Chops",
                                       "Standing Lifts", "Pulley Rotation (Bottom-Up, Horizontal, Top-Down)",
                                       "Landmine Rotation (Rainbow)", "Standing Cable Core Rotation"],

            # CORE (new dedicated pool)
            "core_antiextension":     ["Deadbug (Training Table)", "Pilates Roll-Down (Training Table)",
                                       "Physioball Saw (Plank)", "Slide Board Plank Saws",
                                       "Hollow Hold", "Dead Bug", "Front Plank"],
            "core_antirotation":      ["Bird Dog (Training Table)", "Physioball Circles (Plank)",
                                       "Pallof Press (Standing Anti-Rotation)", "Suitcase March"],
            "core_dynamic":           ["Slide Board Mountain Climbers", "Physioball Saw (Plank)",
                                       "Ab Wheel / Barbell Rollout"],

            # CARRY
            "carry":                  ["Farmer Carry", "Suitcase Carry", "Front Rack Carry",
                                       "Overhead Carry", "Bottoms-Up Kettlebell Carry"],
        }

        # Legacy single-name mapping (keeps old callers working)
        legacy_fallback = {
            "squat": "Goblet Squat",
            "squat_unilateral": "Rear Foot Elevated Split Squat",
            "hinge": "Trap Bar Deadlift",
            "hinge_unilateral": "Single Leg RDL (weighted or unweighted)",
            "hip_extension_teaching": "Bridge (Single & Double Leg)",
            "push_horizontal": "Pushup (Full, Eccentric, Isometric)",
            "push_vertical": "Half-Kneeling Landmine Press",
            "pull_horizontal": "Chest Supported Row",
            "pull_vertical": "Lat Pulldown (Close and Wide Grip)",
        }

        # Pick the pool · concerns and constraints both shape this
        # Spine-safe routing fires for formal flags OR a lower_back concern
        client_concerns_for_check = (getattr(assessment, "concerns", None) if assessment else None) or []
        spine_constrained = (self._has_spine_constraint(constraints)
                             or self._has_concern(client_concerns_for_check, "lower_back", "back"))
        rationale = None

        # Pull concerns from the assessment if available
        client_concerns = []
        if assessment is not None:
            client_concerns = [str(c).lower().replace(" ", "_").replace("-", "_")
                               for c in (getattr(assessment, "concerns", None) or [])]

        has_knee_concern = any("knee" in c for c in client_concerns)
        has_shoulder_concern = any("shoulder" in c for c in client_concerns)

        # ── CONCERN-DRIVEN POOL ROUTING (highest priority) ──
        # Bad knee · all squat/lunge patterns route to knee-friendly hip-emphasis pool
        if has_knee_concern and pattern in ("squat", "squat_unilateral", "squat_lateral"):
            candidates = pools.get("squat_knee_friendly", pools["squat"])
            rationale = "Knee-friendly · hip-emphasis pattern (knee concern noted)"
        # Bad shoulder · pressing patterns route to shoulder-friendly variants
        elif has_shoulder_concern and pattern == "push_horizontal":
            candidates = pools.get("push_horizontal_shoulder_friendly", pools["push_horizontal"])
            rationale = "Shoulder-friendly press · neutral grip / floor-supported (shoulder concern noted)"
        elif has_shoulder_concern and pattern == "push_vertical":
            candidates = pools.get("push_vertical_shoulder_friendly", pools["push_vertical"])
            rationale = "Landmine angle · reduces direct overhead load (shoulder concern noted)"
        # ── SPINE CONSTRAINT ROUTING (existing behavior) ──
        elif spine_constrained and f"{pattern}_axial_safe" in pools:
            candidates = pools[f"{pattern}_axial_safe"]
            rationale = "Non-axial variant selected · spine-safe pattern"
        elif spine_constrained and pattern == "squat":
            candidates = pools.get("squat_axial_safe", pools["squat"])
            rationale = "Non-axial squat · protects spine"
        elif spine_constrained and pattern == "hinge":
            candidates = pools.get("hinge_axial_safe", pools["hinge"])
            rationale = "Non-axial hinge · protects spine"
        elif spine_constrained and pattern == "push_vertical":
            candidates = pools.get("push_vertical_axial_safe", pools["push_vertical"])
            rationale = "Half-kneeling / seated press · no axial load"
        elif pattern in pools:
            candidates = pools[pattern]
        else:
            candidates = [legacy_fallback.get(pattern, "Goblet Squat")]

        # Filter out any candidate whose DB entry violates constraints
        # ── PICK ORDER ──
        # 1) prefer an exercise the coach actually tested (in strength_marker_tests)
        #    that matches this pattern · if you tested Front Squat, you get Front Squat
        # 2) rotate the pool by day_type so Day 1 ≠ Day 4 in the same week
        # 3) fall back to first valid pool candidate
        name = None

        # 1 · Test-match preference (skip if it'd duplicate within the block)
        if assessment is not None:
            tested_match = self._match_pattern_to_tested_exercise(
                pattern, candidates, assessment, exclude_names=exclude_names)
            if tested_match:
                name = tested_match
                if not rationale:
                    rationale = "Pattern picked from your tested exercises"

        # 2 · Day-based rotation through the pool · skip excluded names
        client_concerns = getattr(assessment, "concerns", None) if assessment else None
        if name is None and day_type:
            rotated = self._rotate_pool(candidates, day_type, pattern, day_num=day_num)
            # Filter out anything already used in the same block
            rotated = [c for c in rotated if c not in exclude_names]
            # Weakest-link emphasis · reorders an already-approved list toward
            # the weakest tested joint and away from any joint the ROM × force
            # router says to unload. Selection bias only · it can never
            # introduce a candidate the contraindication check would reject,
            # because the rejection happens after this, in
            # _first_valid_candidate. No objective data = no reordering.
            rotated = self._bias_candidates(rotated)
            name = self._first_valid_candidate(rotated, constraints, concerns=client_concerns)

        # 3 · Default · skip excluded names
        if name is None:
            filtered = [c for c in candidates if c not in exclude_names]
            filtered = self._bias_candidates(filtered)
            name = (self._first_valid_candidate(filtered, constraints, concerns=client_concerns)
                    or (filtered[0] if filtered else candidates[0]))

        # If concerns drove a non-default pick, surface that in the rationale
        if client_concerns and not rationale:
            rationale = f"Selected with care for · {', '.join(client_concerns)}"

        if "unilateral" in pattern and spine_constrained and not rationale:
            rationale = "Unilateral variant selected due to spine-loading constraint"
        if pattern == "hip_extension_teaching" and not rationale:
            rationale = "Hip extension pattern without spine load · clean teaching exercise"

        return Exercise(name=name, library="external_training", rationale=rationale)

    def _match_pattern_to_tested_exercise(self, pattern: str,
                                            candidates: list[str],
                                            assessment,
                                            exclude_names: set = None) -> str | None:
        """Find a tested exercise whose name matches one of the pool candidates
        for this pattern.

        IMPORTANT · returns the TESTED EXERCISE NAME (not the pool canonical)
        when the match is via alias-group · this way the program shows the
        exact exercise the coach measured ("3 Point Row" vs the picker's
        default "Single Arm Dumbbell Row").

        Match priority ·
          1. EXACT normalized name vs any pool candidate → return tested name
          2. ALIAS group match · tested + any pool candidate share an alias
             group (e.g., "row") → return tested name
          3. None → caller falls through to default rotation

        exclude_names: skip any candidate or tested name already used in the
        same block (prevents duplicates across two patterns that resolve to
        the same anchored exercise).
        """
        exclude_names = exclude_names or set()
        tests = getattr(assessment, "strength_marker_tests", None) or []
        if not tests:
            return None

        try:
            from strength_anchor_resolver import normalize_exercise_name, _alias_group_for
        except Exception:
            return None

        cand_norms = [(c, normalize_exercise_name(c), _alias_group_for(c)) for c in candidates]

        for t in tests:
            tname = (getattr(t, "exercise_name", None) or "").strip()
            if not tname or tname in exclude_names:
                continue
            tname_norm = normalize_exercise_name(tname)
            tgroup = _alias_group_for(tname)

            # 1 · Exact normalized match against any candidate
            for _cand, cnorm, _cgroup in cand_norms:
                if cnorm and cnorm == tname_norm:
                    return tname

            # 2 · Alias group match · share the same alias group
            if tgroup:
                for _cand, _cnorm, cgroup in cand_norms:
                    if cgroup and cgroup == tgroup:
                        return tname

        return None

    @staticmethod
    def _parse_dose(dose):
        """(sets, reps) from a dose string, or (None, None) if it isn't N × M."""
        if not dose:
            return None, None
        m = re.match(r"\s*(\d+)\s*[x×]\s*(\d+)\s*(?:/side)?\s*(?:·.*)?$",
                     str(dose))
        if not m:
            return None, None
        return int(m.group(1)), int(m.group(2))

    def _entry_or_shim(self, name: str) -> dict:
        """The library entry for an exercise, or a name-derived stand-in.

        The accessory and corrective libraries hold plenty of exercises the
        unified DB has never seen. Without a stand-in, every one of them would
        silently skip weakest-link emphasis, routing annotation and per-side
        dosing · which is most of the accessory work in a session.
        """
        entry = self._find_entry_by_name(name)
        if entry is not None:
            return entry
        try:
            from joint_taxonomy import shim_entry_for_name
            return shim_entry_for_name(name)
        except Exception:
            return {"name": name}

    def _bias_candidates(self, candidates: list) -> list:
        """Apply weakest-link emphasis to a candidate list.

        Returns the list unchanged when there is no objective data, which is
        the common case and the reason no-data output stays byte-identical.
        """
        weak = getattr(self, "_weak_joints", None) or []
        avoid = getattr(self, "_advisory_unload", None) or set()
        if not candidates or (not weak and not avoid):
            return candidates
        try:
            from weakest_link import bias_candidates
            return bias_candidates(candidates, self._find_entry_by_name,
                                   weak, avoid)
        except Exception:
            return candidates

    def _rotate_pool(self, candidates: list[str], day_type: str,
                      pattern: str, day_num: int = 1) -> list[str]:
        """Rotate the pool order so different days don't always pick the same
        exercise. Same client + same day_num always gets the same pick (stable),
        but Day 1 and Day 4 differ.

        Mechanism · take a per-pattern base offset (so different patterns start
        at different points in their pool), then ADD day_num so each day rotates
        one further than the last.
        """
        if not candidates:
            return candidates
        if len(candidates) <= 1:
            return list(candidates)

        import hashlib
        # Per-pattern base offset · stable across deploys
        digest = hashlib.md5(pattern.encode("utf-8")).digest()
        base = digest[0]
        # Combine with day_num so each day rotates +1 (or +2 etc)
        seed = (base + day_num) % len(candidates)
        return list(candidates[seed:]) + list(candidates[:seed])

    def _first_valid_candidate(self, candidates: list[str], constraints: list[str],
                                concerns: list[str] = None) -> str | None:
        """Return the first candidate whose contraindications don't clash with
        constraints OR client concerns (bad knee, bad shoulder, etc.)."""
        for name in candidates:
            entry = self._find_entry_by_name(name)
            if entry is None:
                # If we can't find it · allow it (PDF still renders the name)
                # but only when we have NO concerns to check (otherwise risky)
                if not concerns:
                    return name
                continue
            if not self._violates_constraints(entry, constraints, concerns=concerns):
                return name
        return None

    def _find_entry_by_name(self, name: str) -> dict | None:
        """Look up an exercise entry in the unified DB by its canonical name."""
        entries = self.db.get('exercises', {})
        if isinstance(entries, dict):
            for eid, entry in entries.items():
                if entry.get('name') == name:
                    return entry
        elif isinstance(entries, list):
            for entry in entries:
                if entry.get('name') == name:
                    return entry
        return None

    def _strength_dose(self, wk_num: int, exercise_type: str, pattern: str = None,
                       assessment: Assessment = None, side: str = None) -> str:
        """Dose for one week, for one exercise type.

        v2.0 · this no longer hard-codes a ladder. Sets and reps come from the
        client's resolved ProgressionProfile (progression_profile.py), which is
        the single source for all three places the ladder used to live ·
        here, plan_pdf._WEEK_4_TEMPLATE and strength_math.WEEK_TEMPLATES.

        The legacy IMS Block 1 ladder is still what a client with no age,
        training age or conditioning on file receives ·

          Compound   · W1 3×12 · W2 3×10 · W3 4×8  · W4 4×6
          Accessory  · W1 3×12 · W2 3×10 · W3 4×10 · W4 4×8
          Corrective · 3×12 throughout

        A 68-year-old deconditioned client and a 28-year-old athlete now get
        different ladders. That was the point.

        Phase 3 · ``assessment`` and ``side`` are read. When a side bias exists
        for the joint this exercise loads and per-side dosing is coherent for
        the exercise type, the weaker side earns an extra set and the dose
        string comes back in its per-side form ("L 4 × 12 · R 3 × 12").

        Recovery-adjusted · if _current_recovery < 0.85 the profile has already
        stepped one ladder more conservative, and W1 volume is trimmed on top.
        """
        is_side = pattern and ("unilateral" in pattern or "teaching" in pattern)
        suffix = "/side" if is_side else ""
        recovery = getattr(self, '_current_recovery', 1.0)

        profile = getattr(self, "_progression", None)
        if profile is None:
            try:
                from progression_profile import legacy_profile
                profile = legacy_profile(getattr(self, "_cfg", None))
            except Exception:
                profile = None

        if profile is not None:
            sets, reps = profile.dose(wk_num, exercise_type)
        else:  # pragma: no cover · config missing entirely
            fallback = {
                "compound": {1: (3, 12), 2: (3, 10), 3: (4, 8), 4: (4, 6)},
                "accessory": {1: (3, 12), 2: (3, 10), 3: (4, 10), 4: (4, 8)},
                "corrective": {1: (3, 12), 2: (3, 12), 3: (3, 12), 4: (3, 12)},
            }
            sets, reps = fallback.get(exercise_type, {}).get(wk_num, (3, 10))

        # ── Phase 3 · per-side dose ──
        if assessment is not None:
            split = self._side_split(assessment, exercise_type, pattern, side,
                                     sets, reps)
            if split:
                from side_dose import format_dose
                return format_dose(split)

        base = f"{sets} × {reps}{suffix}"
        if wk_num == 1 and recovery < 0.85:
            return self._volume_adjusted_dose(base, recovery)
        return base

    def _side_split(self, assessment: Assessment, exercise_type: str,
                    pattern: str, side, sets: int, reps: int):
        """Resolve the per-side sets/reps for one exercise, or None.

        ``side`` may be an explicit side ("L"/"R"), a library entry dict, or
        None · in which case the joint tagging on the exercise decides.
        """
        biases = getattr(self, "_side_biases", None)
        if not biases:
            return None
        try:
            from side_dose import applies_to, bias_for_exercise, split_dose
        except Exception:
            return None
        cfg = getattr(self, "_cfg", None)
        name = side.get("name", "") if isinstance(side, dict) else ""
        if not applies_to(exercise_type, pattern, cfg, name=name):
            return None

        bias = None
        if isinstance(side, dict):
            bias = bias_for_exercise(side, biases)
        elif isinstance(side, str) and side.upper() in ("L", "R"):
            for b in biases.values():
                if b.weak_side == side.upper():
                    bias = b
                    break
        if bias is None:
            return None
        return split_dose(sets, reps, bias, cfg)

    def _progression_note(self, wk_num: int, exercise_type: str) -> str:
        """One-line note · what changes this week for this exercise type.

        4-week IMS Block 1 ·
          W1 · Base Volume        · 3 × 12 · groove the pattern
          W2 · Tempo Control      · 3 × 10 · 3-sec eccentric
          W3 · Strength Build     · 4 × 8  · push load
          W4 · Performance Week   · 4 × 6  · top-end strength

        Returns None when no modification is needed.
        """
        if wk_num == 1:
            return None

        if exercise_type == "compound":
            if wk_num == 2:
                return "Three sets of ten · 3-sec eccentric · same load family as W1."
            if wk_num == 3:
                return "Four sets of eight · standard tempo · push the working weight."
            if wk_num == 4:
                return "Four sets of six · top-end strength · or retest 10RM if coach approves."

        if exercise_type == "accessory":
            if wk_num == 2:
                return "Drop reps to 10 · 3-sec eccentric on every rep."
            if wk_num == 3:
                return "Add a fourth set · keep the rep target at 10."
            if wk_num == 4:
                return "Drop reps to 8 · push the load."

        if exercise_type == "corrective":
            if wk_num == 2:
                return "Add a 2-sec iso hold at end range."
            if wk_num == 3:
                return "Extend iso to 3 sec if tolerated."
            if wk_num == 4:
                return "Same dose · push effort and end-range control."

        return None

    def _apply_week_progression(self, ex: Exercise, wk_num: int, exercise_type: str) -> Exercise:
        """Apply IMS Block 1 (4-week straight-set) progression levers.

        For compounds and accessories, the sets/reps are already set by
        _strength_dose. This method ONLY adds tempo notes where appropriate.
        Personalized loads (when test data exists) are layered on later by
        the PDF renderer using the strength_math module.

        Week 1 · base dose, standard tempo
        Week 2 · same sets, fewer reps, 3-sec eccentric tempo note
        Week 3 · 4 sets of 8, standard tempo, RPE 8
        Week 4 · 4 sets of 6, RPE 8-9 (or retest)
        """
        if wk_num == 1:
            return ex

        if wk_num == 2:
            if exercise_type in ("compound", "accessory"):
                ex.tempo = "3-sec eccentric"
            elif exercise_type == "corrective":
                ex.tempo = "2-sec iso at end range"
            return ex

        if wk_num == 3:
            if exercise_type == "corrective":
                ex.tempo = "3-sec iso at end range"
            # Compounds and accessories use standard tempo · no mutation needed
            return ex

        if wk_num == 4:
            # No mutation · the dose is already 4 × 6 (compound) / 4 × 8 (accessory)
            # The "or retest 10RM" note is added by the PDF renderer
            return ex

        return ex

    def _bump_reps(self, dose: str, by: int) -> str:
        """Increase the rep count in a dose string.
        Handles '3x8', '3x8/side', '3x10', etc.
        """
        import re
        # Match NxM patterns · "3x8" → "3x10"
        def replace_rep(match):
            sets = match.group(1)
            reps = int(match.group(2))
            return f"{sets}x{reps + by}"
        # Only bump the FIRST occurrence (some doses have multiple)
        result = re.sub(r'(\d+)x(\d+)', replace_rep, dose, count=1)
        return result

    def _reduce_sets(self, dose: str, by: int) -> str:
        """Reduce the set count in a dose string for deload."""
        import re
        def replace_set(match):
            sets = int(match.group(1))
            reps = match.group(2)
            new_sets = max(sets - by, 1)  # never go below 1
            return f"{new_sets}x{reps}"
        result = re.sub(r'(\d+)x(\d+)', replace_set, dose, count=1)
        return result

    # ------------------------------------------------------
    # LOAD · ONE MECHANISM
    # ------------------------------------------------------
    #
    # _fill_load() and _suggest_starting_load() were DELETED in v2.0.
    #
    # They were a second load path. They parsed the free-text
    # strength_marker_results dict with a regex, took 70% of the first number
    # they found, and appended "  @ ~N lbs" to the dose string · after
    # _attach_week_prescriptions had already built a properly resolved
    # per-week load from the same client's test data. Two mechanisms writing
    # the same field, one of them a regex over free text, with no defined
    # precedence between them.
    #
    # The single load path is now ·
    #
    #     _attach_week_prescriptions
    #        ├─ strength_anchor_resolver → strength_math   (tested anchor)
    #        └─ force_load                                 (measured isometric,
    #                                                       fallback anchor only)
    #
    # strength_marker_results is still accepted on the wire and still shown to
    # the coach, but it does not write a load. Do not reintroduce a shortcut
    # here · if a load needs to come from somewhere new, it goes through
    # _attach_week_prescriptions or it does not happen.
    #
    # ------------------------------------------------------
    # BLOCK BUILDERS · STRENGTH B (accessory + FRA-specific)
    # ------------------------------------------------------

    def _build_strength_b(self, day_type: str, priorities: list[FRAPriority],
                          assessment: Assessment, wk_num: int,
                          exclude_names: set = None,
                          day_num: int = 1) -> Block:
        exclude_names = set(exclude_names or [])
        exercises = []

        # 1 · Core accessory · prefer the tagged library picker (rotates by
        # session + week, filters by concerns + level). Falls back to the
        # legacy hard-coded rotation if the library file isn't present.
        core_ex = self._pick_core_accessory(assessment, day_type, day_num,
                                              wk_num, exclude_names=exclude_names)
        if core_ex is None:
            # Legacy fallback · keeps the old behavior alive
            tier = self._infer_client_tier(assessment)
            core_candidates = [
                "Deadbug (Training Table)",
                "Dead Bug",
                "Bird Dog (Training Table)",
                "Pilates Roll-Down (Training Table)",
                "Hollow Hold",
                "Physioball Saw (Plank)",
            ]
            if tier == "advanced":
                core_candidates = [
                    "Slide Board Plank Saws",
                    "Physioball Saw (Plank)",
                    "Ab Wheel / Barbell Rollout",
                    "Physioball Circles (Plank)",
                ] + core_candidates
            # Filter out names already used in the session
            core_candidates = [c for c in core_candidates if c not in exclude_names]
            if not core_candidates:
                core_candidates = ["Dead Bug"]  # absolute fallback
            core_name = core_candidates[(wk_num - 1) % len(core_candidates)]
            core_entry = self._find_entry_by_name(core_name)
            core_dose = (core_entry or {}).get("dose") or "3 × 10"
            core_ex = Exercise(
                name=core_name,
                library="external_training",
                dose=core_dose,
                progression_note=self._progression_note(wk_num, "accessory"),
            )
        core_ex = self._apply_week_progression(core_ex, wk_num, "accessory")
        core_ex = self._attach_week_prescriptions(core_ex, assessment,
                                                  exercise_type="accessory")
        exercises.append(core_ex)

        # 2 · Pattern complement
        has_spine = self._has_spine_constraint(assessment.constraints)
        if day_type == "strength_lb":
            # Default · rotate through hip-bridge progression
            bridge_pool = "hip_bridge_axial_safe" if has_spine else "hip_bridge"
            bridge_candidates = {
                "hip_bridge": [
                    "Single-Leg Hip Bridge", "Foot-Elevated Hip Bridge",
                    "Dumbbell Hip Bridge", "Banded Hip Bridge",
                    "Barbell Hip Bridge", "Foot-Elevated Single-Leg Hip Bridge",
                ],
                "hip_bridge_axial_safe": [
                    "Single-Leg Hip Bridge", "Foot-Elevated Hip Bridge",
                    "Dumbbell Hip Bridge", "Banded Hip Bridge", "Frog Pump",
                ],
            }[bridge_pool]
            bridge_name = bridge_candidates[(wk_num - 1) % len(bridge_candidates)]

            # If a tested Hip Thrust anchor exists, USE it directly · spec says
            # "if exact tested anchor exists for Hip Thrust, use Hip Thrust in
            # the program. Do not map Hip Thrust to Single-Leg Hip Bridge unless
            # Hip Thrust is contraindicated."
            try:
                from strength_anchor_resolver import (
                    resolve_anchor_for_exercise, _alias_group_for,
                )
                tests = getattr(assessment, "strength_marker_tests", None) or []
                # Look for an anchor in the hip_extension alias group
                for t in tests:
                    tn = getattr(t, "exercise_name", None) or ""
                    if _alias_group_for(tn) == "hip_extension":
                        # Use the tested name directly (Hip Thrust), unless it's
                        # already what we'd pick OR contraindicated for spine
                        if not has_spine:
                            bridge_name = tn
                            break
                        # Spine-constrained · Hip Thrust requires axial loading
                        # in some setups · keep the bridge variant for safety
            except Exception:
                pass

            # Cross-block + within-block dedup · if this name is already used
            # in the session (Strength A picked it OR earlier in this block),
            # swap to the next valid alternative.
            existing_names = {e.name for e in exercises} | set(exclude_names)
            if bridge_name in existing_names:
                # Spec · for clients with knee concerns, prefer ankle stability,
                # calf-tib, hamstring bridge OVER repeating Hip Thrust.
                concerns = getattr(assessment, "concerns", None) or []
                has_knee_concern = any(c in concerns for c in
                                         ("bad_knee", "meniscus_flare", "post_surgery_knee"))
                if has_knee_concern:
                    knee_friendly_subs = [
                        "Hamstring Bridge", "Single-Leg Hip Bridge",
                        "Foot-Elevated Hip Bridge", "Frog Pump",
                        "Tibialis Raise", "Standing Calf Raise",
                    ]
                    for cand in knee_friendly_subs:
                        if cand not in existing_names:
                            bridge_name = cand
                            break
                else:
                    # General fallback · walk the bridge_candidates pool
                    for cand in bridge_candidates:
                        if cand not in existing_names:
                            bridge_name = cand
                            break

            second_ex = Exercise(
                name=bridge_name,
                library="external_training",
                dose="3 × 10/side" if "Single" in bridge_name else "3 × 10",
                progression_note=self._progression_note(wk_num, "accessory"),
                rationale="Hip extension volume · progression rotated weekly",
            )
        else:
            # UB day · keep Landmine SA Press as vertical-press volume default
            second_ex = Exercise(
                name="Landmine SA Press",
                library="external_training",
                dose="3 × 6/side",
                progression_note=self._progression_note(wk_num, "accessory"),
            )
        second_ex = self._apply_week_progression(second_ex, wk_num, "accessory")
        second_ex = self._attach_week_prescriptions(second_ex, assessment,
                                                    exercise_type="accessory")
        exercises.append(second_ex)

        # 3 · FRA-specific corrective for the day's top priority
        if priorities:
            top = priorities[0]
            # Build session-level used names to keep dedup tight
            session_used = set(exclude_names) | {e.name for e in exercises}
            # Prefer the tagged library picker · rotates by session+week,
            # filters by concerns + level. Fall back to the legacy hard-coded
            # mapping if the library file isn't present or no variant matches.
            corrective = self._pick_corrective_from_library(
                top, assessment, day_type=day_type,
                day_num=day_num, wk_num=wk_num,
                exclude_names=session_used,
            )
            if corrective is None:
                corrective = self._pick_corrective(top)
            if corrective:
                corrective.dose = corrective.dose or self._strength_dose(
                    wk_num, "corrective", assessment=assessment,
                    side=self._find_entry_by_name(corrective.name))
                corrective.progression_note = self._progression_note(wk_num, "corrective")
                corrective = self._apply_week_progression(corrective, wk_num, "corrective")
                corrective = self._attach_week_prescriptions(corrective, assessment,
                                                             exercise_type="corrective")
                exercises.append(corrective)

        return Block(
            name="Strength B",
            exercises=exercises,
            duration_note="Accessory work + FRA-specific corrective"
        )

    def _pick_corrective(self, priority: FRAPriority) -> Exercise | None:
        joint = priority.joints[0].lower() if priority.joints else ""
        direction = priority.directions[0].lower() if priority.directions else ""

        mapping = {
            ("shoulder", "er"): "Cable/Band Shoulder ER",
            ("shoulder", "ir"): "Cable/Band Shoulder IR",
            ("hip", "ir"): "Cable Hip IR (clamshell + band)",
            ("hip", "er"): "Banded Clamshell",
            ("ankle", "plantarflexion"): "Calf Raise (bilateral → unilateral)",
            ("ankle", "eversion"): "Banded Ankle Eversion",
            ("hamstring", ""): "Nordic Hamstring Curl (assisted)"
        }
        name = mapping.get((joint, direction))
        if not name:
            for (j, _), val in mapping.items():
                if j == joint:
                    name = val
                    break
        if not name:
            return None
        side_note = ""
        if "L+R" not in priority.description and len(priority.sides) == 1:
            side_note = f" ({priority.sides[0]} side)"
        return Exercise(
            name=name + side_note,
            library="external_training",
            rationale=f"Corrective for {priority.description}"
        )

    # ------------------------------------------------------
    # BLOCK BUILDERS · COOL DOWN
    # ------------------------------------------------------

    def _build_cool_down(self, priorities: list[FRAPriority], wk_num: int) -> Block:
        """Integration work. PAIL/RAIL combos allowed here."""
        exercises = []

        # 1 · CARs at the day's joint focus
        if priorities:
            joint = priorities[0].joints[0].title() if priorities[0].joints else "General"
            exercises.append(Exercise(
                name=f"Seated {joint} CARs",
                library="cars",
                dose="1x5/side",
                rationale="Integrate the day's priority at end-range control"
            ))

        # 2 · transition/flow drill
        exercises.append(Exercise(
            name="Modified 90/90 Windshield Wipers",
            library="base_positions",
            dose="1x6/side",
            rationale="Integrate hip rotation across positions"
        ))

        # 3 · OPTIONAL PAIL/RAIL combo (coach adds when target joint needs it)
        if priorities and self._priority_needs_pail_rail(priorities[0]):
            pail_rail_ex = self._build_pail_rail_exercise(
                priorities[0], assessment=getattr(self, "_current_assessment", None)
            )
            if pail_rail_ex:
                exercises.append(pail_rail_ex)

        # 4 · low-intensity integration
        exercises.append(Exercise(
            name="Standing Arm Swings to Capsule End Range (Controlled)",
            library="end_range",
            dose="1x10",
            rationale="Gentle shoulder capsular finish"
        ))

        return Block(
            name="Dynamic Cool Down",
            exercises=exercises,
            duration_note="PAIL/RAIL combos allowed here (per IMS rule)"
        )

    def _build_capsule_work_block(self, priorities: list[FRAPriority],
                                    assessment: 'Assessment' = None) -> 'Block':
        """Capsule work block · runs at the end of every strength session.

        Per IMS spec ·
          - 1 hip target per session (not all 6)
          - Default to client level 1 (20-40% effort)
          - Renders client-friendly cue · setup/intent live in coach mode
          - Client plan never sees 100% prescriptions · gated in the library
            file via client_facing: false

        Picks the FIRST hip-related FRA priority. If no hip priority exists,
        returns None · the block doesn't show up. This honors "use deep flexion,
        abduction, and adduction only when clearly selected as priority limitations."
        """
        if not priorities:
            return None

        # Find the first hip-related priority
        hip_priority = None
        for p in priorities:
            joints = [j.lower() for j in (p.joints or [])]
            if "hip" in joints or "hip" in (p.description or "").lower():
                hip_priority = p
                break

        if hip_priority is None:
            # No hip priority · skip the block (don't fabricate one)
            return None

        # Build the exercise with structured cues + level-1 effort
        ex = self._build_pail_rail_exercise(hip_priority, assessment=assessment)
        if ex is None:
            return None

        return Block(
            name="Capsule Work (PAIL/RAIL)",
            exercises=[ex],
            duration_note="3-4 min · end of session capsule loading",
        )

    def _build_pail_rail_exercise(self, priority: FRAPriority,
                                    assessment: 'Assessment' = None) -> 'Exercise':
        """Build a PAIL/RAIL combo Exercise using the structured hip mobility
        library. Falls back to a generic Exercise if the library is missing or
        no matching target found.

        Honors the IMS rules ·
          - 20s PAIL · 20s RAIL · 20s breathe (fixed structure)
          - Effort scales by client level, NOT by time
          - Level 1 (foundation) · 20-40%
          - Level 2 (build) · 40-60%
          - Level 3 (capacity) · 60-80%
          - Advanced/coach-only · 80-100% · NOT printed in client PDFs
          - Default · level_1 unless coach has set otherwise
        """
        # Determine the client's level. For now, conservative default · level_1.
        # TODO · wire to assessment.client_level once that field exists.
        level_id = "level_1"
        if assessment is not None:
            level_attr = getattr(assessment, "mobility_level", None)
            if level_attr in ("level_1", "level_2", "level_3", "level_advanced_supervised"):
                level_id = level_attr

        targets = (self.hip_mobility or {}).get("targets", [])
        levels = (self.hip_mobility or {}).get("effort_ramp_by_level", {})
        level_payload = levels.get(level_id, {})

        # Resolve the priority to a hip target by alias matching
        target = self._resolve_hip_mobility_target(priority, targets)

        if target is None or not level_payload:
            # Library missing or no match · graceful fallback
            return Exercise(
                name=f"{priority.description} PAIL/RAIL Combo",
                library="pails_rails",
                dose="20s PAIL · 20s RAIL · 20s reset · light effort",
                rationale="Capsular work at end of session"
            )

        # Build the dose · 20s/20s/20s + effort percentage from the level
        rounds = level_payload.get("rounds", 2)
        pail_pct = level_payload.get("pail_pct", "20-40%")
        rail_pct = level_payload.get("rail_pct", "20-40%")
        dose = (f"20s PAIL @ {pail_pct} · 20s RAIL @ {rail_pct} · "
                f"20s breathe · {rounds} rounds")

        return Exercise(
            name=target["name"],
            library="hip_mobility_targets",
            dose=dose,
            rationale=target.get("client_friendly_cue", "Capsular work at end of session"),
            # Stash the structured cue data on the exercise so the renderer can
            # surface it in coach mode if desired
        )

    def _resolve_hip_mobility_target(self, priority: FRAPriority,
                                       targets: list) -> Optional[dict]:
        """Find a hip target whose fra_aliases match this priority. Returns
        None if no match (caller falls back)."""
        if not targets:
            return None
        priority_str = (priority.description or "").lower().strip()
        if not priority_str:
            return None
        # Only proceed for hip priorities · no point checking shoulder etc.
        priority_joints = [j.lower() for j in (priority.joints or [])]
        if "hip" not in priority_joints and "hip" not in priority_str:
            return None

        # Try each target's aliases · pick the first matching one based on
        # default_priority order (primary first, then supporting)
        primary_targets = [t for t in targets if t.get("default_priority") == "primary"]
        supporting_targets = [t for t in targets if t.get("default_priority") == "supporting"]

        for t in primary_targets + supporting_targets:
            for alias in t.get("fra_aliases", []):
                if alias.lower() in priority_str:
                    return t
        return None

    def _priority_needs_pail_rail(self, priority: FRAPriority) -> bool:
        """Add PAIL/RAIL combo if priority is severe (red-rated)."""
        # Simple heuristic · could be enhanced with mobility_map lookup
        return True  # default to including for priority work

    # ------------------------------------------------------
    # BLOCK BUILDERS · CARDIO DAY
    # ------------------------------------------------------

    def _build_cardio_session(self, day_num: int, wk_num: int,
                                assessment: 'Assessment' = None) -> Session:
        """Cardio day · personalized via cardio_rules engine.

        All decisions about machine choice, intervals, and progression text
        live in cardio_rules.py. This builder just orchestrates the blocks.
        """
        # Build normalized profile via the rules engine
        try:
            from cardio_rules import (
                normalize_cardio_profile,
                generate_cardio_progression,
            )
            profile = getattr(assessment, "cardio_profile", None) if assessment else None
            concerns = getattr(assessment, "concerns", None) if assessment else None
            constraints_rich = getattr(assessment, "constraints_rich", None) if assessment else None
            normalized = normalize_cardio_profile(profile, concerns=concerns,
                                                   constraints_rich=constraints_rich)
            progression = generate_cardio_progression(normalized)
        except Exception:
            normalized = None
            progression = None

        if progression and wk_num in progression:
            wk = progression[wk_num]
            modality_label = wk["machine"]
            focus = wk["focus"]
            main = wk["main"]
            rationale = wk["rationale"]
            duration_note = wk.get("machine_rationale", "")
        else:
            # Hard fallback · generic prescription if rules engine failed entirely
            modality_label = "Primary Modality (Bike, Arc Trainer, Rower, or SkiErg)"
            focus = f"Week {wk_num} · Zone 2 Base"
            main = "15-20 min Zone 2 · RPE 4-5 · nasal breathing only"
            rationale = "Build aerobic base."
            duration_note = ""

        # Warmup (fixed)
        warmup = Block(
            name="Cardio Warmup (10 min)",
            exercises=[
                Exercise(
                    name="Easy Aerobic Ramp",
                    library="external_training",
                    dose="5 min Zone 1 · nasal breathing",
                    rationale="Wake up the aerobic system",
                ),
                Exercise(
                    name="Dynamic Joint Prep",
                    library="external_training",
                    dose="5 min · leg swings, hip openers, thoracic rotations",
                    rationale="Prep joints for sustained cyclic movement",
                ),
            ],
        )

        # Main conditioning · prescription text + chosen modality
        conditioning = Block(
            name=focus,
            exercises=[
                Exercise(
                    name=modality_label,
                    library="external_training",
                    dose=main,
                    rationale=rationale,
                ),
            ],
            duration_note=duration_note,
        )

        # Post-cardio reset (FRC-inspired flow)
        reset = Block(
            name="Post-Cardio Reset",
            exercises=[
                Exercise(
                    name="Hip Abducted Rock Backs",
                    library="base_positions",
                    dose="1x10 slow",
                    rationale="Open posterior hip",
                ),
                Exercise(
                    name="Hip Flexion Rocking (Quadruped)",
                    library="base_positions",
                    dose="1x8/side",
                ),
                Exercise(
                    name="90/90 Transitions",
                    library="base_positions",
                    dose="1x6/side",
                ),
                Exercise(
                    name="Shoulder IR + Scapular CARs",
                    library="cars",
                    dose="2x5 each direction",
                ),
            ],
            duration_note="~5 min",
        )

        return Session(
            day_number=day_num,
            day_type="cardio",
            focus=f"Cardio · {focus}",
            blocks=[
                warmup,
                conditioning,
                self._build_hiit_finisher(assessment, context="cardio", week_num=wk_num),
                reset,
                self._build_coach_finisher([]),
            ],
        )

    def _cardio_modality_label(self, profile) -> str:
        """Build the modality line shown to the client."""
        if profile is None or not getattr(profile, "primary_modality", None):
            return "Primary Modality (Bike, Arc Trainer, Rower, or SkiErg)"

        from cardio_profile import MODALITY_DISPLAY
        primary = MODALITY_DISPLAY.get(profile.primary_modality, profile.primary_modality)
        secondaries = [MODALITY_DISPLAY.get(m, m) for m in profile.secondary_modalities]
        if secondaries:
            return f"{primary}  ·  alt: {', '.join(secondaries)}"
        return primary

    def _cardio_prescription(self, profile, wk_num: int) -> dict:
        """Build a per-week prescription dict based on the cardio profile.

        Rules ·
          1. Deconditioned/beginner · longer Z2 ramp, no intervals at all
          2. cleared_for_intervals + good HR recovery · add controlled intervals W3-W4
          3. not_cleared_for_intervals · pure aerobic build, never intervals
          4. Default (no profile) · existing 4-week aerobic progression
        """
        # Default · current behavior
        default_plan = {
            1: {
                "focus": "Week 1 · Zone 2 Base",
                "main": "15-20 min steady state · RPE 4-5 · nasal breathing only",
                "rationale": "Build aerobic base. If you can't nasal breathe, slow down.",
            },
            2: {
                "focus": "Week 2 · Zone 2 Extended",
                "main": "25 min steady state · RPE 4-5 · nasal breathing",
                "rationale": "Same zone, longer duration. Capacity build.",
            },
            3: {
                "focus": "Week 3 · Zone 2 → Zone 3 Bridge",
                "main": "10 min Z2 warmup + 15 min Z3 (RPE 6-7) + 5 min Z2 cooldown",
                "rationale": "First tempo exposure. Breathing deeper but sustainable.",
            },
            4: {
                "focus": "Week 4 · Z3-Z4 Intervals",
                "main": "5 min Z2 warmup · 4 rounds (3 min Z3 / 1 min Z4) · 5 min cooldown",
                "rationale": "Performance week · alternate tempo and threshold work.",
            },
        }

        if profile is None or not profile.has_data():
            return default_plan.get(wk_num, default_plan[1])

        cleared = profile.cleared_for_intervals()
        deconditioned = profile.is_deconditioned()
        poor_recovery = profile.has_limitation("high_stress_poor_recovery")
        hr_q = profile.hr_recovery.quality()  # 'strong' / 'normal' / 'poor' / None

        # Block intervals if · explicitly not cleared, deconditioned, or poor HR recovery
        block_intervals = (
            profile.has_limitation("not_cleared_for_intervals")
            or deconditioned
            or poor_recovery
            or hr_q == "poor"
            or not cleared
        )

        # Deconditioned · gentler ramp · 8-15 min Z2 · stay in Z2 all 4 weeks
        if deconditioned:
            plans = {
                1: {
                    "focus": "Week 1 · Zone 2 Foundation",
                    "main": "8-10 min Zone 2 · RPE 4 · nasal breathing · stop if you lose conversational pace",
                    "rationale": "We start small and let your body absorb the work.",
                },
                2: {
                    "focus": "Week 2 · Zone 2 Build",
                    "main": "12-15 min Zone 2 · RPE 4-5 · nasal breathing",
                    "rationale": "Same intensity, slightly longer. No hurry.",
                },
                3: {
                    "focus": "Week 3 · Zone 2 Sustain",
                    "main": "15-18 min Zone 2 · RPE 4-5 · same modality preferred",
                    "rationale": "Confidence building · let the engine grow.",
                },
                4: {
                    "focus": "Week 4 · Zone 2 Retest",
                    "main": "10 min Zone 2 retest · same machine, same RPE · note any HR drop",
                    "rationale": "We measure progress by HR at the same workload, not chasing harder.",
                },
            }
            return plans.get(wk_num, plans[1])

        # Cleared for intervals · add controlled pickups W3-W4 if HR recovery is good
        if cleared and not block_intervals:
            plans = {
                1: {
                    "focus": "Week 1 · Zone 2 Baseline",
                    "main": "15-20 min Zone 2 · RPE 4-5 · nasal breathing",
                    "rationale": "Build aerobic base before adding intensity.",
                },
                2: {
                    "focus": "Week 2 · Zone 2 Extended",
                    "main": "20-22 min Zone 2 · RPE 4-5 · same modality · same resistance or +1",
                    "rationale": "Same zone, longer duration · capacity build.",
                },
                3: {
                    "focus": "Week 3 · Zone 2 + Controlled Pickups",
                    "main": ("20 min Zone 2 base · include 4 × 30-sec controlled pickups · "
                              "RPE 6-7 on the pickup, full recovery between"),
                    "rationale": "First intensity exposure · short windows, plenty of recovery.",
                },
                4: {
                    "focus": "Week 4 · Interval Block",
                    "main": ("5 min Z2 warmup · 6 rounds (20s hard · 70s easy) · 5 min cooldown · "
                              "machine you tolerate best"),
                    "rationale": "Threshold exposure · stop if joint tolerance drops or HR doesn't recover.",
                },
            }
            return plans.get(wk_num, plans[1])

        # Default cleared-but-cautious arc · pure aerobic, no intervals
        plans = {
            1: {
                "focus": "Week 1 · Zone 2 Base",
                "main": "15-20 min Zone 2 · RPE 4-5 · nasal breathing only",
                "rationale": "Build aerobic base. If you can't nasal breathe, slow down.",
            },
            2: {
                "focus": "Week 2 · Zone 2 Extended",
                "main": "20-25 min Zone 2 · RPE 4-5 · nasal breathing",
                "rationale": "Same zone, longer duration · capacity build.",
            },
            3: {
                "focus": "Week 3 · Zone 2 Sustained",
                "main": "25 min Zone 2 · same machine, same intensity · own the pace",
                "rationale": "Hold quality at duration · no need to push harder yet.",
            },
            4: {
                "focus": "Week 4 · Zone 2 Retest",
                "main": "10 min Zone 2 retest · same machine, same RPE · note HR drop vs Week 1",
                "rationale": "We measure progress by HR at the same workload.",
            },
        }
        return plans.get(wk_num, plans[1])

    # ------------------------------------------------------
    # BLOCK BUILDERS · INTEGRATION DAY (4x/week)
    # ------------------------------------------------------

    def _build_integration_session(self, assessment: Assessment, day_num: int,
                                   priorities: list[FRAPriority], wk_num: int) -> Session:
        """Full-body integration · focus on worst-scoring joint."""
        worst = self._worst_joint(assessment)
        focus_priority = priorities[0] if priorities else None

        blocks = []

        # Extended mobility work
        if focus_priority:
            blocks.append(self._build_mobility_prep([focus_priority], assessment.constraints,
                                                     wk_num=wk_num, assessment=assessment))

        # Light full-body strength
        blocks.append(Block(
            name="Integration Strength",
            exercises=[
                Exercise(name="Farmer Carry", library="external_training", dose="3x30m"),
                Exercise(name="SL Glute Bridge", library="external_training", dose="3x10/side"),
                Exercise(name="Dead Hang", library="external_training", dose="3x max hold")
            ],
            duration_note="Low intensity · focus on control and integration"
        ))

        # Extended cool-down with PAIL/RAIL
        blocks.append(Block(
            name="Extended Integration",
            exercises=[
                Exercise(name=f"{worst.joint.title()} PAIL/RAIL combo",
                        library="pails_rails", dose="2 rounds · L1 effort"),
                Exercise(name="Full-body CARs sequence", library="cars", dose="1 round all joints"),
                Exercise(name="Breath work · 5 min box breathing", library="manual", dose="5 min")
            ]
        ))

        focus_text = f"Integration · Full Body + {worst.joint.title()} {worst.direction} Focus"
        return Session(
            day_number=day_num,
            day_type="integration",
            focus=focus_text,
            blocks=blocks
        )

    # ------------------------------------------------------
    # LIBRARY SEARCH
    # ------------------------------------------------------

    def _search_library(self, library: str, priority: FRAPriority,
                        constraints: list[str], limit: int = 2) -> list[dict]:
        """Search the unified db for exercises matching a priority."""
        matches = []
        joint_needed = priority.joints[0].lower() if priority.joints else None
        direction_needed = priority.directions[0].lower() if priority.directions else None

        for ex_id, ex in self.db["exercises"].items():
            if ex.get("library") != library:
                continue
            ex_joint = str(ex.get("joint", "")).lower()
            if joint_needed and joint_needed not in ex_joint:
                continue
            # Direction match · check name / modality / setup
            if direction_needed:
                text = (str(ex.get("name", "")) + " " + str(ex.get("modality", "")) +
                        " " + str(ex.get("setup", ""))).lower()
                if direction_needed not in text:
                    continue
            # Constraints filter
            if self._violates_constraints(ex, constraints):
                continue
            matches.append(ex)
            if len(matches) >= limit:
                break

        return matches

    def _violates_constraints(self, ex: dict, constraints: list[str],
                                concerns: list[str] = None) -> bool:
        """Check if an exercise is contraindicated by any client constraint
        OR by a concern flag (bad knee, bad shoulder, etc.).

        Constraints · formal flags from the assessment (SI joint, disc, etc.)
        Concerns · checkbox-driven joint warnings (bad_knee, bad_shoulder, etc.)
        """
        contras = ex.get("contraindications", []) or []
        if not isinstance(contras, list):
            contras = [contras]
        contras_lower = [str(c).lower() for c in contras]

        # 1 · Direct constraint match (existing behavior)
        for constraint in (constraints or []):
            c_lower = constraint.lower().replace(" ", "_")
            if any(c_lower in con for con in contras_lower):
                return True

        # 2 · Concerns-based filtering · check joint of exercise vs bad joints
        if concerns:
            ex_joint = str(ex.get("joint", "")).lower()
            ex_secondary = [str(j).lower() for j in (ex.get("secondary_joints") or [])]
            ex_pattern = str(ex.get("pattern", "")).lower()
            ex_tier = str(ex.get("tier", "")).lower()

            for concern in concerns:
                concern_lower = concern.lower().replace(" ", "_").replace("-", "_")
                bad_joint = self._concern_to_joint(concern_lower)
                if bad_joint is None:
                    continue

                # Heavy-loading patterns through that joint · veto
                if self._is_heavy_load_for_joint(ex_pattern, ex_joint,
                                                   ex_secondary, ex_tier, bad_joint):
                    return True

        return False

    def _concern_to_joint(self, concern: str) -> str | None:
        """Map a checkbox concern flag to the joint it implies.

        Returns canonical joint name or None if not a joint concern.
        """
        mapping = {
            "bad_knee": "knee", "knee": "knee", "knee_pain": "knee",
            "bad_shoulder": "shoulder", "shoulder": "shoulder", "shoulder_pain": "shoulder",
            "lower_back": "lumbar", "low_back": "lumbar", "back": "lumbar",
            "lumbar": "lumbar", "back_pain": "lumbar",
            "hip": "hip", "bad_hip": "hip", "hip_pain": "hip",
            "neck": "cervical", "neck_pain": "cervical", "cervical": "cervical",
            "wrist": "wrist", "bad_wrist": "wrist", "wrist_pain": "wrist",
            "elbow": "elbow", "bad_elbow": "elbow", "elbow_pain": "elbow",
            "ankle": "ankle", "bad_ankle": "ankle", "ankle_pain": "ankle",
        }
        return mapping.get(concern)

    def _is_heavy_load_for_joint(self, pattern: str, joint: str,
                                   secondary: list, tier: str,
                                   bad_joint: str) -> bool:
        """Decide if an exercise heavily loads the bad_joint.

        Rules · err on the side of safety
          - Mobility / CARs / hover / lift-off · never heavy · always OK
          - All other patterns · if bad joint is primary, secondary, OR
            implied by pattern → veto
        """
        # Direct joint match · primary or secondary
        joints_loaded = [joint] + list(secondary)

        # Pattern-based joint mapping · catches things like "squat" → knee
        # even when the entry's joint field doesn't list knee explicitly
        pattern_joint_map = {
            "squat": ["knee", "hip"],
            "squat_unilateral": ["knee", "hip"],
            "squat_lateral": ["knee", "hip"],
            "squat_axial_safe": ["knee", "hip"],
            "lunge": ["knee", "hip"],
            "lunge_lateral": ["knee", "hip"],
            "lunge_unilateral": ["knee", "hip"],
            "hinge": ["lumbar", "hip"],
            "hinge_unilateral": ["hip"],
            "hinge_axial_safe": ["lumbar", "hip"],
            "press": ["shoulder", "elbow"],
            "push_horizontal": ["shoulder", "elbow"],
            "push_vertical": ["shoulder", "elbow", "lumbar"],
            "push_vertical_axial_safe": ["shoulder", "elbow"],
            "pull_horizontal": ["shoulder", "elbow"],
            "pull_vertical": ["shoulder", "elbow"],
            "carry": ["shoulder", "wrist", "lumbar"],
            "hip_extension_teaching": ["hip", "lumbar"],
        }
        if pattern in pattern_joint_map:
            joints_loaded.extend(pattern_joint_map[pattern])

        if bad_joint not in joints_loaded:
            return False  # exercise doesn't touch the bad joint · safe

        # Pure mobility patterns through the bad joint are still OK
        # (CARs, hovers, lift-offs, segmented rolldowns, etc.)
        safe_patterns = {
            "cars", "spinal_articulation", "lift_off", "hover", "err",
            "antiextension", "isometric", "breathing",
        }
        if pattern in safe_patterns:
            return False

        # Anything else loaded through the bad joint = veto
        return True

    def _exercise_from_db_entry(self, entry: dict, priority: FRAPriority) -> Exercise:
        """Convert a unified-db entry into an Exercise."""
        return Exercise(
            name=entry.get("name", "Unknown"),
            library=entry.get("library", "unknown"),
            library_id=entry.get("id"),
            rationale=f"Targets {priority.description}"
        )


# ==========================================================
# CONVENIENCE · PARSE FRA PRIORITY STRINGS
# ==========================================================

def sanitize_priority_description(raw: str) -> str:
    """Clean up coach-typed priority strings before they get rendered.

    Fixes ·
      - "Ir" / "ir" → "IR" · "Er" / "er" → "ER"
      - common typos · "Dorsifexion" → "Dorsiflexion", "Plantar Flexion"
        → "Plantarflexion"
      - sentence-case words that should be Title Case (Hip, Shoulder, etc.)
      - shorthand sides · "R Hip" → "Right Hip", "L Shoulder" → "Left Shoulder"
      - L+R / R+L stays as-is (idiomatic)

    Returns the cleaned string, preserving overall structure.
    """
    if not raw:
        return raw
    s = str(raw).strip()

    # Common typos → canonical spelling (case-insensitive replace, preserve case
    # in the rest of the string)
    typo_fixes = {
        r'\bdorsifexion\b':       'Dorsiflexion',
        r'\bplantar\s+flexion\b': 'Plantarflexion',
        r'\bplanterflexion\b':    'Plantarflexion',
        r'\bdorsi\s+flexion\b':   'Dorsiflexion',
        r'\beversion\b':          'Eversion',
        r'\binversion\b':         'Inversion',
        r'\bplantarflexion\b':    'Plantarflexion',
        r'\bdorsiflexion\b':      'Dorsiflexion',
        r'\bflexion\b':           'Flexion',
        r'\bextension\b':         'Extension',
        r'\babduction\b':         'Abduction',
        r'\badduction\b':         'Adduction',
        r'\brotation\b':          'Rotation',
        r'\binternal\b':          'Internal',
        r'\bexternal\b':          'External',
    }
    for pattern, replacement in typo_fixes.items():
        s = re.sub(pattern, replacement, s, flags=re.IGNORECASE)

    # Capitalize joint names
    joint_words = ['hip', 'knee', 'ankle', 'foot', 'shoulder', 'scapular',
                   'elbow', 'wrist', 'hand', 'thoracic', 'cervical', 'lumbar',
                   'spine', 'neck', 'back', 'hamstring', 'quad', 'glute', 'calf']
    for w in joint_words:
        s = re.sub(rf'\b{w}\b', w.title(), s, flags=re.IGNORECASE)

    # IR / ER → uppercase (whole-word only · "ire" or "her" not affected)
    s = re.sub(r'\bIr\b', 'IR', s)
    s = re.sub(r'\bir\b', 'IR', s)
    s = re.sub(r'\bEr\b', 'ER', s)
    s = re.sub(r'\ber\b', 'ER', s)

    # Side shorthand · "R Hip" → "Right Hip", "L Shoulder" → "Left Shoulder"
    # Only at start of token AND followed by a known joint
    s = re.sub(r'\b[Rr]\s+(Hip|Knee|Ankle|Foot|Shoulder|Elbow|Wrist|Hand|Scapular)\b',
                r'Right \1', s)
    s = re.sub(r'\b[Ll]\s+(Hip|Knee|Ankle|Foot|Shoulder|Elbow|Wrist|Hand|Scapular)\b',
                r'Left \1', s)

    # Collapse whitespace
    s = re.sub(r'\s+', ' ', s).strip()
    return s


def parse_fra_priority(description: str) -> FRAPriority:
    """Parse strings like 'Hip IR L+R' or 'Left Shoulder External Rotation'."""
    desc = sanitize_priority_description(description)
    lower = desc.lower()

    # Region detection
    lower_body_keywords = {"hip", "knee", "ankle", "foot", "toe", "hamstring", "quad", "glute", "calf"}
    upper_body_keywords = {"shoulder", "scapular", "elbow", "wrist", "hand"}
    spine_keywords = {"cervical", "thoracic", "lumbar", "spine", "neck", "back"}

    region = "unknown"
    joints = []
    for k in lower_body_keywords:
        if k in lower:
            region = "lower_body"
            joints.append(k)
    for k in upper_body_keywords:
        if k in lower:
            region = "upper_body"
            joints.append(k)
    for k in spine_keywords:
        if k in lower:
            region = "spine"
            joints.append(k)

    # Direction parsing · word-boundary aware to avoid "ir" matching "sides"
    import re
    directions = []
    has_ir = bool(re.search(r'\bir\b', lower)) or "internal rotation" in lower
    has_er = bool(re.search(r'\ber\b', lower)) or "external rotation" in lower
    if has_ir: directions.append("IR")
    if has_er: directions.append("ER")
    if "flexion" in lower and "plantar" not in lower and "dorsi" not in lower:
        directions.append("flexion")
    if "extension" in lower: directions.append("extension")
    if "plantarflexion" in lower or "plantar" in lower: directions.append("plantarflexion")
    if "dorsiflexion" in lower: directions.append("dorsiflexion")
    if "eversion" in lower: directions.append("eversion")
    if "inversion" in lower: directions.append("inversion")
    if "abduction" in lower: directions.append("abduction")
    if "adduction" in lower: directions.append("adduction")
    if "restriction" in lower and not directions: directions.append("restricted")

    # Side parsing
    sides = []
    if "l+r" in lower or "bilateral" in lower or "both" in lower:
        sides = ["L", "R"]
    elif "left" in lower or lower.startswith("l "):
        sides = ["L"]
    elif "right" in lower or lower.startswith("r "):
        sides = ["R"]
    else:
        sides = ["bilateral"]

    return FRAPriority(
        description=desc,
        joints=joints or ["unknown"],
        directions=directions or ["general"],
        sides=sides,
        region=region
    )


# ==========================================================
# MAIN · DEMO USING MATT'S ASSESSMENT
# ==========================================================

def build_matt_program():
    """Demo · build Matt's 4-week program from the assessment we have."""
    matt = Assessment(
        name="Matt",
        age_range="late 40s",
        sex="M",
        background="ex-military",
        training_frequency=3,
        primary_goal="Restore mobility, improve strength, optimize joint function, and build an aerobic base",
        fra_priorities=[
            parse_fra_priority("Hip Internal Rotation L+R"),
            parse_fra_priority("Left Shoulder External Rotation"),
            parse_fra_priority("Ankle Plantarflexion + Eversion"),
            parse_fra_priority("Bilateral Hamstring Restriction"),
        ],
        strength_markers=["inverted_rows", "incline_pushups", "lat_pulldown", "landmine_sa_press"],
        training_age_years=6,
        conditioning_level="moderate",
        constraints=["SI_joint_sensitivity", "no_axial_loading"],
        mobility_map=[
            MobilityRating(joint="hip", direction="IR", side="L", rating="red"),
            MobilityRating(joint="hip", direction="IR", side="R", rating="red"),
            MobilityRating(joint="shoulder", direction="ER", side="L", rating="red"),
            MobilityRating(joint="ankle", direction="plantarflexion", side="bilateral", rating="yellow"),
            MobilityRating(joint="ankle", direction="eversion", side="bilateral", rating="yellow"),
            MobilityRating(joint="hamstring", direction="flexibility", side="bilateral", rating="red"),
        ],
        body_comp={},
        # progression_mode is resolved by the generator from the client's
    # ProgressionProfile · it is not an input, and it is no longer the
    # word "autoregulated".
        strength_marker_results={
            "inverted_rows": "18 reps (30s)",
            "incline_pushups": "22 reps (30s)",
            "lat_pulldown": "140x3",
            "landmine_sa_press": "6 x 40 lbs"
        }
    )

    generator = Generator()
    program = generator.build_program(matt, block_number=1)

    out_path = str(Path(__file__).resolve().parent.parent / "examples" / "matt_program.json")
    program.to_json(out_path)

    # Summary print
    print(f"Generated · {program.client_name} · Block {program.block_number}")
    print(f"Weeks · {len(program.weeks)}")
    for week in program.weeks:
        print(f"\n--- Week {week.week_number} · {week.intent} ---")
        for session in week.sessions:
            print(f"  Day {session.day_number} · {session.focus}")
            for block in session.blocks:
                print(f"    [{block.name}]  · {len(block.exercises)} exercises")
                for ex in block.exercises[:3]:
                    print(f"      · {ex.name} — {ex.dose}")

    print(f"\nProgram JSON written · {out_path}")
    return program


if __name__ == "__main__":
    build_matt_program()
