...
def normalize_cardio_profile(profile, concerns=None, constraints_rich=None) -> dict:
    ...
    # Per spec · a modality marked AVOID cannot also appear as a SECONDARY
    # alternative. Coaches who flag something to avoid don't want it offered
    # as a fallback, even if they accidentally checked both boxes.
    avoid_set = set(avoid)
    secondary = [m for m in secondary if m not in avoid_set]
...
# Treadmill is added selectively · safe for low-back-sensitive, shoulder-sensitive,
# and as a 4th option for knee-sensitive (walking only). Not safe for wrist if
# clients use handrails or hip if hip flexion provocative.
_SAFE_MACHINES_BY_LIMIT = {
    "knee_sensitive":     ["stationary_bike", "upright_bike", "arc_trainer", "treadmill"],
    "low_back_sensitive": ["stationary_bike", "upright_bike", "treadmill", "arc_trainer"],
    "shoulder_sensitive": ["stationary_bike", "upright_bike", "treadmill", "arc_trainer"],
    "hip_sensitive":      ["upright_bike", "stationary_bike", "arc_trainer"],
    "wrist_sensitive":    ["stationary_bike", "upright_bike", "treadmill", "arc_trainer"],
}

# Machines flagged as risky for each limitation (not just "less safe" but
# actively contraindicated unless explicitly tolerated as a secondary).
# Treadmill RUNNING is treated as risky for impact-sensitive limitations,
# but the cardio engine generates walking-only progressions when treadmill
# is selected (no running unless explicitly cleared) so the modality itself
# is not blanket-blocked.
_RISKY_MACHINES_BY_LIMIT = {
    # Knee-sensitive · spec says prefer bike/arc · rower is not preferred
    # because of the knee flexion under load. Still usable if tolerated.
    "knee_sensitive":     ["rower"],
...
def choose_primary_cardio_machine(normalized: dict) -> tuple[str, str]:
    ...
    Returns (machine_id, rationale_text). For decision audit (rejected
    alternatives, contradictions resolved), call `decide_machine_with_audit`.
    """
    machine, rationale, _ = decide_machine_with_audit(normalized)
    return (machine, rationale)


def decide_machine_with_audit(normalized: dict) -> tuple[str, str, list[dict]]:
    """Same as choose_primary_cardio_machine but also returns a list of
    rejected candidates with the reason they were rejected, suitable for
    rendering in the coach appendix DECISIONS section.

    Each rejected entry: {"machine": str, "reason": str}
    """
    rejected = []
    ...
    if primary and _machine_is_safe(primary, normalized):
        return (primary,
                f"Coach-selected primary modality · client tolerates",
                rejected)

    # 2 · primary is set but risky · log it as rejected with reason
    if primary and not _machine_is_safe(primary, normalized):
        ...
        if primary in avoid:
            conflicting.append("on avoid list")
        rejected.append({
            "machine": primary,
            "reason": (f"Coach primary · conflicts with: {', '.join(conflicting)}"
                       if conflicting else "Coach primary · routed away by safety check"),
        })
        rationale_part = (f"Coach primary ({MODALITY_DISPLAY.get(primary, primary)}) "
...
# Coach-selected primary always wins · this is the fallback when no primary
# is set AND no limit-specific safe list applies.
_GENERAL_PRIORITY = [
    "stationary_bike", "upright_bike", "arc_trainer",
    "treadmill", "rower", "skierg", "assault_bike",
]


# ─── 2b · CONTRADICTION DETECTION ─────────────────────────────

def detect_contradictions(normalized: dict) -> list[dict]:
    """Audit the normalized profile for contradictions before render.

    Returns a list of dicts ·
        {
            "severity": "error" | "warning" | "info",
            "message":  human-readable description,
            "resolution": what the engine did about it,
        }

    Contradictions detected ·
      - Primary modality is also in the avoid list
      - Z2 baseline test was on a now-avoided machine
      - Interval test was on a now-avoided machine
      - Knee-sensitive but bike + arc all in avoid list
      - Low-back-sensitive but rower selected without explicit tolerance
      - Shoulder-sensitive but SkiErg/Assault selected without explicit tolerance
      - Interval clearance blocked but coach left primary on a sprint-only machine
    """
    out = []
    primary = normalized.get("primary_modality")
    secondary = set(normalized.get("secondary_modalities", []))
    avoid = set(normalized.get("avoid_modalities", []))
    limits = set(normalized.get("limitations", []))
    z2 = normalized.get("z2_baseline") or {}
    iv = normalized.get("interval_test") or {}

    def _disp(m):
        return MODALITY_DISPLAY.get(m, m or "—")

    # 1 · Primary in avoid list
    if primary and primary in avoid:
        out.append({
            "severity": "error",
            "message": f"Primary modality ({_disp(primary)}) is also on the avoid list",
            "resolution": "Engine routed to a safe alt and ignored the primary",
        })

    # 2 · Z2 baseline machine is in avoid list
    z2_machine = z2.get("machine")
    if z2_machine and z2_machine in avoid:
        out.append({
            "severity": "warning",
            "message": (f"Z2 baseline was tested on {_disp(z2_machine)} but it's now in the avoid list"),
            "resolution": "Baseline data archived · machine no longer used",
        })

    # 3 · Interval test machine is in avoid list
    iv_machine = iv.get("machine")
    if iv_machine and iv_machine in avoid:
        out.append({
            "severity": "warning",
            "message": (f"Interval test was on {_disp(iv_machine)} but it's now in the avoid list"),
            "resolution": "Interval data archived · machine no longer used",
        })

    # 4 · Knee-sensitive but bike+arc all in avoid list
    if "knee_sensitive" in limits:
        knee_safe = {"stationary_bike", "upright_bike", "arc_trainer"}
        avoided_safe = knee_safe & avoid
        if len(avoided_safe) == len(knee_safe):
            out.append({
                "severity": "warning",
                "message": ("Knee-sensitive client · all preferred machines "
                            "(stationary bike, upright bike, arc trainer) are on the avoid list"),
                "resolution": "Engine fell back to default · coach should review machine selection",
            })

    # 5 · Low-back-sensitive on rower without explicit tolerance
    if "low_back_sensitive" in limits:
        if primary == "rower" and "rower" not in secondary:
            out.append({
                "severity": "warning",
                "message": ("Low-back-sensitive client routed to Rower · spec says avoid "
                            "unless specifically tolerated"),
                "resolution": "Mark Rower as a tolerated secondary to confirm, or change primary",
            })

    # 6 · Shoulder-sensitive on SkiErg/Assault without explicit tolerance
    if "shoulder_sensitive" in limits:
        if primary in ("skierg", "assault_bike") and primary not in secondary:
            out.append({
                "severity": "warning",
                "message": (f"Shoulder-sensitive client routed to {_disp(primary)} · "
                            f"arms-driven modality"),
                "resolution": "Mark as a tolerated secondary to confirm, or change primary",
            })

    # 7 · Interval clearance blocked but coach left primary as a sprint-only machine
    clearance = determine_interval_clearance(normalized)
    if clearance == "blocked":
        if primary == "assault_bike" and "assault_bike" not in secondary:
            out.append({
                "severity": "info",
                "message": ("Intervals blocked but Assault Bike picked as primary · "
                            "machine is fine for Z2, just confirm the choice"),
                "resolution": "Z2 work on Assault Bike is acceptable · no sprint prescriptions",
            })

    return out


# ─── 2c · SAFE-SUBSTITUTE HELPER FOR PRIORITY-MAPPED MOVES ───

# Map exercise names that are unsafe for specific limits to a safer
# alternative. Used by the strength program builder when an FRA priority
# would otherwise produce a knee-aggressive (or back/shoulder-aggressive)
# pick.
_PRIORITY_PICK_SUBSTITUTIONS = {
    "knee_sensitive": {
        "Deep Squat Hold":              "Heel-Elevated Deep Squat",
        "Bear Sit to Deep Squat Stand": "90/90 Hip Hold",
        "Stand to Deep Squat":          "Heel-Elevated Deep Squat",
        "Deep Squat with T-Spine Reach":"Heel-Elevated Deep Squat",
    },
    "low_back_sensitive": {
        "Deep Squat Hold":              "Supported 90/90 Hip Hold",
    },
    "shoulder_sensitive": {
        "Child's Pose with Arm Reach":  "Supine Lat Stretch (no arm reach)",
        "Sleeper Stretch":              "Cross-Body Shoulder Stretch (mild)",
    },
}


def safe_substitute_for_priority_pick(name: str, normalized: dict) -> str:
    """If `name` is contraindicated by an active limitation, return the safe
    substitute. Otherwise return the original name."""
    if not name or not normalized:
        return name
    for limit in normalized.get("limitations", []):
        subs = _PRIORITY_PICK_SUBSTITUTIONS.get(limit, {})
        if name in subs:
            return subs[name]
    return name
...
def generate_cardio_progression(normalized: dict) -> dict:
    ...
    is_treadmill = machine_id == "treadmill"
    has_knee = "knee_sensitive" in limits or "meniscus_flare" in limits

    def _wk(focus, main, rationale, coach=None):
        # Treadmill rewrite · per spec, treadmill prescriptions ALWAYS read
        # as walking. The generic "X min Zone 2" gets re-prefixed to
        # "Treadmill walk · X min Zone 2" with incline and pace guidance.
        if is_treadmill:
            # Strip the leading machine-agnostic phrasing and rebuild
            # for clarity. Block 1 default = walking, no running.
            if "Retest" in focus:
                main = (f"Repeat 10-min treadmill walk baseline · same incline · "
                         f"compare pace, HR, RPE, and joint response")
            elif "Foundation" in focus or "Baseline" in focus:
                main = (f"Treadmill walk · 10-15 min · RPE 4-5 · "
                         f"conversational pace · 0-3% incline")
            elif "Build" in focus:
                main = (f"Treadmill walk · 15-18 min · RPE 4-5 · "
                         f"same pace · add time before speed")
            elif "Capacity" in focus or "Sustain" in focus:
                main = (f"Treadmill walk · 18-22 min · RPE 4-5 · "
                         f"optional 1-3% incline if joints stay calm")
            else:
                # Default · prefix walking and incline
                main = f"Treadmill walk · {main}"
            # Append constraint-specific notes
            if has_knee:
                coach = ((coach + " · ") if coach else "") + \
                         "Treadmill walking only · no running or incline intervals this block"
            elif clearance in ("blocked", "z2_only"):
                coach = ((coach + " · ") if coach else "") + \
                         "Zone 2 treadmill walking only · no intervals, no sprints"
        return {
            "focus": focus,
            "main": main,
            "rationale": rationale,
...
def generate_cardio_coach_flags(normalized: dict) -> list[str]:
    ...
    # Treadmill + knee-sensitive · special-case warning per spec
    if normalized.get("primary_modality") == "treadmill" and "knee_sensitive" in limits:
        flags.append("Treadmill marked as primary, but knee-sensitive client. "
                     "Use walking only if symptoms stay calm; bike remains safer default.")
    if "low_back_sensitive" in limits:
...
_FINISHER_VETOES_BY_LIMIT = {
    "knee_sensitive": [
        ...
        "alternating reverse lunge", "reverse lunge", "reverse lunges",
        "jump rope",
        "kettlebell swing",  # aggressive · veto by default for knee-sensitive
        "deep squat",         # catches "Deep Squat Hold" too
        "box jump", "broad jump", "depth jump",
        "burpee", "skater",
        "step-up", "step up",
        "plyo push-up",
        "pistol squat", "shrimp squat",
        # Treadmill running/sprints/incline intervals · always out for knee
        "treadmill sprint", "treadmill interval", "treadmill running",
        "treadmill run", "incline sprint", "lateral treadmill",
    ],
    "low_back_sensitive": [
        ...
        "heavy hinge",
        # Treadmill · aggressive incline can drive lumbar extension
        "aggressive incline", "incline sprint", "treadmill sprint",
    ],
    "shoulder_sensitive": [
        "skierg",
    ...
    "not_cleared_for_intervals": [
        ...
        "hop", "burpee", "skater",
        # Treadmill · explicit interval/sprint variants get caught by "sprint"
        # and "interval" already, but list pickups separately for clarity
        "treadmill pickup", "controlled pickup",
    ],
    "meniscus_flare": [
        "deep squat", "lateral bound", "jump rope", "box jump",
        "burpee", "treadmill running", "treadmill run",
        "treadmill sprint", "incline sprint",
    ],
}
